...
use {
    ...
    bovey_metrics::{inc_new_counter_error, inc_new_counter_info},
    bovey_poh::poh_recorder::WorkingBankEntry,
    bovey_runtime::{bank::MAX_LEADER_SCHEDULE_STAKES, bank_forks::BankForks},
    bovey_sdk::{
        clock::Slot,
        pubkey::Pubkey,
        signature::Keypair,
...
pub enum Error {
    #[error(transparent)]
    Blockstore(#[from] bovey_ledger::blockstore::BlockstoreError),
    #[error(transparent)]
    ClusterInfo(#[from] bovey_gossip::cluster_info::ClusterInfoError),
    ...
    #[error(transparent)]
    TransportError(#[from] bovey_sdk::transport::TransportError),
    ...
    fn new(
        ...
        thread_hdls.extend(socks.into_iter().map(|sock| {
            ...
            let run_transmit = move || loop {
                ...
                let res = Self::handle_error(res, "bovey-broadcaster-transmit");
                if let Some(res) = res {
                    return res;
                }
            };
            Builder::new()
                .name("bovBroadcastTx".to_string())
                .spawn(run_transmit)
                .unwrap()
        }));
        ...
        thread_hdls.push({
            ...
            let run_record = move || loop {
                ...
                let res = Self::handle_error(res, "bovey-broadcaster-record");
                if let Some(res) = res {
                    return res;
                }
            };
            Builder::new()
                .name("bovBroadcastRec".to_string())
                .spawn(run_record)
                .unwrap()
        });

        let retransmit_thread = Builder::new()
            .name("bovBroadcastRtx".to_string())
            .spawn(move || loop {
                if let Some(res) = Self::handle_error(
                    ...
                    "bovey-broadcaster-retransmit-check_retransmit_signals",
                ) {
                    return res;
                }
            })
...
pub mod test {
    use {
        ...
        rand::Rng,
        bovey_entry::entry::create_ticks,
