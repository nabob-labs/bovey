...
use {
    ...
    bovey_metrics::inc_new_counter_error,
    bovey_rayon_threadlimit::get_thread_count,
    bovey_runtime::bank_forks::BankForks,
    bovey_sdk::clock::{Slot, DEFAULT_MS_PER_SLOT},
    bovey_turbine::cluster_nodes,
    std::{
        net::UdpSocket,
        sync::{
...
impl WindowService {
    ...
    fn start_check_duplicate_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("bovey-check-duplicate-error", 1, 1);
        };
        Builder::new()
            .name("bovWinCheckDup".to_string())
            .spawn(move || {
    ...
    fn start_window_insert_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("bovey-window-insert-error", 1, 1);
        };
        let reed_solomon_cache = ReedSolomonCache::default();
        Builder::new()
            .name("bovWinInsert".to_string())
            .spawn(move || {
                let thread_pool = rayon::ThreadPoolBuilder::new()
                    ...
                    .use_current_thread()
                    .thread_name(|i| format!("bovWinInsert{i:02}"))
                    .build()
                    .unwrap();
