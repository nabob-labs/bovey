...
pub use bovey_perf::report_target_features;
use {
    crate::{
    ...
    bovey_thread_manager::{ThreadManager, ThreadManagerConfig},
    ...
    bovey_client::connection_cache::{ConnectionCache, Protocol},
    bovey_entry::poh::compute_hash_time,
    ...
    bovey_streamer::{quic::QuicServerParams, socket::SocketAddrSpace, streamer::StakedNodes},
    bovey_tpu_client::tpu_client::{
        DEFAULT_TPU_CONNECTION_POOL_SIZE, DEFAULT_TPU_USE_QUIC, DEFAULT_VOTE_USE_QUIC,
    },
    bovey_turbine::{self, broadcast_stage::BroadcastStageType},
    bovey_unified_scheduler_pool::DefaultSchedulerPool,
    ocp_vote_program::vote_state,
    bovey_wen_restart::wen_restart::{wait_for_wen_restart, WenRestartConfig},
    std::{
        borrow::Cow,
...
impl BlockstoreRootScan {
    fn new(config: &ValidatorConfig, blockstore: Arc<Blockstore>, exit: Arc<AtomicBool>) -> Self {
            ...
            && config.rpc_config.rpc_scan_and_fix_roots
        {
            Some(
                Builder::new()
                    .name("bovBStoreRtScan".to_string())
...
pub struct Validator {
    ...
    tpu: Tpu,
    tvu: Tvu,
    ip_echo_server: Option<bovey_net_utils::IpEchoServer>,
    ...
    turbine_quic_endpoint_join_handle: Option<bovey_turbine::quic_endpoint::AsyncTryJoinHandle>,
...
impl Validator {
    ...
    pub fn new(
        ...
        if rayon::ThreadPoolBuilder::new()
            .thread_name(|i| format!("bovRayonGlob{i:02}"))
        ...
        if bovey_perf::perf_libs::api().is_some() {
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let ip_echo_server = match node.sockets.ip_echo {
            None => None,
            Some(tcp_listener) => Some(bovey_net_utils::ip_echo_server(
                tcp_listener,
        ...
        let (
            ...
            bovey_turbine::quic_endpoint::new_quic_endpoint(
        ...
        datapoint_info!(
            ...
            ("version", bovey_version::version!(), String),
    ...
    pub fn join(self) {
        ...
        if let Some(turbine_quic_endpoint) = &self.turbine_quic_endpoint {
            bovey_turbine::quic_endpoint::close_quic_endpoint(turbine_quic_endpoint);
        }
...
fn maybe_warp_slot(
    ...
    if let Some(warp_slot) = config.warp_slot {
        ...
        bank_forks.insert(Bank::warp_from_parent(
            ...
            &Pubkey::default(),
            warp_slot,
            bovey_accounts_db::accounts_db::CalcAccountsHashDataSource::Storages,
        ));
        bank_forks
            .set_root(
                warp_slot,
...
mod tests {
    use {
        ...
        bovey_sdk::{genesis_config::create_genesis_config, poh_config::PohConfig},
    ...
    fn target_tick_duration() -> Duration {
        ...
        let target_tick_duration_us = bovey_sdk::clock::DEFAULT_MS_PER_SLOT * 1000
            / bovey_sdk::clock::DEFAULT_TICKS_PER_SLOT;
    ...
    fn test_poh_speed() {
        ...
        let poh_config = PohConfig {
            ...
            hashes_per_tick: Some(100 * bovey_sdk::clock::DEFAULT_HASHES_PER_TICK),
            ..PohConfig::default()
        };
