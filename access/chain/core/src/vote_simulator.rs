...
use {
    ...
    bovey_sdk::{clock::Slot, hash::Hash, pubkey::Pubkey, signature::Signer},
    bovey_vote::vote_transaction,
    ocp_vote_program::vote_state::{process_vote_unchecked, Lockout, TowerSync},
    std::{
                ...
                if vote.contains(&parent) {
                        ...
                        parent_bank.get_vote_account(&keypairs.vote_keypair.pubkey())
                    {
                        ...
                        process_vote_unchecked(
                            &mut vote_state,
                            ocp_vote_program::vote_state::Vote::new(
                                vec![parent],
                                parent_bank.hash(),
                            ),
                        )
