...
//!
//! This crate implements 3 bovey-runtime traits [`InstalledScheduler`], [`UninstalledScheduler`]
...
//! changes) into the associated [`Bank`](bovey_runtime::bank::Bank) via `bovey-ledger`'s helper
...
use {
    bovey_banking_stage_ingress_types::{BankingPacketBatch, BankingPacketReceiver},
    ...
    log::*,
    scopeguard::defer,
    bovey_ledger::blockstore_processor::{
        execute_batch, TransactionBatchWithIndexes, TransactionStatusSender,
    },
    bovey_poh::poh_recorder::{RecordTransactionsSummary, TransactionRecorder},
    bovey_pubkey::Pubkey,
    bovey_runtime::{
    ...
    bovey_transaction_error::{TransactionError, TransactionResult as Result},
    bovey_unified_scheduler_logic::{
...
/// `bovey-runtime`, because it contains some internal fields, whose types aren't available in
/// `bovey-runtime` ( [`TransactionStatusSender`] and [`TransactionRecorder`]). Refer to the doc
/// comment with a diagram at [`bovey_runtime::installed_scheduler_pool::InstalledScheduler`] for
...
// the memory.
//
// Along the lines, this isn't problematic for the development settings (= bovey-test-validator),
...
/// `bovey-unified-scheduler-logic` for the crate's original intent (separation of concerns from
...
/// bovey-runtime code as `Box<dyn _>`.  This also implements the [`SpawnableScheduler`] subtrait
...
/// [`BankWithScheduler`](bovey_runtime::installed_scheduler_pool::BankWithScheduler) for
/// type-level protection against double-use by different banks. As soon as the bank is
/// [`is_complete()`](`bovey_runtime::bank::Bank::is_complete`) (i.e. ready for freezing), the
...
/// [`BankForks`](bovey_runtime::bank_forks::BankForks) under forky (unsteady rooting) cluster
...
/// [`BankForks::insert()`](bovey_runtime::bank_forks::BankForks::insert) via
...
///     }
///     state bovey-unified-scheduler-pool {
    ...
    fn start_threads(
                ...
                loop {
                    let (task, sender) = select_biased! {
                        ...
                        recv(handler_context.banking_packet_receiver) -> banking_packet => {
                            // See bovey_core::banking_stage::unified_scheduler module doc as to
        ...
        self.handler_threads = (0..self.pool.handler_count)
            .map({
                |thx| {
                    thread::Builder::new()
                        .name(format!("bovScHandler{:02}", thx))
...
mod tests {
    use {
        ...
        assert_matches::assert_matches,
        bovey_clock::{Slot, MAX_PROCESSING_AGE},
        bovey_hash::Hash,
        ...
        bovey_poh::poh_recorder::create_test_recorder_with_index_tracking,
        bovey_pubkey::Pubkey,
        bovey_runtime::{
            bank::Bank,
        ...
        bovey_timings::ExecuteTimingType,
        bovey_transaction::sanitized::SanitizedTransaction,
    ...
    fn test_task_handler_poh_recording(tx_result: TxResult, should_succeed_to_record_to_poh: bool) {
        ...
        let GenesisConfigInfo {
            ...
            ref mint_keypair,
            ..
        } = bovey_ledger::genesis_utils::create_genesis_config(10_000);
        let bank = Bank::new_for_tests(&genesis_config);
        let bank_forks = BankForks::new_rw_arc(bank);
        ...
        if should_succeed_to_record_to_poh {
            if expected_tx_result.is_ok() {
                ...
                assert_matches!(
                    ...
                    Ok((_, (bovey_entry::entry::Entry {transactions, ..} , _)))
