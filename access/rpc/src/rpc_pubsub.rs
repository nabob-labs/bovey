...
use {
    ...
    bovey_account_decoder::{UiAccount, UiAccountEncoding},
    ...
    bovey_sdk::{clock::Slot, pubkey::Pubkey, signature::Signature},
    bovey_transaction_status::UiTransactionEncoding,
    std::{str::FromStr, sync::Arc},
};

// We have to keep both of the following traits to not break backwards compatibility.
// `RpcBovPubSubInternal` is actually used by the current PubSub API implementation.
// `RpcBovPubSub` and the corresponding `gen_client` module are preserved
...
pub use internal::RpcBovPubSubInternal;
...
mod internal {
    ...
    #[rpc]
    pub trait RpcBovPubSubInternal {
        // Get notification every time account data is changed
        // Accepts pubkey parameter as base-58 encoded string
...
impl RpcBovPubSubInternal for RpcBovPubSubImpl {
    fn account_subscribe(
        &self,
        pubkey_str: String,
        config: Option<RpcAccountInfoConfig>,
...
mod tests {
    use {
        ...
        serial_test::serial,
        bovey_account_decoder::{parse_account_data::parse_account_data_v3, UiAccountEncoding},
        bovey_rpc_client_api::response::{
        ...
        ocp_vote_program::vote_state::Vote,
        std::{
            sync::{
    ...
    fn test_account_subscribe() {
        ...
        let new_stake_authority = bovey_pubkey::new_rand();
    ...
    fn test_get_version() {
        ...
        let expected_version = bovey_version::Version::default();
