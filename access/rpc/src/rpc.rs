//! The `rpc` module implements the Bovey RPC interface.
#[cfg(feature = "dev-context-only-utils")]
use bovey_runtime::installed_scheduler_pool::BankWithScheduler;
use {
    crate::{
    ...
    jsonrpc_derive::rpc,
    bovey_account_decoder::{
        encode_ui_account,
        parse_account_data::OcpTokenAdditionalDataV2,
        parse_token::{is_known_ocp_token_id, token_amount_to_ui_amount_v3, UiTokenAmount},
    ...
    bovey_client::connection_cache::Protocol,
    bovey_entry::entry::Entry,
    bovey_faucet::faucet::request_airdrop_transaction,
    bovey_feature_set as feature_set,
    bovey_gossip::cluster_info::ClusterInfo,
    bovey_inline_ocp::{
        token::{OCP_TOKEN_ACCOUNT_MINT_OFFSET, OCP_TOKEN_ACCOUNT_OWNER_OFFSET},
    ...
    bovey_metrics::inc_new_counter_info,
    bovey_perf::packet::PACKET_DATA_SIZE,
    bovey_rpc_client_api::{
        config::*,
    ...
    bovey_runtime_transaction::runtime_transaction::RuntimeTransaction,
    bovey_sdk::{
    ...
    bovey_send_transaction_service::send_transaction_service::TransactionInfo,
    ocp_stake_program,
    bovey_storage_bigtable::Error as StorageError,
    bovey_transaction_status::{
    ...
    ocp_vote_program::vote_state::MAX_LOCKOUT_HISTORY,
    ocp_token_2022::{
        extension::{
...
#[cfg(test)]
use {
    bovey_client::connection_cache::ConnectionCache,
    bovey_gossip::contact_info::ContactInfo,
    bovey_ledger::get_tmp_ledger_path,
    bovey_runtime::commitment::CommitmentSlots,
    bovey_send_transaction_service::{
        send_transaction_service::SendTransactionService, tpu_info::NullTpuInfo,
    },
    bovey_streamer::socket::SocketAddrSpace,
};
...
impl Default for RpcBigtableConfig {
    fn default() -> Self {
        let bigtable_instance_name = bovey_storage_bigtable::DEFAULT_INSTANCE_NAME.to_string();
        let bigtable_app_profile_id = bovey_storage_bigtable::DEFAULT_APP_PROFILE_ID.to_string();
        Self {
            enable_bigtable_ledger_upload: false,
            bigtable_instance_name,
            bigtable_app_profile_id,
            timeout: None,
            max_message_size: bovey_storage_bigtable::DEFAULT_MAX_MESSAGE_SIZE,
        }
    }
}
...
pub struct JsonRpcRequestProcessor {
    ...
    bigtable_ledger_storage: Option<bovey_storage_bigtable::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
    ...
    pub fn new_from_bank(
        ...
        let cluster_info = Arc::new({
            ...
            let contact_info = ContactInfo::new_localhost(
                &keypair.pubkey(),
                bovey_sdk::timing::timestamp(), // wallclock
            );
    ...
    pub async fn get_program_accounts(
        ...
        let keyed_accounts = {
            if let Some(owner) = get_ocp_token_owner_filter(&program_id, &filters) {
                self.get_filtered_ocp_token_accounts_by_owner(
                    Arc::clone(&bank),
                    program_id,
                    owner,
                    filters,
                ...
                .await?
            } else if let Some(mint) = get_ocp_token_mint_filter(&program_id, &filters) {
                self.get_filtered_ocp_token_accounts_by_mint(
                    Arc::clone(&bank),
                    program_id,
                    mint,
                    filters,
        ...
        let accounts = if is_known_ocp_token_id(&program_id)
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, bovey_storage_bigtable::Error>,
    ) -> Result<()> {
        if let Err(bovey_storage_bigtable::Error::BlockNotFound(slot)) = result {
    ...
    pub fn get_token_account_balance(
        ...
        if !is_known_ocp_token_id(account.owner()) {
    ...
    pub fn get_token_supply(
        ...
        if !is_known_ocp_token_id(mint_account.owner()) {
            return Err(Error::invalid_params(
    ...
    pub async fn get_token_largest_accounts(
        ...
        if !is_known_ocp_token_id(&mint_owner) {
            return Err(Error::invalid_params(
                "Invalid param: not a Token mint".to_string(),
            ));
        }
        ...
        for (address, account) in self
            .get_filtered_ocp_token_accounts_by_mint(
                Arc::clone(&bank),
                mint_owner,
                mint,
                vec![],
    ...
    pub async fn get_token_accounts_by_owner(
        ...
        let keyed_accounts = self
            .get_filtered_ocp_token_accounts_by_owner(
                Arc::clone(&bank),
                token_program_id,
                owner,
                filters,
    ...
    /// Get an iterator of ocp-token accounts by owner address
    async fn get_filtered_ocp_token_accounts_by_owner(
        &self,
        bank: Arc<Bank>,
        program_id: Pubkey,
        owner_key: Pubkey,
        ...
        filters.push(RpcFilterType::Memcmp(Memcmp::new_raw_bytes(
            OCP_TOKEN_ACCOUNT_OWNER_OFFSET,
        ...
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::OcpTokenOwner)
        {
            ...
            self.get_filtered_indexed_accounts(
                &bank,
                &IndexKey::OcpTokenOwner(owner_key),
                &program_id,
                filters,
                sort_results,
            )
    ...
    /// Get an iterator of ocp-token accounts by mint address
    async fn get_filtered_ocp_token_accounts_by_mint(
        &self,
        bank: Arc<Bank>,
        program_id: Pubkey,
        mint_key: Pubkey,
        ...
        filters.push(RpcFilterType::Memcmp(Memcmp::new_raw_bytes(
            OCP_TOKEN_ACCOUNT_MINT_OFFSET,
            mint_key.to_bytes().into(),
        )));
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::OcpTokenMint)
        {
            ...
            self.get_filtered_indexed_accounts(
                &bank,
                &IndexKey::OcpTokenMint(mint_key),
                &program_id,
                filters,
                sort_results,
            )
    ...
    fn get_stake_minimum_delegation(&self, config: RpcContextConfig) -> Result<RpcResponse<u64>> {
        ...
        let stake_minimum_delegation =
            ocp_stake_program::get_minimum_delegation(&bank.feature_set);
...
fn verify_transaction(
    ...
    let move_precompile_verification_to_vm =
        feature_set.is_active(&feature_set::move_precompile_verification_to_vm::id());
    if !move_precompile_verification_to_vm {
...
fn get_encoded_account(
    ...
    match account_resolver::get_account_from_overwrites_or_bank(pubkey, bank, overwrite_accounts) {
        Some(account) => {
            let response = if is_known_ocp_token_id(account.owner())
                && encoding == UiAccountEncoding::JsonParsed
            {
...
/// Analyze custom filters to determine if the result will be a subset of ocp-token accounts by
/// owner.
...
fn get_ocp_token_owner_filter(program_id: &Pubkey, filters: &[RpcFilterType]) -> Option<Pubkey> {
    if !is_known_ocp_token_id(program_id) {
        return None;
    }
        ...
        match filter {
            ...
            RpcFilterType::Memcmp(memcmp) => {
                ...
                if let Some(bytes) = memcmp.raw_bytes_as_ref() {
                    ...
                    } else if offset == OCP_TOKEN_ACCOUNT_OWNER_OFFSET {
        ...
        || token_account_state_filter
    {
        if let Some(incorrect_owner_len) = incorrect_owner_len {
            info!(
                "Incorrect num bytes ({:?}) provided for ocp_token_owner_filter",
                incorrect_owner_len
            );
        }
        owner_key
    } else {
        debug!("ocp_token program filters do not match by-owner index requisites");
        None
    }
}
...
/// NOTE: `optimize_filters()` should almost always be called before using this method because of
/// the requirement that `Memcmp::raw_bytes_as_ref().is_some()`.
fn get_ocp_token_mint_filter(program_id: &Pubkey, filters: &[RpcFilterType]) -> Option<Pubkey> {
    if !is_known_ocp_token_id(program_id) {
        return None;
    }
    let mut data_size_filter: Option<u64> = None;
        ...
        match filter {
            ...
            RpcFilterType::Memcmp(memcmp) => {
                ...
                if let Some(bytes) = memcmp.raw_bytes_as_ref() {
                    if offset == account_packed_len && *program_id == token_2022::id() {
                        memcmp_filter = Some(bytes);
                    } else if offset == OCP_TOKEN_ACCOUNT_MINT_OFFSET {
                        if bytes.len() == PUBKEY_BYTES {
        ...
        || token_account_state_filter
    {
        if let Some(incorrect_mint_len) = incorrect_mint_len {
            info!(
                "Incorrect num bytes ({:?}) provided for ocp_token_mint_filter",
                incorrect_mint_len
            );
        }
        mint
    } else {
        debug!("ocp_token program filters do not match by-mint index requisites");
        None
    }
}
...
pub mod rpc_minimal {
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_version(&self, _: Self::Metadata) -> Result<RpcVersionInfo> {
            ...
            let version = bovey_version::Version::default();
            Ok(RpcVersionInfo {
                bovey_core: version.to_string(),
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        bovey_ledger::leader_schedule_utils::leader_schedule_by_identity(
    ...
    impl BankData for BankDataImpl {
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == bovey_sdk::slot_history::Check::Found {
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        // OCP Token-specific RPC endpoints for
        // program details
...
pub mod rpc_accounts_scan {
    ...
    pub trait AccountsScan {
        ...
        // OCP Token-specific RPC endpoints
        // program details
...
pub mod rpc_full {
    use {
        super::*,
        bovey_sdk::message::{SanitizedVersionedMessage, VersionedMessage},
        bovey_transaction_status::parse_ui_inner_instructions,
    };
    #[rpc]
    pub trait Full {
        type Metadata;
...
pub fn create_test_transaction_entries(
    ...
    let success_tx = bovey_sdk::system_transaction::transfer(
        mint_keypair,
        &keypair1.pubkey(),
        rent_exempt_amount,
        blockhash,
    ...
    let entry_1 = bovey_entry::entry::next_entry(&blockhash, 1, vec![success_tx]);
    // Failed transaction, InstructionError
    let ix_error_tx = bovey_sdk::system_transaction::transfer(
        keypair2,
        &keypair3.pubkey(),
    ...
    let entry_2 = bovey_entry::entry::next_entry(&entry_1.hash, 1, vec![ix_error_tx]);
...
pub fn populate_blockstore_for_tests(
    ...
    let shreds = bovey_ledger::blockstore::entries_to_test_shreds(
        &entries,
        slot,
        parent_slot,
        true,
    ...
    assert_eq!(
        bovey_ledger::blockstore_processor::process_entries_for_tests(
            &BankWithScheduler::new_without_scheduler(bank),
            entries,
            Some(
                &bovey_ledger::blockstore_processor::TransactionStatusSender {
...
pub mod tests {
    use {
        ...
        bovey_accounts_db::accounts_db::{AccountsDbConfig, ACCOUNTS_DB_CONFIG_FOR_TESTING},
        bovey_entry::entry::next_versioned_entry,
        bovey_gossip::{contact_info::ContactInfo, socketaddr},
        bovey_ledger::{
        ...
        bovey_send_transaction_service::tpu_info::NullTpuInfo,
        bovey_transaction_status::{
        ...
        ocp_pod::optional_keys::OptionalNonZeroPubkey,
        ocp_token_2022::{
            extension::{
            ...
            bovey_program::{program_option::COption, pubkey::Pubkey as OcpTokenPubkey},
    ...
    impl RpcHandler {
        ...
        fn store_vote_account(&self, vote_pubkey: &Pubkey, vote_state: VoteState) {
            ...
            let mut vote_account =
                AccountSharedData::new(balance, space, &ocp_vote_program::id());
    ...
    fn test_rpc_request_processor_new() {
        let bob_pubkey = bovey_pubkey::new_rand();
    ...
    fn test_rpc_simulate_transaction_with_parsing_token_accounts() {
        ...
        let mint_rent_exempt_amount =
            bank.get_minimum_balance_for_rent_exemption(ocp_token::state::Mint::LEN);
        let mint_pubkey = Pubkey::from_str("mint111111111111111111111111111111111111111").unwrap();
        let mut mint_data = [0u8; ocp_token::state::Mint::LEN];
        Pack::pack_into_slice(
            &ocp_token::state::Mint {
        ...
        let token_account_rent_exempt_amount =
            bank.get_minimum_balance_for_rent_exemption(ocp_token::state::Account::LEN);
        let token_account_pubkey = Pubkey::new_unique();
        let owner_pubkey = Pubkey::from_str("owner11111111111111111111111111111111111111").unwrap();
        let mut token_account_data = [0u8; ocp_token::state::Account::LEN];
        Pack::pack_into_slice(
            &ocp_token::state::Account {
                mint: mint_pubkey,
                owner: owner_pubkey,
                amount: 1,
                delegate: COption::None,
                state: ocp_token::state::AccountState::Initialized,
                    ...
                    "accounts": [
                        null,
                        {
                            "data": {
                              ...
                              "owner": bs58::encode(ocp_token::id()).into_string(),
                              "rentEpoch": u64::MAX,
                              "space": ocp_token::state::Account::LEN
                        },
                    ],
                    "err": null,
                    "innerInstructions": null,
    ...
    fn test_rpc_simulate_transaction_with_inner_instructions() {
        ...
        let lookup_table_space = bovey_sdk::address_lookup_table::state::LOOKUP_TABLE_META_SIZE;
        ...
        let (instruction, lookup_table_address) =
            bovey_sdk::address_lookup_table::instruction::create_lookup_table(
    ...
    fn test_rpc_verify_pubkey() {
        let pubkey = bovey_pubkey::new_rand();
    ...
    fn test_get_vote_accounts() {
        ...
        let credits_per_slot =
            ocp_vote_program::vote_state::VOTE_CREDITS_MAXIMUM_PER_SLOT as u64;
    ...
    #[test]
    fn test_token_rpcs() {
        for program_id in bovey_account_decoder::parse_token::ocp_token_ids() {
            let rpc = RpcHandler::start();
            let bank = rpc.working_bank();
            let RpcHandler { io, meta, .. } = rpc;
            let mint = OcpTokenPubkey::new_from_array([2; 32]);
            let owner = OcpTokenPubkey::new_from_array([3; 32]);
            let delegate = OcpTokenPubkey::new_from_array([4; 32]);
            let token_account_pubkey = bovey_pubkey::new_rand();
            let token_with_different_mint_pubkey = bovey_pubkey::new_rand();
            let new_mint = OcpTokenPubkey::new_from_array([5; 32]);
            if program_id == bovey_inline_ocp::token_2022::id() {
                ...
                let other_token_account_pubkey = bovey_pubkey::new_rand();
            ...
            let accounts: Vec<RpcKeyedAccount> =
                serde_json::from_value(result["result"].clone()).unwrap();
            if program_id == bovey_inline_ocp::token::id() {
            ...
            let token_with_smaller_balance = bovey_pubkey::new_rand();
    ...
    #[test_case(ocp_token::id(), None, None; "ocp_token")]
    #[test_case(ocp_token_2022::id(), Some(InterestBearingConfig { pre_update_average_rate: 500.into(), current_rate: 500.into(),..Default::default() }), None; "ocp_token_2022_with _interest")]
    #[test_case(ocp_token_2022::id(), None, Some(ScaledUiAmountConfig { new_multiplier: 2.0f64.into(), ..Default::default() }); "ocp-token-2022 with multiplier")]
    fn test_token_parsing(
        ...
        let (program_name, account_size, mint_size, additional_data) = if program_id
            == bovey_inline_ocp::token_2022::id()
        {
            let account_base = TokenAccount {
                mint,
                owner,
            ...
            let additional_data = OcpTokenAdditionalDataV2 {
                decimals,
            ...
            bank.store_account(&Pubkey::from_str(&mint.to_string()).unwrap(), &mint_account);
            ("ocp-token-2022", account_size, mint_size, additional_data)
        } else {
            ...
            bank.store_account(&Pubkey::from_str(&mint.to_string()).unwrap(), &mint_account);
            let additional_data = OcpTokenAdditionalDataV2::with_decimals(decimals);
            ("ocp-token", account_size, mint_size, additional_data)
        };
    ...
    #[test]
    fn test_get_ocp_token_owner_filter() {
        // Filtering on token-v3 length
        let owner = Pubkey::new_unique();
        assert_eq!(
            get_ocp_token_owner_filter(
                &ocp_token::id(),
                &[
        ...
        assert!(get_ocp_token_owner_filter(
            &bovey_inline_ocp::token::id(),
            &[
                RpcFilterType::Memcmp(Memcmp::new_raw_bytes(32, owner.to_bytes().to_vec())),
                RpcFilterType::Memcmp(Memcmp::new_raw_bytes(165, vec![ACCOUNTTYPE_ACCOUNT])),
            ],
    ...
    #[test]
    fn test_get_ocp_token_mint_filter() {
        // Filtering on token-v3 length
        let mint = Pubkey::new_unique();
        assert_eq!(
            get_ocp_token_mint_filter(
                &bovey_inline_ocp::token::id(),
                &[
        ...
        // Can't filter on account type for token-v3
        assert!(get_ocp_token_mint_filter(
            &bovey_inline_ocp::token::id(),
            &[
                RpcFilterType::Memcmp(Memcmp::new_raw_bytes(0, mint.to_bytes().to_vec())),
                RpcFilterType::Memcmp(Memcmp::new_raw_bytes(165, vec![ACCOUNTTYPE_ACCOUNT])),
    ...
    fn test_decode_and_deserialize_too_large_payloads_fail() {
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base58 encoded bovey_transaction::Transaction too large: {tx58_len} bytes (max: encoded/raw {MAX_BASE58_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64, TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base64 encoded bovey_transaction::Transaction too large: {tx64_len} bytes (max: encoded/raw {MAX_BASE64_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "decoded bovey_transaction::Transaction too large: {too_big} bytes (max: {PACKET_DATA_SIZE} bytes)"
            ))
        );
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64.clone(), TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(
                "failed to deserialize bovey_transaction::Transaction: invalid value: \
