...
use {
    crate::{
        rpc_pubsub::{RpcBovPubSubImpl, RpcBovPubSubInternal},
...
impl PubSubService {
    pub fn new(
        ...
        let thread_hdl = Builder::new()
            .name("bovRpcPubSub".to_string())
            .spawn(move || {
                info!("PubSubService has started");
                let runtime = tokio::runtime::Builder::new_multi_thread()
                    .thread_name("bovRpcPubSubRt")
...
pub fn test_connection(
    subscriptions: &Arc<RpcSubscriptions>,
) -> (RpcBovPubSubImpl, TestBroadcastReceiver) {
    let current_subscriptions = Arc::new(DashMap::new());

    let rpc_impl = RpcBovPubSubImpl::new(
        PubSubConfig {
...
mod tests {
    ...
    fn test_pubsub_new() {
        ...
        assert_eq!(thread.name().unwrap(), "bovRpcPubSub");
    }
}
