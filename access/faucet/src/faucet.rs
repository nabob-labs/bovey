//! The `faucet` module provides an object for launching a Bovey Faucet,
//! which is the custodian of any remaining lamports in a mint.
//! The Bovey Faucet builds and sends airdrop transactions,
...
impl Faucet {
    ...
    pub fn new_with_allowed_ips(
        ...
        if let Some((per_request_cap, per_time_cap)) = per_request_cap.zip(per_time_cap) {
            if per_time_cap < per_request_cap {
                warn!(
                    "per_time_cap {} BOV < per_request_cap {} BOV; \
                    maximum single requests will fail",
                    lamports_to_bov(per_time_cap),
                    lamports_to_bov(per_request_cap),
                );
            }
        }
        Self {
        ...
        if let Some(cap) = self.per_time_cap {
            if new_total > cap {
                return Err(FaucetError::PerTimeCapExceeded(
                    lamports_to_bov(request_amount),
                    to.to_string(),
                    lamports_to_bov(new_total),
                    lamports_to_bov(cap),
                ));
            }
        }
    ...
    /// the request exceeds this per-request limit, this method returns a signed OCP Memo
    /// transaction with the memo: `"request too large; req: <REQUEST> BOV cap: <CAP> BOV"`
    pub fn build_airdrop_transaction(
        ...
        match req {
            FaucetRequest::GetAirdrop {
                ...
                info!(
                    "Requesting airdrop of {} BOV to {:?}",
                    lamports_to_bov(lamports),
                    to
                );
                ...
                if let Some(cap) = self.per_request_cap {
                    if lamports > cap {
                        ...
                        let memo_instruction = Instruction {
                            program_id: Pubkey::from(ocp_memo::id().to_bytes()),
                            accounts: vec![],
...
#[cfg(test)]
mod tests {
    use {super::*, bovey_system_interface::instruction::SystemInstruction, std::time::Duration};
    ...
    fn test_faucet_build_airdrop_transaction() {
            ...
            faucet.build_airdrop_transaction(request, ip).unwrap()
        {
            ...
            assert_eq!(
                message.account_keys,
                vec![mint_pubkey, Pubkey::from(ocp_memo::id().to_bytes())]
            );
            assert_eq!(message.recent_blockhash, blockhash);
