use {
    ...
    log::*,
    bovey_clap_utils::input_parsers::{lamports_of_bov, value_of},
    bovey_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    bovey_keypair::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = bovey_cli_config::Config::default().keypair_path;

    bovey_logger::setup_with_default_filter();
    bovey_metrics::set_panic_hook("faucet", /*version:*/ None);
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("per_time_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for time slice, in BOV"),
        )
        .arg(
            Arg::with_name("per_request_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for a single request, in BOV"),
        )
        .arg(
    ...
    let per_time_cap = lamports_of_bov(&matches, "per_time_cap");
    let per_request_cap = lamports_of_bov(&matches, "per_request_cap");
