...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "bovey", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The Bovey CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq, Eq)]
pub struct Config {
    /// The RPC address of a Bovey validator node.
    ///
    /// Typical values for mainnet, devnet, and testnet are [described in the
    /// Bovey documentation][rpcdocs].
    ///
    ...
    /// documentation for `bovey_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `bovey_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from Bovey addresses to human-readable names.
    ///
    ...
    /// `bovey_commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "bovey", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet-beta.bovey.com".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.bovey.com"),
            "ws://api.devnet.bovey.com/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.bovey.com"),
            "wss://api.devnet.bovey.com/".to_string()
        );
