...
use {
    ...
    bovey_rayon_threadlimit::get_max_thread_count,
...
fn init(name: &OsStr) {
    ...
    INIT_HOOK.call_once(|| {
        let path;
        let lib_name = if let Some(perf_libs_path) = bovey_perf::perf_libs::locate_perf_libs() {
            bovey_perf::perf_libs::append_to_ld_library_path(
...
/// Work consensus!)
///
/// The bovey core protocol currently requires an `Entry` to contain `transactions` that are
/// executable in parallel. Implemented in:
///
/// * For TPU: `bovey_core::banking_stage::BankingStage::process_and_record_transactions()`
/// * For TVU: `bovey_core::replay_stage::ReplayStage::replay_blockstore_into_bank()`
///
...
///
/// This enforcement is done via a call to `bovey_runtime::accounts::Accounts::lock_accounts()`
...
pub fn thread_pool_for_tests() -> ThreadPool {
    ...
    rayon::ThreadPoolBuilder::new()
        .num_threads(4)
        .thread_name(|i| format!("bovEntryTest{i:02}"))
        .build()
...
pub fn thread_pool_for_benches() -> ThreadPool {
    rayon::ThreadPoolBuilder::new()
        ...
        .thread_name(|i| format!("bovEntryBnch{i:02}"))
        .build()
        .expect("new rayon threadpool")
}
...
mod tests {
    use {
        ...
        bovey_perf::test_tx::{test_invalid_tx, test_tx},
        bovey_pubkey::Pubkey,
