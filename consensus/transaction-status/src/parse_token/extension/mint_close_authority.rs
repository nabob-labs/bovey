use {
    super::*,
    ocp_token_2022::bovey_program::{program_option::COption, pubkey::Pubkey},
};
...
mod test {
    use {
        ...
        serde_json::Value,
        bovey_pubkey::Pubkey,
        ocp_token_2022::{instruction::*, bovey_program::message::Message},
    };
    ...
    fn test_parse_initialize_mint_close_authority_instruction() {
        ...
        let mint_close_authority_ix =
            initialize_mint_close_authority(&ocp_token_2022::id(), &mint_pubkey, None).unwrap();
        let message = Message::new(&[mint_close_authority_ix], None);
        let compiled_instruction = &message.instructions[0];
        assert_eq!(
            parse_token(
