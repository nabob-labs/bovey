use {
    ...
    bincode::deserialize,
    serde_json::json,
    bovey_loader_v2_interface::LoaderInstruction,
    bovey_message::{compiled_instruction::CompiledInstruction, AccountKeys},
    bovey_program::loader_upgradeable_instruction::UpgradeableLoaderInstruction,
};
...
mod test {
    ...
    fn test_parse_bpf_loader_instructions() {
        ...
        #[allow(deprecated)]
        let instruction =
            bovey_loader_v2_interface::write(&account_pubkey, &program_id, offset, bytes.clone());
        ...
        #[allow(deprecated)]
        let instruction = bovey_loader_v2_interface::finalize(&account_pubkey, &program_id);
        let mut message = Message::new(&[instruction], Some(&fee_payer));
        assert_eq!(
            parse_bpf_loader(
                &message.instructions[0],
