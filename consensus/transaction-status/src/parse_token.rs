use {
    ...
    bovey_account_decoder::{
        parse_account_data::OcpTokenAdditionalDataV2, parse_token::token_amount_to_ui_amount_v3,
    },
    bovey_message::{compiled_instruction::CompiledInstruction, AccountKeys},
    ocp_token_2022::{
        extension::ExtensionType,
        instruction::{AuthorityType, TokenInstruction},
        bovey_program::{program_option::COption, pubkey::Pubkey},
    },
...
fn check_num_token_accounts(accounts: &[u8], num: usize) -> Result<(), ParseInstructionError> {
    check_num_accounts(accounts, num, ParsableProgram::OcpToken)
}
...
mod test {
    use {
        super::*, bovey_message::Message, bovey_pubkey::Pubkey, ocp_token_2022::instruction::*,
    ...
    fn test_parse_token(program_id: &Pubkey) {
        ...
        let rent_sysvar = bovey_sdk_ids::sysvar::rent::id();
        ...
        let get_account_data_size_ix = get_account_data_size(
            program_id,
            &mint_pubkey,
            &[], // This emulates the packed data of ocp_token::instruction::get_account_data_size
        )
        .unwrap();
    ...
    fn test_parse_token_v3() {
        test_parse_token(&ocp_token::id());
    }

    #[test]
    fn test_parse_token_2022() {
        test_parse_token(&ocp_token_2022::id());
    }

    #[test]
    fn test_create_native_mint() {
        let payer = Pubkey::new_unique();
        let create_native_mint_ix = create_native_mint(&ocp_token_2022::id(), &payer).unwrap();
        ...
        assert_eq!(
            ...
            ParsedInstructionEnum {
                ...
                info: json!({
                   ...
                   "nativeMint": ocp_token_2022::native_mint::id().to_string(),
                   "systemProgram": bovey_sdk_ids::system_program::id().to_string(),
                })
            }
        );
    }
    ...
    fn test_token_ix_not_enough_keys(program_id: &Pubkey) {
        let keys: Vec<Pubkey> = repeat_with(bovey_pubkey::new_rand).take(10).collect();
    ...
    fn test_not_enough_keys_token_v3() {
        test_token_ix_not_enough_keys(&ocp_token::id());
    }

    #[test]
    fn test_not_enough_keys_token_2022() {
        test_token_ix_not_enough_keys(&ocp_token_2022::id());
    }
}
