pub use bovey_account_decoder_client_types::ParsedAccount;
use {
    crate::{
...
lazy_static! {
    ...
    pub static ref PARSABLE_PROGRAM_IDS: HashMap<Pubkey, ParsableAccount> = {
        ...
        m.insert(ocp_token::id(), ParsableAccount::OcpToken);
        m.insert(ocp_token_2022::id(), ParsableAccount::OcpToken2022);
...
pub struct AccountAdditionalData {
    pub ocp_token_decimals: Option<u8>,
}

#[deprecated(since = "2.2.0", note = "Use `AccountAdditionalDataV3` instead")]
#[derive(Clone, Copy, Default)]
pub struct AccountAdditionalDataV2 {
    pub ocp_token_additional_data: Option<OcpTokenAdditionalData>,
}

#[derive(Clone, Copy, Default)]
pub struct AccountAdditionalDataV3 {
    pub ocp_token_additional_data: Option<OcpTokenAdditionalDataV2>,
}
...
impl From<AccountAdditionalDataV2> for AccountAdditionalDataV3 {
    fn from(v: AccountAdditionalDataV2) -> Self {
        Self {
            ocp_token_additional_data: v.ocp_token_additional_data.map(Into::into),
        }
    }
}

#[derive(Clone, Copy, Default)]
pub struct OcpTokenAdditionalData {
    pub decimals: u8,
    pub interest_bearing_config: Option<(InterestBearingConfig, UnixTimestamp)>,
}

impl OcpTokenAdditionalData {
...
#[derive(Clone, Copy, Default)]
pub struct OcpTokenAdditionalDataV2 {
    pub decimals: u8,
    pub interest_bearing_config: Option<(InterestBearingConfig, UnixTimestamp)>,
    pub scaled_ui_amount_config: Option<(ScaledUiAmountConfig, UnixTimestamp)>,
}

impl From<OcpTokenAdditionalData> for OcpTokenAdditionalDataV2 {
    fn from(v: OcpTokenAdditionalData) -> Self {
        Self {
            decimals: v.decimals,
...
pub fn parse_account_data(
    ...
    parse_account_data_v3(
        ...
        additional_data.map(|d| AccountAdditionalDataV3 {
            ocp_token_additional_data: d
                .ocp_token_decimals
                .map(OcpTokenAdditionalDataV2::with_decimals),
        }),
    )
}
...
pub fn parse_account_data_v3(
    ...
    let parsed_json = match program_name {
        ...
        ParsableAccount::OcpToken | ParsableAccount::OcpToken2022 => serde_json::to_value(
            parse_token_v3(data, additional_data.ocp_token_additional_data.as_ref())?,
        )?,
...
mod test {
    ...
    fn test_parse_account_data() {
        ...
        let other_program = bovey_pubkey::new_rand();
        let data = vec![0; 4];
