use {
    ...
    bovey_cost_model::cost_model::CostModel,
    bovey_entry::entry::{
        self, create_ticks, Entry, EntrySlice, EntryType, EntryVerificationStatus, VerifyRecyclers,
    },
    bovey_measure::{measure::Measure, measure_us},
    bovey_metrics::datapoint_error,
    bovey_rayon_threadlimit::{get_max_thread_count, get_thread_count},
    bovey_runtime::{
    ...
    bovey_timings::{report_execute_timings, ExecuteTimingType, ExecuteTimings},
    bovey_transaction_status::token_balances::TransactionTokenBalancesSet,
    bovey_vote::vote_account::VoteAccountsHashMap,
    std::{
        borrow::Cow,
...
use {qualifier_attr::qualifiers, bovey_runtime::bank::HashOverrides};

pub struct TransactionBatchWithIndexes<'a, 'b, Tx: VMMessage> {
...
pub struct LockedTransactionsWithIndexes<Tx: VMMessage> {
...
fn do_get_first_error<T, Tx: VMTransaction>(
...
fn get_first_error<T, Tx: VMTransaction>(
    batch: &TransactionBatch<Tx>,
...
fn create_thread_pool(num_threads: usize) -> ThreadPool {
    rayon::ThreadPoolBuilder::new()
        ...
        .thread_name(|i| format!("bovReplayTx{i:02}"))
        .build()
    ...
    let pre_commit_callback = |timings: &mut _, processing_results: &_| -> PreCommitResult {
        match extra_pre_commit_callback {
            ...
            Some(extra_pre_commit_callback) => {
                ...
                // bovey_core::banking_stage::Consumer::execute_and_commit_transactions_locked()
                Ok(Some(freeze_lock))
            }
        }
    };
...
fn check_block_cost_limits_if_enabled(
    ...
    let (check_block_cost_limits_result, check_block_cost_limits_us) = measure_us!(if bank
        .feature_set
        .is_active(&bovey_feature_set::apply_cost_tracker_during_replay::id())
    {
...
pub mod tests {
    use {
        ...
        rand::{thread_rng, Rng},
        bovey_cost_model::transaction_cost::TransactionCost,
        bovey_entry::entry::{create_ticks, next_entry, next_entry_mut},
        bovey_program_runtime::declare_process_instruction,
        bovey_runtime::{
        ...
        bovey_vote::{vote_account::VoteAccount, vote_transaction},
        ocp_vote_program::{
            self,
            ...
            roots_stakes
                ...
                .map(|(root, stake)| {
                    ...
                    let mut vote_account =
                        AccountSharedData::new(1, VoteState::size_of(), &ocp_vote_program::id());
    ...
    fn test_confirm_slot_entries_progress_num_txs_indexes() {
        let GenesisConfigInfo {
            ...
            mint_keypair,
            ..
        } = create_genesis_config(100 * LAMPORTS_PER_BOV);
        let genesis_hash = genesis_config.hash();
        let (bank, _bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
        ...
        let keypair3 = Keypair::new();
        let keypair4 = Keypair::new();
        bank.transfer(LAMPORTS_PER_BOV, &mint_keypair, &keypair1.pubkey())
            .unwrap();
        bank.transfer(LAMPORTS_PER_BOV, &mint_keypair, &keypair2.pubkey())
            .unwrap();
    ...
    fn test_execute_batch_pre_commit_callback(
        ...
        bovey_logger::setup();
        let dummy_leader_pubkey = bovey_sdk::pubkey::new_rand();
        let GenesisConfigInfo {
            genesis_config,
            mint_keypair,
            ..
        ...
        let bank = Arc::new(bank);
        let pubkey = bovey_sdk::pubkey::new_rand();
