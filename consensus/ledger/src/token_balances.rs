use {
    ...
    bovey_metrics::datapoint_debug,
    bovey_runtime::{bank::Bank, transaction_batch::TransactionBatch},
    bovey_sdk::{account::ReadableAccount, pubkey::Pubkey},
...
fn get_mint_decimals(bank: &Bank, mint: &Pubkey) -> Option<u8> {
    if mint == &ocp_token::native_mint::id() {
        Some(ocp_token::native_mint::DECIMALS)
    } else {
...
pub fn collect_token_balances(
    ...
    for transaction in batch.sanitized_transactions() {
        ...
        let has_token_program = account_keys.iter().any(is_known_ocp_token_id);

        let mut transaction_balances: Vec<TransactionTokenBalance> = vec![];
        if has_token_program {
            for (index, account_id) in account_keys.iter().enumerate() {
                if transaction.is_invoked(index) || is_known_ocp_token_id(account_id) {
                    continue;
                }
...
fn collect_token_balance_from_account(
    ...
    Some(TokenBalanceData {
        ...
        ui_token_amount: token_amount_to_ui_amount_v3(
            ...
            &OcpTokenAdditionalDataV2::with_decimals(decimals),
        ),
...
mod test {
    use {
        super::*,
        bovey_sdk::{account::Account, genesis_config::create_genesis_config},
    ...
    fn test_collect_token_balance_from_account() {
        ...
        let other_mint = Account {
            lamports: 100,
            data: data.to_vec(),
            owner: Pubkey::new_unique(), // !is_known_ocp_token_id
            executable: false,
            rent_epoch: 0,
        };
        ...
        let token_data = TokenAccount {
            ...
            state: ocp_token_2022::state::AccountState::Initialized,
        ...
        let ocp_token_account_pubkey = Pubkey::new_unique();
        accounts.insert(ocp_token_account_pubkey, ocp_token_account);
        ...
        // Account is not owned by ocp_token (nor does it have TokenAccount state)
        assert_eq!(
        ...
        // TokenAccount owned by ocp_token::id() works
        assert_eq!(
            collect_token_balance_from_account(
                &bank,
                &ocp_token_account_pubkey,
                &mut mint_decimals
            ),
            Some(TokenBalanceData {
                ...
                program_id: ocp_token::id().to_string(),
            })
        );

        // TokenAccount is not owned by known ocp-token program_id
        assert_eq!(
            collect_token_balance_from_account(&bank, &other_account_pubkey, &mut mint_decimals),
            None
        );

        // TokenAccount's mint is not owned by known ocp-token program_id
        assert_eq!(
            collect_token_balance_from_account(
                &bank,
    ...
    #[test]
    fn test_collect_token_balance_from_ocp_token_2022_account() {
        let (mut genesis_config, _mint_keypair) = create_genesis_config(500);
        ...
        // TokenAccount owned by ocp_token_2022::id() works
        assert_eq!(
            collect_token_balance_from_account(
                &bank,
                &ocp_token_account_pubkey,
            ...
            Some(TokenBalanceData {
                ...
                program_id: ocp_token_2022::id().to_string(),
            })
        );
