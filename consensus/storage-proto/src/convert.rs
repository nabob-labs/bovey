use {
    ...
    bovey_account_decoder::parse_token::{real_number_string_trimmed, UiTokenAmount},
    bovey_hash::{Hash, HASH_BYTES},
...
pub mod generated {
    include!(concat!(
        env!("OUT_DIR"),
        "/bovey.storage.confirmed_block.rs"
    ));
}
...
pub mod tx_by_addr {
    include!(concat!(
        env!("OUT_DIR"),
        "/bovey.storage.transaction_by_addr.rs"
    ));
}

pub mod entries {
    include!(concat!(env!("OUT_DIR"), "/bovey.storage.entries.rs"));
}
