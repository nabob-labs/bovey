use {
    ...
    bovey_cluster_type::ClusterType,
...
pub fn lamports_of_bov(matches: &ArgMatches<'_>, name: &str) -> Option<u64> {
    value_of(matches, name).map(bov_to_lamports)
}
...
mod tests {
    ...
    #[test]
    fn test_lamports_of_bov() {
        let matches = app().get_matches_from(vec!["test", "--single", "50"]);
        assert_eq!(lamports_of_bov(&matches, "single"), Some(50_000_000_000));
        assert_eq!(lamports_of_bov(&matches, "multiple"), None);
        let matches = app().get_matches_from(vec!["test", "--single", "1.5"]);
        assert_eq!(lamports_of_bov(&matches, "single"), Some(1_500_000_000));
        assert_eq!(lamports_of_bov(&matches, "multiple"), None);
        let matches = app().get_matches_from(vec!["test", "--single", "0.03"]);
        assert_eq!(lamports_of_bov(&matches, "single"), Some(30_000_000));
    }
}
