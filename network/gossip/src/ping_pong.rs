...
const PING_PONG_HASH_PREFIX: &[u8] = "BOVEY_PING_PONG".as_bytes();
...
fn hash_ping_token<const N: usize>(token: &[u8; N]) -> Hash {
    bovey_sdk::hash::hashv(&[PING_PONG_HASH_PREFIX, token])
}
...
mod tests {
    ...
    fn test_ping_pong() {
        ...
        assert_eq!(
            bovey_sdk::hash::hashv(&[PING_PONG_HASH_PREFIX, &ping.token]),
            pong.hash
        );
    }
