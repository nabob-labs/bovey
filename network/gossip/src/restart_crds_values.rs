use {
    ...
    bovey_serde_varint as serde_varint,
    thiserror::Error,
};
...
impl RestartHeaviestFork {
    pub(crate) fn new_rand<R: Rng>(rng: &mut R, from: Option<Pubkey>) -> Self {
        let from = from.unwrap_or_else(bovey_pubkey::new_rand);
        Self {
            from,
...
mod test {
    use {
        ...
        bovey_sdk::{signature::Signer, signer::keypair::Keypair, timing::timestamp},
