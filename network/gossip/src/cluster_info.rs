...
use {
    ...
    bovey_runtime::bank_forks::BankForks,
    bovey_sanitize::Sanitize,
    bovey_sdk::{
...
const MIN_STAKE_FOR_GOSSIP: u64 = bovey_sdk::native_token::LAMPORTS_PER_BOV;
...
impl ClusterInfo {
    ...
    pub fn get_node_version(&self, pubkey: &Pubkey) -> Option<bovey_version::Version> {
        let gossip_crds = self.gossip.crds.read().unwrap();
        gossip_crds
    ...
    pub fn gossip(
        ...
        let thread_pool = ThreadPoolBuilder::new()
            ...
            .thread_name(|i| format!("bovRunGossip{i:02}"))
            .build()
            .unwrap();
        let mut epoch_specs = bank_forks.map(EpochSpecs::from);
        Builder::new()
            .name("bovGossip".to_string())
            .spawn(move || {
                let mut last_push = 0;
    ...
    pub(crate) fn start_socket_consume_thread(
        ...
        let thread_pool = ThreadPoolBuilder::new()
            ...
            .thread_name(|i| format!("bovGossipCons{i:02}"))
            .build()
            .unwrap();
        let mut epoch_specs = bank_forks.map(EpochSpecs::from);
        ...
        let thread_name = String::from("bovGossipConsum");
    ...
    pub(crate) fn listen(
        ...
        let thread_pool = ThreadPoolBuilder::new()
            .num_threads(get_thread_count().min(8))
            .thread_name(|i| format!("bovGossipWork{i:02}"))
            .build()
            .unwrap();
        let mut epoch_specs = bank_forks.map(EpochSpecs::from);
        Builder::new()
            .name("bovGossipListen".to_string())
            .spawn(move || {
                while !exit.load(Ordering::Relaxed) {
...
mod tests {
    use {
        ...
        bincode::serialize,
        itertools::izip,
        bovey_ledger::shred::Shredder,
        bovey_net_utils::bind_to,
        bovey_sdk::signature::{Keypair, Signer},
        ocp_vote_program::{vote_instruction, vote_state::Vote},
        std::{
            iter::repeat_with,
            net::{IpAddr, Ipv4Addr},
            panic,
    ...
    fn test_gossip_node() {
        ...
        let (node, _, _) = ClusterInfo::spy_node(bovey_pubkey::new_rand(), 0);
        assert!(ClusterInfo::is_spy_node(
            &node,
            &SocketAddrSpace::Unspecified
        ));
        let (node, _, _) =
            ClusterInfo::gossip_node(bovey_pubkey::new_rand(), &"1.1.1.1:0".parse().unwrap(), 0);
        assert!(ClusterInfo::is_spy_node(
            &node,
            &SocketAddrSpace::Unspecified
        ));
    ...
    fn test_handle_pull() {
        ...
        let entrypoint_pubkey = bovey_pubkey::new_rand();
    ...
    fn test_cluster_spy_gossip() {
        ...
        let (spy, _, _) = ClusterInfo::spy_node(bovey_pubkey::new_rand(), 0);
        let cluster_info = Arc::new({
            let keypair = Arc::new(Keypair::new());
            let node = Node::new_localhost_with_pubkey(&keypair.pubkey());
            ClusterInfo::new(node.info, keypair, SocketAddrSpace::Unspecified)
    ...
    fn insert_info_test() {
        ...
        let d = ContactInfo::new_localhost(&bovey_pubkey::new_rand(), timestamp());
    ...
    fn new_with_external_ip_test_random() {
        ...
        let node = Node::new_with_external_ip(&bovey_pubkey::new_rand(), config);
    ...
    fn test_pull_from_entrypoint_if_not_present() {
        ...
        let other_node_pubkey = bovey_pubkey::new_rand();
    ...
    fn test_process_entrypoint_adopt_shred_version() {
        ...
        let mut gossiped_entrypoint1_info =
            ContactInfo::new_localhost(&bovey_pubkey::new_rand(), timestamp());
