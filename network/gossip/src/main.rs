...
use {
    ...
    bovey_gossip::{contact_info::ContactInfo, gossip_service::discover},
    bovey_pubkey::Pubkey,
...
fn parse_gossip_host(matches: &ArgMatches, entrypoint_addr: Option<SocketAddr>) -> IpAddr {
    matches
        ...
        .map(|gossip_host| {
            bovey_net_utils::parse_host(gossip_host).unwrap_or_else(|e| {
        ...
        .unwrap_or_else(|| {
            if let Some(entrypoint_addr) = entrypoint_addr {
                bovey_net_utils::get_public_ip_addr(&entrypoint_addr).unwrap_or_else(|err| {
...
fn get_entrypoint_shred_version(entrypoint: &Option<SocketAddr>) -> Option<u16> {
    ...
    match bovey_net_utils::get_cluster_shred_version(entrypoint) {
        Err(err) => {
...
fn parse_entrypoint(matches: &ArgMatches) -> Option<SocketAddr> {
    matches.value_of("entrypoint").map(|entrypoint| {
        bovey_net_utils::parse_host_port(entrypoint).unwrap_or_else(|e| {
...
fn get_gossip_address(matches: &ArgMatches, entrypoint_addr: Option<SocketAddr>) -> SocketAddr {
    ...
    SocketAddr::new(
        ...
        value_t!(matches, "gossip_port", u16).unwrap_or_else(|_| {
            bovey_net_utils::find_available_port_in_range(
