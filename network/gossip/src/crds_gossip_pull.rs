...
use {
    ...
    bovey_bloom::bloom::{Bloom, ConcurrentBloom},
    bovey_sdk::{
        hash::Hash,
...
impl bovey_sanitize::Sanitize for CrdsFilter {
    fn sanitize(&self) -> std::result::Result<(), bovey_sanitize::SanitizeError> {
    ...
    pub(crate) fn new_pull_request(
        ...
        let (weights, nodes): (Vec<u64>, Vec<ContactInfo>) =
            crds_gossip::dedup_gossip_addresses(nodes, stakes)
                .into_values()
                .map(|(stake, node)| {
                    let stake = stake.min(stake_cap) / LAMPORTS_PER_BOV;
...
pub(crate) mod tests {
    use {
        ...
        bovey_perf::test_tx::new_test_vote_tx,
        bovey_sdk::{
    ...
    fn test_new_pull_request() {
        ...
        let mut new = ContactInfo::new_localhost(&bovey_pubkey::new_rand(), now);
        ...
        let mut offline = ContactInfo::new_localhost(&bovey_pubkey::new_rand(), now);
    ...
    fn test_new_mark_creation_time() {
        ...
        let mut old = ContactInfo::new_localhost(&bovey_pubkey::new_rand(), 0);
        ...
        crds.insert(old.clone(), now, GossipRoute::LocalMessage)
            .unwrap();
        let mut new = ContactInfo::new_localhost(&bovey_pubkey::new_rand(), 0);
    ...
    fn test_generate_pull_responses() {
        ...
        node_crds
            ...
            .unwrap();
        let new = ContactInfo::new_localhost(&bovey_pubkey::new_rand(), now);
        ping_cache.mock_pong(*new.pubkey(), new.gossip().unwrap(), Instant::now());
        let new = CrdsValue::new_unsigned(CrdsData::from(new));
        node_crds
    ...
    fn test_process_pull_request_response() {
        ...
        node_crds
            ...
            .unwrap();
        let mut ping_cache = new_ping_cache();
        let new = ContactInfo::new_localhost(&bovey_pubkey::new_rand(), 1);
        ping_cache.mock_pong(*new.pubkey(), new.gossip().unwrap(), Instant::now());
        let new = CrdsValue::new_unsigned(CrdsData::from(new));
        node_crds.insert(new, 0, GossipRoute::LocalMessage).unwrap();

        let mut dest_crds = Crds::default();
        let new_id = bovey_pubkey::new_rand();
    ...
    fn test_process_pull_response() {
        ...
        let peer_pubkey = bovey_pubkey::new_rand();
        let peer_entry =
