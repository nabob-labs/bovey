use {
    ...
    bovey_sdk::{native_token::LAMPORTS_PER_BOV, pubkey::Pubkey},
...
fn get_stake_bucket(stake: Option<&u64>) -> usize {
    let stake = stake.copied().unwrap_or_default() / LAMPORTS_PER_BOV;
...
mod tests {
    ...
    fn test_get_stake_bucket() {
        ...
        for (k, bucket) in buckets.into_iter().enumerate() {
            let stake = (k as u64) * LAMPORTS_PER_BOV;
        ...
        for (stake, bucket) in [
            ...
            let stake = stake * LAMPORTS_PER_BOV;
            assert_eq!(get_stake_bucket(Some(&stake)), bucket);
        }
        assert_eq!(
    ...
    fn test_push_active_set() {
        ...
        const MAX_STAKE: u64 = (1 << 20) * LAMPORTS_PER_BOV;
