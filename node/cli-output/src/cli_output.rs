...
use {
    ...
    bovey_sysvar::stake_history::StakeHistoryEntry,
    bovey_transaction::{versioned::VersionedTransaction, Transaction},
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for account in &self.accounts {
            writeln!(
                ...
                "{:<44}  {}",
                account.address,
                &format!("{} BOV", lamports_to_bov(account.lamports))
            )?;
        }
        Ok(())
    }
...
impl fmt::Display for CliSupply {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln_name_value(f, "Total:", &format!("{} BOV", lamports_to_bov(self.total)))?;
        writeln_name_value(
            f,
            "Circulating:",
            &format!("{} BOV", lamports_to_bov(self.circulating)),
        )?;
        writeln_name_value(
            f,
            "Non-Circulating:",
            &format!("{} BOV", lamports_to_bov(self.non_circulating)),
        )?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if !self.encoded_confirmed_block.rewards.is_empty() {
            ...
            writeln!(
                ...
                "Total Rewards: {}◎{:<12.9}",
                sign,
                lamports_to_bov(total_rewards.unsigned_abs())
            )?;
        }
...
mod tests {
    use {
        super::*,
        clap::{App, Arg},
        bovey_keypair::keypair_from_seed,
        bovey_message::Message,
        bovey_pubkey::Pubkey,
        bovey_signature::Signature,
        bovey_signer::{null_signer::NullSigner, Signer, SignerError},
