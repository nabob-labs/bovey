use {
    ...
    bovey_clap_utils::input_parsers::{keypair_of, keypairs_of, pubkey_of, value_of, values_of},
    bovey_core::{
    ...
    bovey_perf::recycler::enable_recycler_warming,
    bovey_poh::poh_service,
    bovey_rpc::{
    ...
    bovey_send_transaction_service::send_transaction_service,
    bovey_streamer::{quic::QuicServerParams, socket::SocketAddrSpace},
...
pub fn execute(
    ...
    info!("{} {}", crate_name!(), bovey_version);
    info!("Starting validator with: {:#?}", std::env::args_os());

    let cuda = matches.is_present("cuda");
    if cuda {
        bovey_perf::perf_libs::init_cuda();
        enable_recycler_warming();
    }

    bovey_core::validator::report_target_features();
    ...
    let bind_address = bovey_net_utils::parse_host(matches.value_of("bind_address").unwrap())
        .expect("invalid bind_address");
    let rpc_bind_address = if matches.is_present("rpc_bind_address") {
        bovey_net_utils::parse_host(matches.value_of("rpc_bind_address").unwrap())
            .expect("invalid rpc_bind_address")
    } else if private_rpc {
        bovey_net_utils::parse_host("127.0.0.1").unwrap()
    } else {
        bind_address
    };
    ...
    let entrypoint_addrs = values_t!(matches, "entrypoint", String)
        ...
        .into_iter()
        .map(|entrypoint| {
            bovey_net_utils::parse_host_port(&entrypoint)
    ...
    let rpc_send_transaction_tpu_peers = matches
        ...
        .map(|values| {
            values
                .map(bovey_net_utils::parse_host_port)
    ...
    let mut validator_config = ValidatorConfig {
        ...
        rpc_config: JsonRpcConfig {
            ...
            faucet_addr: matches.value_of("rpc_faucet_addr").map(|address| {
                bovey_net_utils::parse_host_port(address).expect("failed to parse faucet address")
            }),
            full_api,
    ...
    let dynamic_port_range =
        bovey_net_utils::parse_port_range(matches.value_of("dynamic_port_range").unwrap())
    ...
    let public_rpc_addr = matches
        ...
        .map(|addr| {
            bovey_net_utils::parse_host_port(addr)
    ...
    if !matches.is_present("no_os_network_limits_test") {
        if SystemMonitorService::check_os_network_limits() {
            ...
            Err("OS network limit test failed. See \
                https://docs.boveylabs.com/operations/guides/validator-start#system-tuning"
                .to_string())?;
        }
    }
    ...
    let gossip_host: IpAddr = matches
        ...
        .map(|gossip_host| {
            bovey_net_utils::parse_host(gossip_host)
        ...
        .or_else(|| {
            if !entrypoint_addrs.is_empty() {
                ...
                order.into_iter().find_map(|i| {
                    ...
                    bovey_net_utils::get_public_ip_addr_with_binding(entrypoint_addr, bind_address)
                        .map_or_else(
                            |err| {
                                warn!(
    ...
    let gossip_port = value_t!(matches, "gossip_port", u16).or_else(|_| {
        bovey_net_utils::find_available_port_in_range(bind_address, (0, 1))
    ...
    let public_tpu_addr = matches
        ...
        .map(|public_tpu_addr| {
            bovey_net_utils::parse_host_port(public_tpu_addr)
    ...
    let public_tpu_forwards_addr = matches
        ...
        .map(|public_tpu_forwards_addr| {
            bovey_net_utils::parse_host_port(public_tpu_forwards_addr)
    ...
    bovey_metrics::set_panic_hook("validator", Some(String::from(bovey_version)));
    bovey_entry::entry::init_poh();
    ...
    let validator = match Validator::new(
        ...
        Err(err) => {
            if matches!(
                ...
                // https://github.com/bovey-foundation/bovey-improvement-documents/pull/46
...
fn get_cluster_shred_version(entrypoints: &[SocketAddr], bind_address: IpAddr) -> Option<u16> {
    ...
    for entrypoint in entrypoints {
        match bovey_net_utils::get_cluster_shred_version_with_binding(entrypoint, bind_address) {
...
fn process_account_indexes(matches: &ArgMatches) -> AccountSecondaryIndexes {
    let account_indexes: HashSet<AccountIndex> = matches
        ...
        .map(|value| match value {
            ...
            "ocp-token-mint" => AccountIndex::OcpTokenMint,
            "ocp-token-owner" => AccountIndex::OcpTokenOwner,
            _ => unreachable!(),
        })
        .collect();
