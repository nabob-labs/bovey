use {
    ...
    itertools::Itertools,
    log::*,
    bovey_accounts_db::accounts_index::{AccountIndex, AccountSecondaryIndexes},
    bovey_clap_utils::{
        input_parsers::{pubkey_of, pubkeys_of, value_of},
        input_validators::normalize_to_url_if_moniker,
    },
    bovey_core::consensus::tower_storage::FileTowerStorage,
    bovey_faucet::faucet::run_local_faucet_with_port,
    bovey_rpc::{
...
fn main() {
    ...
    let version = bovey_version::version!();
    ...
    bovey_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    info!("{} {}", crate_name!(), bovey_version::version!());
    ...
    let cli_config = if let Some(config_file) = matches.value_of("config_file") {
        bovey_cli_config::Config::load(config_file).unwrap_or_default()
    } else {
        bovey_cli_config::Config::default()
    };
    ...
    let gossip_host = matches.value_of("gossip_host").map(|gossip_host| {
        bovey_net_utils::parse_host(gossip_host).unwrap_or_else(|err| {
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        bovey_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        bovey_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
    ...
    let faucet_lamports = bov_to_lamports(value_of(&matches, "faucet_bov").unwrap());
    ...
    let faucet_per_time_cap = value_t!(matches, "faucet_per_time_bov_cap", f64)
        .ok()
        .map(bov_to_lamports);
    let faucet_per_request_cap = value_t!(matches, "faucet_per_request_bov_cap", f64)
        .ok()
        .map(bov_to_lamports);
    ...
    if TestValidatorGenesis::ledger_exists(&ledger_path) {
        ...
        println_name_value(
            "\nNotice!",
            "No wallet available. `bovey airdrop` localnet BOV after creating one\n",
        );
    }
