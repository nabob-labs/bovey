use {
    bovey_core::validator::{ValidatorConfig, ValidatorStartProgress},
    bovey_download_utils::{download_snapshot_archive, DownloadProgressRecord},
    bovey_genesis_utils::download_then_check_genesis_hash,
    bovey_gossip::{
...
fn verify_reachable_ports(
    ...
    if !bovey_net_utils::verify_all_reachable_udp(
    ...
    bovey_net_utils::verify_all_reachable_tcp(&cluster_entrypoint.gossip().unwrap(), tcp_listeners)
}
...
fn check_vote_account(
    ...
    let vote_state = ocp_vote_program::vote_state::from(&vote_account);
...
fn download_snapshot(
    ...
    *start_progress.write().unwrap() = ValidatorStartProgress::DownloadingSnapshot {
    ...
    let desired_snapshot_hash = (
        ...
        bovey_runtime::snapshot_hash::SnapshotHash(desired_snapshot_hash.1),
    );
