...
use {
    ...
    bovey_net_utils::PortRange,
    bovey_rpc::{rpc::JsonRpcConfig, rpc_pubsub_service::PubSubConfig},
    bovey_rpc_client::{nonblocking, rpc_client::RpcClient},
    bovey_rpc_client_api::request::MAX_MULTIPLE_ACCOUNTS,
    bovey_runtime::{
...
fn try_transform_program_data(
    ...
    if account.owner() == &bovey_sdk::bpf_loader_upgradeable::id() {
...
impl TestValidatorGenesis {
            ...
            .chunks(MAX_MULTIPLE_ACCOUNTS)
        {
            rpc_client
                ...
                .for_each(|(maybe_account, feature_id)| {
                    if maybe_account
                        .as_ref()
                        .and_then(bovey_sdk::feature::from_account)
    ...
    pub fn add_accounts_from_json_files(
        ...
        for account in accounts {
            let Some(account_path) = bovey_program_test::find_file(account.filename) else {
    ...
    pub fn add_account_with_file_data(
        ...
        self.add_account(
            ...
            AccountSharedData::from(Account {
                lamports,
                data: bovey_program_test::read_file(
                    bovey_program_test::find_file(filename).unwrap_or_else(|| {
...
impl TestValidator {
    ...
    fn initialize_ledger(
        ...
        let validator_identity_lamports = bov_to_lamports(500.);
        let validator_stake_lamports = bov_to_lamports(1_000_000.);
        let mint_lamports = bov_to_lamports(500_000_000.);
        ...
        for (address, account) in bovey_program_test::programs::ocp_programs(&config.rent) {
            accounts.entry(address).or_insert(account);
        }
        for (address, account) in
            bovey_program_test::programs::core_bpf_programs(&config.rent, |feature_id| {
        ...
        for upgradeable_program in &config.upgradeable_programs {
            let data = bovey_program_test::read_file(&upgradeable_program.program_path);
        ...
        let mut genesis_config = create_genesis_config_with_leader_ex_no_features(
            ...
            config.rent.clone(),
            bovey_sdk::genesis_config::ClusterType::Development,
    ...
    fn start(
        ...
        // Needed to avoid panics in `bovey-responder-gossip` in tests that create a number of
...
#[cfg(test)]
mod test {
    use {super::*, bovey_sdk::feature::Feature};
    ...
    async fn test_deactivate_features() {
        ...
        [
            bovey_sdk::feature_set::deprecate_rewards_sysvar::id(),
            bovey_sdk::feature_set::disable_fees_sysvar::id(),
        ]
            .into_iter()
            .for_each(|feature| {
    ...
    async fn test_override_feature_account() {
        let with_deactivate_flag = bovey_sdk::feature_set::deprecate_rewards_sysvar::id();
        let without_deactivate_flag = bovey_sdk::feature_set::disable_fees_sysvar::id();
        ...
        assert_eq!(feature_account.owner, bovey_sdk::feature::id());
    ...
    async fn test_core_bpf_programs() {
        let (test_validator, _payer) = TestValidatorGenesis::default()
            .deactivate_features(&[
                ...
                bovey_sdk::feature_set::migrate_config_program_to_core_bpf::id(),
            ])
            .start_async()
            .await;
        ...
        let fetched_programs = rpc_client
            .get_multiple_accounts(&[
                bovey_sdk_ids::address_lookup_table::id(),
                bovey_sdk_ids::config::id(),
                bovey_sdk_ids::feature::id(),
            ])
            .await
            .unwrap();
        ...
        assert_eq!(account.owner, bovey_sdk_ids::bpf_loader_upgradeable::id());
        assert!(account.executable);

        // Config is a builtin.
        let account = fetched_programs[1].as_ref().unwrap();
        assert_eq!(account.owner, bovey_sdk_ids::native_loader::id());
        assert!(account.executable);
