//! The bovey-program-test provides a BanksClient-based test framework SBF programs
...
use {
    ...
    log::*,
    bovey_accounts_db::epoch_accounts_hash::EpochAccountsHash,
    bovey_banks_client::start_client,
    bovey_banks_server::banks_server::start_local_server,
    ocp_bpf_loader_program::serialization::serialize_parameters,
    bovey_compute_budget::compute_budget::ComputeBudget,
    bovey_feature_set::FEATURE_NAMES,
    bovey_instruction::{error::InstructionError, Instruction},
    bovey_log_collector::ic_msg,
    ...
    ocp_vote_program::vote_state::{self, VoteState, VoteStateVersions},
    std::{
        cell::RefCell,
...
// Export types so test clients can limit their bovey_ crate dependencies
pub use {
    bovey_banks_client::{BanksClient, BanksClientError},
    bovey_banks_interface::BanksTransactionResultWithMetadata,
    bovey_program_runtime::invoke_context::InvokeContext,
    bovey_bpf::{
        error::EbpfError,
        vm::{get_runtime_environment_key, EbpfVm},
    },
    bovey_sdk::transaction_context::IndexOfAccount,
};
...
pub fn invoke_builtin_function(
    builtin_function: bovey_sdk::entrypoint::ProcessInstruction,
...
/// Converts a `bovey-program`-style entrypoint into the runtime's entrypoint style, for
...
impl bovey_sdk::program_stubs::SyscallStubs for SyscallStubs {
    ...
    fn bov_get_epoch_schedule_sysvar(&self, var_addr: *mut u8) -> u64 {
        get_sysvar(
    ...
    fn bov_get_epoch_rewards_sysvar(&self, var_addr: *mut u8) -> u64 {
        get_sysvar(
    ...
    #[allow(deprecated)]
    fn bov_get_fees_sysvar(&self, var_addr: *mut u8) -> u64 {
    ...
    fn bov_get_last_restart_slot(&self, var_addr: *mut u8) -> u64 {
        get_sysvar(
            get_invoke_context()
                .get_sysvar_cache()
...
impl Default for ProgramTest {
    ...
    ///
    fn default() -> Self {
        bovey_logger::setup_with_default(
            "bovey_bpf::vm=debug,\
             bovey_runtime::message_processor=debug,\
             bovey_runtime::system_instruction_processor=trace,\
             bovey_program_test=info",
        );
        let prefer_bpf =
...
impl ProgramTest {
    ...
    ///
    /// See https://github.com/anza-xyz/bovey/blob/c038908600b8a1b0080229dea015d7fc9939c418/runtime/src/bank.rs#L5109-L5126.
    ...
    fn setup_bank(
        ...
        let bootstrap_validator_stake_lamports =
            rent.minimum_balance(VoteState::size_of()) + bov_to_lamports(1_000_000.0);

        let mint_keypair = Keypair::new();
        let voting_keypair = Keypair::new();

        let mut genesis_config = create_genesis_config_with_leader_ex(
            bov_to_lamports(1_000_000.0),
            &mint_keypair.pubkey(),
        ...
        // Add commonly-used OCP programs as a convenience to the user
        for (program_id, account) in programs::ocp_programs(&rent).iter() {
    ...
    /// with BOV for sending transactions
...
impl ProgramTestContext {
        ...
        let warp_bank = if pre_warp_slot == working_slot {
            ...
            bank_forks
                .insert(Bank::warp_from_parent(
                    ...
                    bovey_accounts_db::accounts_db::CalcAccountsHashDataSource::IndexForTests,
                ))
    ...
    pub fn warp_forward_force_reward_interval_end(&mut self) -> Result<(), ProgramTestError> {
        ...
        bank_forks
            .set_root(
                pre_warp_slot,
                &bovey_runtime::accounts_background_service::AbsRequestSender::default(),
                Some(pre_warp_slot),
            )
            .unwrap();
