...
mod ocp_memo_1_0 {
    bovey_sdk::declare_id!("Memo1UhkJRfHyvLMcVucJwxXeuD728EqVDDwQDxFMNo");
}
mod ocp_memo_3_0 {
    bovey_sdk::declare_id!("MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr");
}

static OCP_PROGRAMS: &[(Pubkey, Pubkey, &[u8])] = &[
    (
        bovey_inline_ocp::token::ID,
        bovey_sdk_ids::bpf_loader::ID,
        include_bytes!("programs/ocp_token-3.5.0.so"),
    ),
    (
        bovey_inline_ocp::token_2022::ID,
        bovey_sdk_ids::bpf_loader_upgradeable::ID,
        include_bytes!("programs/ocp_token_2022-5.0.2.so"),
    ),
    (
        ocp_memo_1_0::ID,
        bovey_sdk_ids::bpf_loader::ID,
        include_bytes!("programs/ocp_memo-1.0.0.so"),
    ),
    (
        ocp_memo_3_0::ID,
        bovey_sdk_ids::bpf_loader::ID,
        include_bytes!("programs/ocp_memo-3.0.0.so"),
    ),
    (
        bovey_inline_ocp::associated_token_account::ID,
        bovey_sdk_ids::bpf_loader::ID,
        include_bytes!("programs/ocp_associated_token_account-1.1.1.so"),
    ),
];
...
pub fn ocp_programs(rent: &Rent) -> Vec<(Pubkey, AccountSharedData)> {
    OCP_PROGRAMS
        .iter()
        .flat_map(|(program_id, loader_id, elf)| {
            let mut accounts = vec![];
            if loader_id.eq(&bovey_sdk_ids::bpf_loader_upgradeable::ID) {
