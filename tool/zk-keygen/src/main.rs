...
fn main() -> Result<(), Box<dyn error::Error>> {
    let matches = app(bovey_version::version!())
        .try_get_matches()
    ...
    match subcommand {
        ("new", matches) => {
            ...
            let outfile = if matches.try_contains_id("outfile")? {
                ...
                path.extend([".config", "bovey", key_type.default_file_name()]);
...
mod tests {
    ...
    fn process_test_command(args: &[&str]) -> Result<(), Box<dyn error::Error>> {
        ...
        let app_matches = app(bovey_version).get_matches_from(args);
        do_main(&app_matches)
    }
    ...
    fn test_arguments() {
        ...
        app(bovey_version).debug_assert();
    }
    ...
    fn test_pubkey() {
        ...
        process_test_command(&["bovey-keygen", "pubkey", "elgamal", &keypair_path]).unwrap();
    }
}
