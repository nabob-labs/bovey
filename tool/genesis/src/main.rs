...
use {
    ...
    bovey_accounts_db::hardened_unpack::MAX_GENESIS_ARCHIVE_UNPACKED_SIZE,
    bovey_clap_utils::{
        input_parsers::{
    ...
    bovey_entry::poh::compute_hashes_per_tick,
    bovey_epoch_schedule::EpochSchedule,
    bovey_feature_gate_interface as feature,
    bovey_feature_set::FEATURE_NAMES,
    bovey_fee_calculator::FeeRateGovernor,
    bovey_genesis::{
        genesis_accounts::add_genesis_accounts, Base64Account, StakedValidatorAccountInfo,
        ValidatorAccountsFile,
    },
    bovey_genesis_config::{ClusterType, GenesisConfig},
    bovey_inflation::Inflation,
    bovey_keypair::{read_keypair_file, Keypair},
    bovey_ledger::{blockstore::create_new_ledger, blockstore_options::LedgerColumnOptions},
    bovey_loader_v3_interface::state::UpgradeableLoaderState,
    bovey_native_token::bov_to_lamports,
    bovey_poh_config::PohConfig,
    bovey_pubkey::Pubkey,
    bovey_rent::Rent,
    ...
    bovey_signer::Signer,
    bovey_stake_interface::state::StakeStateV2,
    ocp_stake_program::stake_state,
    ocp_vote_program::vote_state::{self, VoteState},
    std::{
        collections::HashMap,
        error,
        fs::File,
...
fn main() -> Result<(), Box<dyn error::Error>> {
    let default_faucet_pubkey = bovey_cli_config::Config::default().keypair_path;
    ...
    // vote account
    let default_bootstrap_validator_lamports = &bov_to_lamports(500.0)
        .max(VoteState::get_rent_exempt_reserve(&rent))
        .to_string();
    // stake account
    let default_bootstrap_validator_stake_lamports = &bov_to_lamports(0.5)
    ...
    ocp_stake_program::add_genesis_accounts(&mut genesis_config);
    bovey_runtime::genesis_utils::activate_all_features(&mut genesis_config);
    if !features_to_deactivate.is_empty() {
        bovey_runtime::genesis_utils::deactivate_features(
            &mut genesis_config,
...
mod tests {
    use {
        super::*,
        bovey_borsh::v1 as borsh1,
        bovey_genesis_config::GenesisConfig,
        bovey_stake_interface as stake,
    ...
    fn test_append_primordial_accounts_to_genesis() {
        ...
        genesis_accounts.insert(
            bovey_pubkey::new_rand().to_string(),
            Base64Account {
                owner: bovey_pubkey::new_rand().to_string(),
                balance: 2,
                executable: false,
    ...
    fn test_append_validator_accounts_to_genesis() {
        ...
        let validator_accounts = vec![
            StakedValidatorAccountInfo {
                identity_account: bovey_pubkey::new_rand().to_string(),
                vote_account: bovey_pubkey::new_rand().to_string(),
                stake_account: bovey_pubkey::new_rand().to_string(),
