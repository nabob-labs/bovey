...
//!    ```bash
//!    bovey-dos $COMMON --num-signatures 8
//!    ```
//!    1.2 With valid signatures
//!    ```bash
//!    bovey-dos $COMMON --valid-signatures --num-signatures 8
//!    ```
//! 2. With blockhash and payer:
//!    2.1 Single-instruction transaction
//!    ```bash
//!    bovey-dos $COMMON --valid-blockhash --transaction-type transfer --num-instructions 1
//!    ```
//!    2.2 Multi-instruction transaction
//!    ```bash
//!    bovey-dos $COMMON --valid-blockhash --transaction-type transfer --num-instructions 8
//!    ```
//!    2.3 Account-creation transaction
//!    ```bash
//!    bovey-dos $COMMON --valid-blockhash --transaction-type account-creation
//!    ```
//!
...
use {
    ...
    log::*,
    rand::{thread_rng, Rng},
    bovey_bench_tps::bench::generate_and_fund_keypairs,
    bovey_client::{connection_cache::ConnectionCache, tpu_client::TpuClientWrapper},
    bovey_connection_cache::client_connection::ClientConnection as TpuConnection,
    bovey_core::repair::serve_repair::{RepairProtocol, RepairRequestHeader, ServeRepair},
    bovey_dos::cli::*,
    bovey_gossip::{
    ...
    bovey_tpu_client::tpu_client::DEFAULT_TPU_CONNECTION_POOL_SIZE,
    std::{
        net::SocketAddr,
        process::exit,
        sync::Arc,
...
fn get_target(
    ...
    if nodes.is_empty() {
        // skip-gossip case
        target = Some((bovey_sdk::pubkey::new_rand(), entrypoint_addr));
    } else {
        info!("************ NODE ***********");
        for node in nodes {
            info!("{:?}", node);
...
fn main() {
    bovey_logger::setup_with_default_filter();
...
pub mod test {
    use {
        super::*,
        bovey_core::validator::ValidatorConfig,
        bovey_faucet::faucet::run_local_faucet,
        bovey_local_cluster::{
            cluster::Cluster,
            local_cluster::{ClusterConfig, LocalCluster},
            validator_configs::make_identical_validator_configs,
        },
        bovey_quic_client::{QuicConfig, QuicConnectionManager, QuicPool},
        bovey_rpc::rpc::JsonRpcConfig,
        bovey_sdk::timing::timestamp,
        bovey_tpu_client::tpu_client::TpuClient,
    };
