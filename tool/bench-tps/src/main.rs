...
use {
    ...
    bovey_streamer::streamer::StakedNodes,
    bovey_tps_client::TpsClient,
    bovey_tpu_client::tpu_client::{TpuClient, TpuClientConfig},
    std::{
        collections::HashMap,
        fs::File,
        io::prelude::*,
...
fn main() {
    ...
    bovey_metrics::set_panic_hook("bench-tps", /*version:*/ None);

    let matches = cli::build_args(bovey_version::version!()).get_matches();
    ...
    if let Some(instruction_padding_config) = instruction_padding_config {
        ...
        client
            ...
            .expect("Instruction padding program must be deployed to this cluster. Deploy the program using `bovey program deploy ./bench-tps/tests/fixtures/ocp_instruction_padding.so` and pass the resulting program id with `--instruction-padding-program-id`");
    }
