...
use {
    ...
    bovey_cli_config::{Config, CONFIG_FILE},
...
fn get_keypair_from_matches(
    ...
    let keypair_source = if matches.try_contains_id("keypair")? {
        ...
        path.extend([".config", "bovey", "id.json"]);
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    let matches = app(&default_num_threads, bovey_version::version!())
        .try_get_matches()
...
mod tests {
    ...
    fn process_test_command(args: &[&str]) -> Result<(), Box<dyn error::Error>> {
        let default_num_threads = num_cpus::get().to_string();
        let bovey_version = bovey_version::version!();
        let app_matches = app(&default_num_threads, bovey_version).get_matches_from(args);
        do_main(&app_matches)
    }
    ...
    fn test_arguments() {
        ...
        app(&default_num_threads, bovey_version).debug_assert();
    }
