use {
    ...
    bovey_clap_utils::{self, input_parsers::*, keypair::*},
    bovey_cli_config::ConfigInput,
    bovey_cli_output::{
    ...
    bovey_decode_error::DecodeError,
    bovey_hash::Hash,
    bovey_instruction::error::InstructionError,
    bovey_keypair::{read_keypair_file, Keypair},
    bovey_offchain_message::OffchainMessage,
    bovey_program::stake::{instruction::LockupArgs, state::Lockup},
    bovey_pubkey::Pubkey,
    bovey_remote_wallet::remote_wallet::RemoteWalletManager,
    ...
    bovey_rpc_client_nonce_utils::blockhash_query::BlockhashQuery,
    bovey_signature::Signature,
    bovey_signer::{Signer, SignerError},
    bovey_tps_client::{utils::create_connection_cache, TpsClient},
    bovey_tpu_client::tpu_client::{
        TpuClient, TpuClientConfig, DEFAULT_TPU_CONNECTION_POOL_SIZE, DEFAULT_TPU_ENABLE_UDP,
    },
    bovey_transaction::versioned::VersionedTransaction,
    bovey_transaction_error::TransactionError,
    ocp_vote_program::vote_state::VoteAuthorize,
    std::{
...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} BOV)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} BOV)")]
    InsufficientFundsForSpend(f64, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} BOV) + fee ({1} BOV)")]
    InsufficientFundsForSpendAndFee(f64, f64, Pubkey),
    #[error(transparent)]
    InvalidNonce(bovey_rpc_client_nonce_utils::Error),
...
impl From<bovey_rpc_client_nonce_utils::Error> for CliError {
    fn from(error: bovey_rpc_client_nonce_utils::Error) -> Self {
        match error {
            bovey_rpc_client_nonce_utils::Error::Client(client_error) => {
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            get_clap_app(
                crate_name!(),
                crate_description!(),
                bovey_version::version!(),
            )
            .gen_completions_to("bovey", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
        ...
        ("deploy", Some(_matches)) => clap::Error::with_description(
            "`bovey deploy` has been replaced with `bovey program deploy`",
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of bovey-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
        ...
        // Request an airdrop from Bovey Faucet;
...
mod tests {
    use {
        super::*,
        serde_json::json,
        bovey_keypair::{keypair_from_seed, read_keypair_file, write_keypair_file, Keypair},
        bovey_presigner::Presigner,
        bovey_pubkey::Pubkey,
        bovey_rpc_client::mock_sender_for_cli::SIGNATURE,
        bovey_rpc_client_api::{
            request::RpcRequest,
            response::{Response, RpcResponseContext},
        },
        bovey_rpc_client_nonce_utils::blockhash_query,
        bovey_sdk_ids::{stake, system_program},
        bovey_transaction_error::TransactionError,
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(signer_info.index_of(Some(bovey_pubkey::new_rand())), None);
    ...
    fn test_cli_process_command() {
        ...
        assert_eq!(process_command(&config).unwrap(), "0.00000005 BOV");
        ...
        let new_authorized_pubkey = bovey_pubkey::new_rand();
        ...
        let bob_keypair = Keypair::new();
        let bob_pubkey = bob_keypair.pubkey();
        let custodian = bovey_pubkey::new_rand();
        let vote_account_info_response = json!(Response {
            context: RpcResponseContext {
                slot: 1,
                api_version: None
        ...
        let stake_account_pubkey = bovey_pubkey::new_rand();
        ...
        let stake_account_pubkey = bovey_pubkey::new_rand();
        let source_stake_account_pubkey = bovey_pubkey::new_rand();
    ...
    fn test_cli_completions() {
        ...
        for shell in shells {
            ...
            clap_app.gen_completions_to("bovey", shell, &mut buf);
