use {
    ...
    clap::ArgMatches,
    bovey_clap_utils::{
        compute_budget::ComputeUnitLimit, input_parsers::lamports_of_bov, offline::SIGN_ONLY_ARG,
    },
...
impl SpendAmount {
    ...
    pub fn new_from_matches(matches: &ArgMatches<'_>, name: &str) -> Self {
        ...
        let amount = lamports_of_bov(matches, name);
        if amount.is_some() {
    ...
    F: Fn(u64) -> Message,
{
    if sign_only {
        ...
        if amount == SpendAmount::Available && account.owner == bovey_sdk_ids::stake::id() {
