use {
    ...
    bovey_account::{from_account, state_traits::StateMut, Account},
    bovey_clap_utils::{
    ...
    bovey_system_interface::{error::SystemError, instruction as system_instruction},
    bovey_sysvar::stake_history::StakeHistory,
...
impl ArgsConfig for SignOnlySplitNeedsRent {
    fn sign_only_arg<'a, 'b>(&self, arg: Arg<'a, 'b>) -> Arg<'a, 'b> {
        arg.requires("rent_exempt_reserve_bov")
    }
}
    ...
    fn stake_subcommands(self) -> Self {
            ...
            SubCommand::with_name("create-stake-account")
                ...
                .arg(
                    Arg::with_name("amount")
                        ...
                        .required(true)
                        .help(
                            "The amount to send to the stake account, in BOV; accepts keyword ALL",
                        ),
                )
                .arg(pubkey!(
            ...
            SubCommand::with_name("split-stake")
                ...
                .arg(
                    Arg::with_name("amount")
                        ...
                        .validator(is_amount)
                        .required(true)
                        .help("The amount to move into the new stake account, in BOV"),
                )
                .arg(
                    Arg::with_name("seed")
                        .long("seed")
                ...
                .arg(compute_unit_price_arg())
                .arg(
                    Arg::with_name("rent_exempt_reserve_bov")
                        .long("rent-exempt-reserve-bov")
                        .value_name("AMOUNT")
                        .takes_value(true)
                        .validator(is_amount)
                        .requires("sign_only")
        ...
        .subcommand(
            SubCommand::with_name("withdraw-stake")
                .about("Withdraw the unstaked BOV from the stake account")
                .arg(pubkey!(
                    Arg::with_name("stake_account_pubkey")
                        .index(1)
                        .value_name("STAKE_ACCOUNT_ADDRESS")
                ...
                .arg(
                    Arg::with_name("amount")
                        ...
                        .required(true)
                        .help(
                            "The amount to withdraw from the stake account, in BOV; accepts \
            ...
            SubCommand::with_name("stake-account")
                ...
                .arg(
                    Arg::with_name("lamports")
                        .long("lamports")
                        .takes_value(false)
                        .help("Display balance in lamports instead of BOV"),
                )
                .arg(
            ...
            SubCommand::with_name("stake-minimum-delegation")
                ...
                .arg(
                    Arg::with_name("lamports")
                        .long("lamports")
                        .takes_value(false)
                        .help("Display minimum delegation in lamports instead of BOV"),
                ),
        )
    }
}
...
pub fn parse_split_stake(
    ...
    let lamports = lamports_of_bov(matches, "amount").unwrap();
    let seed = matches.value_of("seed").map(|s| s.to_string());
    ...
    let signer_info =
        default_signer.generate_unique_signers(bulk_signers, matches, wallet_manager)?;
    let compute_unit_price = value_of(matches, COMPUTE_UNIT_PRICE_ARG.name);
    let rent_exempt_reserve = lamports_of_bov(matches, "rent_exempt_reserve_bov");
...
pub fn process_create_stake_account(
    ...
    if !sign_only {
        ...
        if let Some(nonce_account) = &nonce_account {
            let nonce_account = bovey_rpc_client_nonce_utils::get_account_with_commitment(
                rpc_client,
                nonce_account,
                config.commitment,
            )?;
...
pub fn process_split_stake(
    ...
    let rent_exempt_reserve = if !sign_only {
        ...
        if lamports < stake_minimum_delegation {
            let lamports = Bov(lamports);
            let stake_minimum_delegation = Bov(stake_minimum_delegation);
            return Err(CliError::BadParameter(format!(
        ...
        rent_exempt_reserve
            .cloned()
            .expect("rent_exempt_reserve_bov is required with sign_only")
    };
...
pub fn get_account_stake_state(
    rpc_client: &RpcClient,
    stake_account_address: &Pubkey,
    stake_account: bovey_account::Account,
    use_lamports_unit: bool,
    with_rewards: Option<usize>,
    use_csv: bool,
    starting_epoch: Option<u64>,
    ...
    match stake_account.state() {
        Ok(stake_state) => {
            ...
            let new_rate_activation_epoch = get_feature_activation_epoch(
                rpc_client,
                &bovey_feature_set::reduce_stake_warmup_cooldown::id(),
            )?;
...
mod tests {
    use {
        ...
        bovey_hash::Hash,
        bovey_keypair::{keypair_from_seed, read_keypair_file, write_keypair, Keypair},
    ...
    fn test_parse_command() {
        ...
        let authorized = bovey_pubkey::new_rand();
        ...
        let vote_account_pubkey = bovey_pubkey::new_rand();
