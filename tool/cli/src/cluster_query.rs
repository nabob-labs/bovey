use {
    ...
    bovey_account::{from_account, state_traits::StateMut},
    bovey_clap_utils::{
    ...
    bovey_clock::{self as clock, Clock, Epoch, Slot},
    bovey_commitment_config::CommitmentConfig,
    bovey_hash::Hash,
    bovey_message::Message,
    bovey_native_token::lamports_to_bov,
    bovey_nonce::state::State as NonceState,
    bovey_program::stake::{self, state::StakeStateV2},
    bovey_pubkey::Pubkey,
    bovey_pubsub_client::pubsub_client::PubsubClient,
    bovey_remote_wallet::remote_wallet::RemoteWalletManager,
    bovey_rent::Rent,
    bovey_rpc_client::rpc_client::{GetConfirmedSignaturesForAddress2Config, RpcClient},
    ...
    bovey_sdk_ids::sysvar::{self, stake_history},
    bovey_signature::Signature,
    bovey_slot_history::{self as slot_history, SlotHistory},
    bovey_system_interface::{instruction as system_instruction, MAX_PERMITTED_DATA_LENGTH},
    bovey_tps_client::TpsClient,
    ...
    ocp_vote_program::vote_state::VoteState,
    std::{
    ...
    fn cluster_query_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("supply")
                .about("Get information about the cluster supply of BOV")
                .arg(
            ...
            SubCommand::with_name("rent")
                ...
                .arg(
                    Arg::with_name("lamports")
                        .long("lamports")
                        .takes_value(false)
                        .help("Display rent in lamports instead of BOV"),
                ),
        )
    }
}
...
pub fn process_catchup(
    ...
    loop {
        ...
        // especially if pointing to api.meinnet-beta.bovey.com at times
...
pub fn process_total_supply(rpc_client: &RpcClient, _config: &CliConfig) -> ProcessResult {
    ...
    Ok(format!("{} BOV", lamports_to_bov(supply.total)))
}
...
pub fn process_show_stakes(
    ...
    let mut program_accounts_config = RpcProgramAccountsConfig {
        account_config: RpcAccountInfoConfig {
            encoding: Some(bovey_account_decoder::UiAccountEncoding::Base64),
...
mod tests {
    use {
        ...
        bovey_keypair::{write_keypair, Keypair},
        std::str::FromStr,
    ...
    fn check_default_rpc_port_inline() {
        assert_eq!(
            ...
            bovey_sdk::rpc_port::DEFAULT_RPC_PORT_STR
        )
    }
}
