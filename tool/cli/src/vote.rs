use {
    ...
    bovey_rpc_client_api::config::RpcGetVoteAccountsConfig,
    bovey_rpc_client_nonce_utils::blockhash_query::BlockhashQuery,
    bovey_system_interface::error::SystemError,
            ...
            SubCommand::with_name("withdraw-from-vote-account")
                ...
                .arg(pubkey!(
                    Arg::with_name("destination_account_pubkey")
                        ...
                        .required(true),
                    "The recipient of withdrawn BOV."
                ))
                .arg(
                    Arg::with_name("amount")
                        ...
                        .help(
                            "The amount to withdraw, in BOV; accepts keyword ALL, which for this \
            ...
            SubCommand::with_name("close-vote-account")
                ...
                .arg(pubkey!(
                    Arg::with_name("destination_account_pubkey")
                        ...
                        .value_name("RECIPIENT_ADDRESS")
                        .required(true),
                    "The recipient of all withdrawn BOV."
                ))
                .arg(
                    Arg::with_name("authorized_withdrawer")
                        .long("authorized-withdrawer")
...
pub fn process_create_vote_account(
    ...
    let vote_account_address = if let Some(seed) = seed {
        Pubkey::create_with_seed(&vote_account_pubkey, seed, &ocp_vote_program::id())?
    } else {
        vote_account_pubkey
    };
    ...
    if !sign_only {
            ...
            rpc_client.get_account_with_commitment(&vote_account_address, config.commitment)
        {
            if let Some(vote_account) = response.value {
                let err_msg = if vote_account.owner == ocp_vote_program::id() {
...
pub(crate) fn get_vote_account(
    ...
    if vote_account.owner != ocp_vote_program::id() {
...
pub fn process_show_vote_account(
    ...
    let tvc_activation_slot =
        rpc_client.get_feature_activation_slot(&bovey_feature_set::timely_vote_credits::id())?;
    ...
    if !sign_only {
        ...
        if let SpendAmount::Some(withdraw_amount) = withdraw_amount {
            ...
            if balance_remaining < minimum_balance && balance_remaining != 0 {
                return Err(CliError::BadParameter(format!(
                    "Withdraw amount too large. The vote account balance must be at least {} BOV \
                     to remain rent exempt",
                    lamports_to_bov(minimum_balance)
                ))
                .into());
            }
        }
...
mod tests {
    use {
        ...
        bovey_hash::Hash,
        bovey_keypair::{read_keypair_file, write_keypair, Keypair},
    ...
    fn test_parse_command() {
        ...
        let authed = bovey_pubkey::new_rand();
        let (keypair_file, mut tmp_file) = make_tmp_file();
        let keypair = Keypair::new();
        write_keypair(&keypair, tmp_file.as_file_mut()).unwrap();
