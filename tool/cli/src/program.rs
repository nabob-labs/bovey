use {
    ...
    log::*,
    bovey_account::{state_traits::StateMut, Account},
    bovey_account_decoder::{UiAccountEncoding, UiDataSliceConfig},
    ocp_bpf_loader_program::syscalls::create_program_runtime_environment_v1,
    bovey_clap_utils::{
        self,
    ...
    bovey_feature_set::{FeatureSet, FEATURE_NAMES},
    bovey_instruction::{error::InstructionError, Instruction},
    bovey_keypair::{keypair_from_seed, read_keypair_file, Keypair},
    bovey_message::Message,
    bovey_packet::PACKET_DATA_SIZE,
    bovey_program::bpf_loader_upgradeable::{
    ...
    bovey_bpf::{elf::Executable, verifier::RequisiteVerifier},
    bovey_sdk_ids::{bpf_loader, bpf_loader_deprecated, compute_budget},
    bovey_signature::Signature,
    bovey_signer::Signer,
    bovey_system_interface::{error::SystemError, MAX_PERMITTED_DATA_LENGTH},
...
impl ProgramSubCommands for App<'_, '_> {
    fn program_subcommands(self) -> Self {
                            ...
                            Arg::with_name("allow_excessive_balance")
                                ...
                                .help(
                                    "Use the designated program id even if the account already \
                                     holds a large balance of BOV (Obsolete)",
                                ),
                        )
                        .arg(
        ...
        .subcommand(
            SubCommand::with_name("deploy")
                .about(
                    "Deploy has been removed. Use `bovey program deploy` instead to deploy \
...
fn process_program_deploy(
    ...
    if !skip_feature_verification {
        if feature_set.is_active(&bovey_feature_set::enable_loader_v4::id()) {
            warn!("Loader-v4 is available now. Please migrate your program.");
        }
        if do_initial_deploy
            && feature_set.is_active(&bovey_feature_set::disable_new_loader_v3_deployments::id())
        {
        ...
        if let Some(write_signer) = write_signer {
            ...
            let transaction_errors = match connection_cache {
                ...
                ConnectionCache::Quic(cache) => {
                    let tpu_client_fut = bovey_client::nonblocking::tpu_client::TpuClient::new_with_connection_cache(
                        rpc_client.get_inner_client().clone(),
                        config.websocket_url.as_str(),
                        bovey_client::tpu_client::TpuClientConfig::default(),
                        cache,
                    );
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic, ephemeral_pubkey: &Pubkey) {
    ...
    eprintln!("`bovey-keygen recover` and the following {words}-word seed phrase:");
    eprintln!("{divider}\n{phrase}\n{divider}");
    eprintln!("To resume a deploy, pass the recovered keypair as the");
    eprintln!("[BUFFER_SIGNER] to `bovey program deploy` or `bovey program write-buffer'.");
    eprintln!("Or to recover the account's lamports, use:");
    eprintln!("{divider}\nbovey program close {ephemeral_pubkey}\n{divider}");
}
...
mod tests {
    use {
        ...
        serde_json::Value,
        bovey_cli_output::OutputFormat,
        bovey_hash::Hash,
        bovey_keypair::write_keypair_file,
    };
