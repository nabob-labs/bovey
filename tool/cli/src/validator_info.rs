use {
    ...
    bovey_account_decoder::validator_info::{
    ...
    bovey_cli_output::{CliValidatorInfo, CliValidatorInfoVec},
    ocp_config_program::{config_instruction, get_config_data, ConfigKeys, ConfigState},
...
fn verify_keybase(
    ...
    if let Some(keybase_username) = keybase_username.as_str() {
        let url =
            format!("https://keybase.pub/{keybase_username}/bovey/validator-{validator_pubkey:?}");
...
fn parse_validator_info(
    ...
    if account.owner != ocp_config_program::id() {
    ...
    fn validator_info_subcommands(self) -> Self {
        self.subcommand(
            SubCommand::with_name("validator-info")
                .about("Publish/get Validator info on Bovey")
                .setting(AppSettings::SubcommandRequiredElseHelp)
                .subcommand(
                    SubCommand::with_name("publish")
                        .about("Publish Validator info on Bovey")
                        .arg(
                ...
                .subcommand(
                    SubCommand::with_name("get")
                        .about("Get and parse Bovey Validator info")
                        .arg(
                            Arg::with_name("info_pubkey")
                                .index(1)
                                .value_name("PUBKEY")
...
pub fn process_set_validator_info(
    ...
    let all_config = rpc_client.get_program_accounts(&ocp_config_program::id())?;
