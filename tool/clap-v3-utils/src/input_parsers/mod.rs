use {
    ...
    bovey_pubkey::{Pubkey, MAX_SEED_LEN},
    bovey_signer::Signer,
    std::str::FromStr,
};
...
#[deprecated(
    since = "1.17.0",
    note = "Please use the functions in `bovey_clap_v3_utils::input_parsers::signer` directly instead"
)]
#[allow(deprecated)]
pub use signer::{
...
#[deprecated(
    since = "1.17.0",
    note = "please use `Amount::parse_decimal` and `Amount::bov_to_lamport` instead"
)]
#[allow(deprecated)]
pub fn lamports_of_bov(matches: &ArgMatches, name: &str) -> Option<u64> {
...
impl Amount {
    ...
    pub fn bov_to_lamport(&self) -> Amount {
        const NATIVE_BOV_DECIMALS: u8 = 9;
        self.to_raw_amount(NATIVE_BOV_DECIMALS)
    }
}
...
mod tests {
    use {
        ...
        bovey_commitment_config::{CommitmentConfig, CommitmentLevel},
        bovey_hash::Hash,
        bovey_pubkey::Pubkey,
    };
