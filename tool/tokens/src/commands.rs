use {
    ...
    bovey_account_decoder::parse_token::real_number_string,
    ...
    bovey_sdk::{
        ...
        message::Message,
        native_token::{lamports_to_bov, bov_to_lamports},
    ...
    bovey_transaction_status::TransactionStatus,
    ocp_associated_token_account::get_associated_token_address,
    ocp_token::bovey_program::program_error::ProgramError,
    std::{
        cmp::{self},
        io,
        str::FromStr,
...
fn distribution_instructions(
    ...
    if args.ocp_token_args.is_some() {
        return build_ocp_token_instructions(allocation, args, do_create_associated_token_account);
    }
    ...
    match &args.stake_args {
        ...
        Some(stake_args) => {
            let unlocked_bov = stake_args.unlocked_bov;
            ...
            let mut instructions = match &stake_args.sender_stake_args {
                ...
                None => {
                    ...
                    stake_instruction::create_account(
                        ...
                        &authorized,
                        &lockup,
                        allocation.amount - unlocked_bov,
                    )
                }
                ...
                Some(sender_stake_args) => {
                    ...
                    instructions.append(&mut stake_instruction::split(
                        ...
                        &stake_authority,
                        allocation.amount - unlocked_bov - rent_exempt_reserve,
                        new_stake_account_address,
                    ));
...
fn build_messages(
    ...
    if let Some(ocp_token_args) = &args.ocp_token_args {
    ...
    for (i, allocation) in allocations.iter().enumerate() {
        ...
        let do_create_associated_token_account = if let Some(ocp_token_args) = &args.ocp_token_args
        {
            ...
            println!(
                ...
                real_number_string(allocation.amount, ocp_token_args.decimals)
            );
            ...
            println!(
                "{:<44}  {:>24.9}",
                allocation.recipient,
                lamports_to_bov(allocation.amount)
            );
            false
        };
...
fn distribute_allocations(
    ...
    if args.ocp_token_args.is_some() {
        check_ocp_token_balances(&messages, allocations, client, args, created_accounts)?;
    } else {
...
fn read_allocations(
    ...
    let allocations = if let Some(amount) = transfer_amount {
        ...
        // We only support BOV token in "require lockup" mode.
        rdr.deserialize()
            .map(|recipient| {
...
pub fn process_allocations(
    ...
    let mut allocations: Vec<TypedAllocation> = read_allocations(
        ...
        with_lockup,
        args.ocp_token_args.is_some(),
    )?;

    let starting_total_tokens = allocations.iter().map(|x| x.amount).sum();
    let starting_total_tokens = if let Some(ocp_token_args) = &args.ocp_token_args {
        Token::ocp_token(starting_total_tokens, ocp_token_args.decimals)
    } else {
        Token::bov(starting_total_tokens)
    };
    println!(
        "{} {}",
    ...
    let (distributed_tokens, undistributed_tokens) =
        if let Some(ocp_token_args) = &args.ocp_token_args {
            (
                Token::ocp_token(distributed_tokens, ocp_token_args.decimals),
                Token::ocp_token(undistributed_tokens, ocp_token_args.decimals),
            )
        } else {
            (
                Token::bov(distributed_tokens),
                Token::bov(undistributed_tokens),
            )
        };
...
fn check_payer_balances(
    ...
    let (distribution_source, unlocked_bov_source) = if let Some(stake_args) = &args.stake_args {
        let total_unlocked_bov = allocations.len() as u64 * stake_args.unlocked_bov;
        undistributed_tokens -= total_unlocked_bov;
        ...
        (
            from_pubkey,
            Some((args.sender_keypair.pubkey(), total_unlocked_bov)),
        )
    } else {
    ...
    if let Some((unlocked_bov_source, total_unlocked_bov)) = unlocked_bov_source {
        ...
        if staker_balance < undistributed_tokens {
            return Err(Error::InsufficientFunds(
                ...
                lamports_to_bov(undistributed_tokens).to_string(),
            ));
        }
        if args.fee_payer.pubkey() == unlocked_bov_source {
            if fee_payer_balance < fees + total_unlocked_bov {
                return Err(Error::InsufficientFunds(
                    vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                    lamports_to_bov(fees + total_unlocked_bov).to_string(),
                ));
            }
        } else {
            if fee_payer_balance < fees {
                return Err(Error::InsufficientFunds(
                    ...
                    lamports_to_bov(fees).to_string(),
                ));
            }
            let unlocked_bov_balance = client.get_balance(&unlocked_bov_source)?;
            if unlocked_bov_balance < total_unlocked_bov {
                return Err(Error::InsufficientFunds(
                    vec![FundingSource::SystemAccount].into(),
                    lamports_to_bov(total_unlocked_bov).to_string(),
                ));
            }
        }
        ...
        if fee_payer_balance < fees + undistributed_tokens {
            return Err(Error::InsufficientFunds(
                vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                lamports_to_bov(fees + undistributed_tokens).to_string(),
            ));
        }
    } else {
        if fee_payer_balance < fees {
...
pub fn process_balances(
    ...
    let allocations: Vec<TypedAllocation> =
        read_allocations(&args.input_csv, None, false, args.ocp_token_args.is_some())?;
    let allocations = merge_allocations(&allocations);

    let token = if let Some(ocp_token_args) = &args.ocp_token_args {
        ocp_token_args.mint.to_string()
    } else {
        "◎".to_string()
    };
    ...
    for allocation in &allocations {
        ...
        if let Some(ocp_token_args) = &args.ocp_token_args {
            print_token_balances(client, allocation, ocp_token_args)?;
        } else {
            let address: Pubkey = allocation.recipient;
            let expected = lamports_to_bov(allocation.amount);
            let actual = lamports_to_bov(client.get_balance(&address).unwrap());
            println!(
                "{:<44}  {:>24.9}  {:>24.9}  {:>24.9}",
                allocation.recipient,
                expected,
...
pub fn test_process_distribute_tokens_with_client(
    ...
    wtr.write_record([
        ...
        lamports_to_bov(expected_amount).to_string(),
    ])
    .unwrap();
    wtr.flush().unwrap();
...
pub fn test_process_create_stake_with_client(client: &RpcClient, sender_keypair: Keypair) {
    ...
    let expected_amount = bov_to_lamports(1000.0);
    let alice_pubkey = pubkey::new_rand();
    ...
    let stake_args = StakeArgs {
        ...
        unlocked_bov: bov_to_lamports(1.0),
    ...
    assert_eq!(
        ...
        expected_amount - bov_to_lamports(1.0),
    );
...
mod tests {
    use {
        ...
        bovey_test_validator::TestValidator,
        bovey_transaction_status::TransactionConfirmationStatus,
    };
    ...
    fn test_process_transfer_amount_allocations() {
        ...
        let client = RpcClient::new_with_commitment(url, CommitmentConfig::processed());
        test_process_distribute_tokens_with_client(&client, alice, Some(bov_to_lamports(1.5)));
    }
    ...
    fn test_read_allocations() {
        ...
        let allocation_bov = TypedAllocation {
            recipient: alice_pubkey,
            amount: bov_to_lamports(42.0),
            lockup_date: None,
        };

        assert_eq!(
            read_allocations(&input_csv, None, true, true).unwrap(),
            vec![allocation_bov.clone()]
        );
        assert_eq!(
    ...
    fn test_read_allocations_transfer_amount() {
        ...
        let amount = bov_to_lamports(1.5);
    ...
    fn test_set_split_stake_lockup() {
        ...
        let allocation = TypedAllocation {
            ...
            amount: bov_to_lamports(1.002_282_880),
    ...
    fn test_check_payer_balances_distribute_tokens_single_payer() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, (allocation_amount + fees_in_bov).to_string());
        } else {
    ...
    fn test_check_payer_balances_distribute_tokens_separate_payers() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, fees_in_bov.to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    }
    ...
    fn test_check_payer_balances_distribute_stakes_single_payer() {
        ...
        let allocation_amount = 1000.0;
        let unlocked_bov = 1.0;
        let stake_args = initialize_stake_account(
            bov_to_lamports(allocation_amount),
            bov_to_lamports(unlocked_bov),
            &alice,
            &client,
        );
        ...
        let expensive_allocations = vec![TypedAllocation {
            ...
            amount: bov_to_lamports(expensive_allocation_amount),
            lockup_date: None,
        }];
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(
                amount,
                (expensive_allocation_amount - unlocked_bov).to_string()
            );
        } else {
            panic!("check_payer_balances should have errored");
        }
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, (unlocked_bov + fees_in_bov).to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    ...
    fn test_check_payer_balances_distribute_stakes_separate_payers() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            assert_eq!(sources, vec![FundingSource::SystemAccount].into());
            assert_eq!(amount, unlocked_bov.to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    ...
    fn test_build_messages_dump_db() {
        ...
        let amount = bov_to_lamports(1.0);
