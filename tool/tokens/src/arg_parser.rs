use {
    crate::args::{
        Args, BalancesArgs, Command, DistributeTokensArgs, SenderStakeArgs, OcpTokenArgs,
    ...
    bovey_cli_config::CONFIG_FILE,
    bovey_remote_wallet::remote_wallet::maybe_wallet_manager,
    bovey_sdk::native_token::bov_to_lamports,
    ...
    App::new(crate_name!())
            ...
            SubCommand::with_name("distribute-tokens")
                ...
                .arg(
                    Arg::with_name("transfer_amount")
                        ...
                        .value_name("AMOUNT")
                        .validator(is_amount)
                        .help("The amount to send to each recipient, in BOV"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("create-stake")
                ...
                .arg(
                    Arg::with_name("unlocked_bov")
                        .default_value("1.0")
                        .long("unlocked-bov")
                        .takes_value(true)
                        .value_name("BOV_AMOUNT")
                        .help("Amount of BOV to put in system account to pay for fees"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("distribute-ocp-tokens")
                .about("Distribute OCP tokens")
                .arg(
                    Arg::with_name("db_path")
                        .long("db-path")
                        .required(true)
                ...
                .arg(
                    Arg::with_name("transfer_amount")
                        ...
                        .value_name("AMOUNT")
                        .validator(is_amount)
                        .help("The amount of OCP tokens to send to each recipient"),
                )
                .arg(
                    Arg::with_name("output_path")
                        .long("output-path")
                ...
                .arg(
                    Arg::with_name("token_account_address")
                        ...
                        .validator(is_valid_pubkey)
                        .help("OCP token account to send from"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("ocp-token-balances")
                .about("Balance of OCP token associated accounts")
                .arg(
                    Arg::with_name("input_csv")
                        .long("input-csv")
                        .required(true)
                ...
                .arg(
                    Arg::with_name("mint_address")
                        ...
                        .validator(is_valid_pubkey)
                        .help("OCP token mint of distribution"),
                ),
        )
        .subcommand(
...
fn parse_distribute_tokens_args(
    ...
    Ok(DistributeTokensArgs {
        ...
        stake_args: None,
        ocp_token_args: None,
        transfer_amount: value_of(matches, "transfer_amount").map(bov_to_lamports),
    })
}
...
fn parse_create_stake_args(
    ...
    let stake_args = StakeArgs {
        unlocked_bov: bov_to_lamports(value_t_or_exit!(matches, "unlocked_bov", f64)),
        lockup_authority,
...
fn parse_distribute_ocp_tokens_args(
    matches: &ArgMatches<'_>,
) -> Result<DistributeTokensArgs, Box<dyn Error>> {
    let mut wallet_manager = maybe_wallet_manager()?;
    let signer_matches = ArgMatches::default(); // No default signer
    ...
    Ok(DistributeTokensArgs {
        ...
        fee_payer,
        stake_args: None,
        ocp_token_args: Some(OcpTokenArgs {
            token_account_address,
            ..OcpTokenArgs::default()
        }),
...
fn parse_balances_args(matches: &ArgMatches<'_>) -> Result<BalancesArgs, Box<dyn Error>> {
    let mut wallet_manager = maybe_wallet_manager()?;
    let ocp_token_args =
        pubkey_of_signer(matches, "mint_address", &mut wallet_manager)?.map(|mint| OcpTokenArgs {
            mint,
            ..OcpTokenArgs::default()
        });
    Ok(BalancesArgs {
    ...
    T: Into<OsString> + Clone,
{
    ...
    let command = match matches.subcommand() {
        ...
        ("distribute-ocp-tokens", Some(matches)) => {
            Command::DistributeTokens(parse_distribute_ocp_tokens_args(matches)?)
        }
        ("balances", Some(matches)) => Command::Balances(parse_balances_args(matches)?),
        ("ocp-token-balances", Some(matches)) => Command::Balances(parse_balances_args(matches)?),
