...
use {
    ...
    bovey_signer::{EncodableKey, EncodableKeypair, Signer, SignerError},
    std::{
        error,
        io::{Read, Write},
        path::Path,
...
impl ElGamalKeypair {
    ...
    /// Deterministically derives an ElGamal keypair from a Bovey signer and a public seed.
    ///
    /// This function exists for applications where a user may not wish to maintain a Bovey signer
    ...
    ///
    /// For the ocp-token-2022 confidential extension, the ElGamal public key is specified in a
    /// token account. A natural way to derive an ElGamal keypair is to define it from the hash of
    /// a Bovey keypair and a Bovey address as the public seed. However, for general hardware
...
#[cfg(not(target_arch = "wasm32"))]
impl ElGamalSecretKey {
    /// Deterministically derives an ElGamal secret key from a Bovey signer and a public seed.
    ///
    ...
    /// Derive a seed from a Bovey signer used to generate an ElGamal secret key.
    ///
...
mod tests {
    use {
        ...
        bovey_pubkey::Pubkey,
        bovey_signer::null_signer::NullSigner,
        std::fs::{self, File},
    };
