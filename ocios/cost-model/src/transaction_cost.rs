...
use bovey_compute_budget_instruction::compute_budget_instruction_details::ComputeBudgetInstructionDetails;
use {
    crate::block_cost_limits, bovey_pubkey::Pubkey,
    bovey_runtime_transaction::transaction_meta::StaticMeta,
...
impl<Tx> TransactionCost<'_, Tx> {
            ...
            Self::SimpleVote { .. } => {
                const _: () = assert!(
                    SIMPLE_VOTE_USAGE_COST
                        == ocp_vote_program::vote_processor::DEFAULT_COMPUTE_UNITS
    ...
    pub fn programs_execution_cost(&self) -> u64 {
        match self {
            Self::SimpleVote { .. } => ocp_vote_program::vote_processor::DEFAULT_COMPUTE_UNITS,
...
impl<Tx: VMMessage> TransactionCost<'_, Tx> {
...
#[cfg(feature = "dev-context-only-utils")]
impl bovey_vm_transaction::vm_message::VMMessage for WritableKeysTransaction {
    ...
    fn recent_blockhash(&self) -> &bovey_hash::Hash {
    ...
    fn instructions_iter(
        &self,
    ) -> impl Iterator<Item = bovey_vm_transaction::instruction::VMInstruction> {
        core::iter::empty()
    }

    fn program_instructions_iter(
        &self,
    ) -> impl Iterator<Item = (&Pubkey, bovey_vm_transaction::instruction::VMInstruction)> + Clone
    {
        core::iter::empty()
    }

    fn account_keys(&self) -> bovey_message::AccountKeys {
        bovey_message::AccountKeys::new(&self.0, None)
    }
    ...
    fn message_address_table_lookups(
        ...
        Item = bovey_vm_transaction::message_address_table_lookup::VMMessageAddressTableLookup,
    > {
        core::iter::empty()
    }
}
...
#[cfg(feature = "dev-context-only-utils")]
impl bovey_vm_transaction::vm_transaction::VMTransaction for WritableKeysTransaction {
    fn signature(&self) -> &bovey_signature::Signature {
        unimplemented!("WritableKeysTransaction::signature")
    }

    fn signatures(&self) -> &[bovey_signature::Signature] {
        unimplemented!("WritableKeysTransaction::signatures")
    }
}

#[cfg(feature = "dev-context-only-utils")]
impl bovey_runtime_transaction::transaction_meta::StaticMeta for WritableKeysTransaction {
    fn message_hash(&self) -> &bovey_hash::Hash {
    ...
    fn signature_details(&self) -> &bovey_message::TransactionSignatureDetails {
        const DUMMY: bovey_message::TransactionSignatureDetails =
            bovey_message::TransactionSignatureDetails::new(0, 0, 0, 0);
        &DUMMY
    }
...
#[cfg(feature = "dev-context-only-utils")]
impl bovey_runtime_transaction::transaction_with_meta::TransactionWithMeta
    for WritableKeysTransaction
{
    #[allow(refining_impl_trait)]
    fn as_sanitized_transaction(
        &self,
    ) -> std::borrow::Cow<bovey_transaction::sanitized::SanitizedTransaction> {
        unimplemented!("WritableKeysTransaction::as_sanitized_transaction");
    }

    fn to_versioned_transaction(&self) -> bovey_transaction::versioned::VersionedTransaction {
...
mod tests {
    use {
        ...
        bovey_message::SimpleAddressLoader,
        bovey_reserved_account_keys::ReservedAccountKeys,
        bovey_runtime_transaction::runtime_transaction::RuntimeTransaction,
        bovey_transaction::{sanitized::MessageHash, versioned::VersionedTransaction},
        bovey_vote::vote_transaction,
        ocp_vote_program::vote_state::TowerSync,
    };
