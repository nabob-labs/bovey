use {
    ...
    bovey_builtins_default_costs::{get_migration_feature_id, MIGRATING_BUILTINS_COSTS},
...
impl ComputeBudgetInstructionDetails {
    ...
    fn process_instruction(&mut self, index: u8, instruction: &VMInstruction) -> Result<()> {
...
mod test {
    use {
        super::*,
        bovey_builtins_default_costs::get_migration_feature_position,
    ...
    fn test_try_from_request_heap() {
        ...
        assert_eq!(
            ComputeBudgetInstructionDetails::try_from(VMMessage::program_instructions_iter(&tx),),
            expected_details
        );
    ...
    fn test_builtin_program_migration() {
        let tx = build_sanitized_transaction(&[
            Instruction::new_with_bincode(Pubkey::new_unique(), &(), vec![]),
            bovey_program::stake::instruction::delegate_stake(
                &Pubkey::new_unique(),
                &Pubkey::new_unique(),
                &Pubkey::new_unique(),
            ),
