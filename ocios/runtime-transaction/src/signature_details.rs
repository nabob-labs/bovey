...
use {
    bovey_transaction_view::static_account_keys_frame::MAX_STATIC_ACCOUNTS_PER_PACKET as FILTER_SIZE,
    bovey_pubkey::Pubkey, bovey_vm_transaction::instruction::VMInstruction,
};
...
#[inline]
fn get_num_signatures_in_instruction(instruction: &VMInstruction) -> u64 {
...
impl SignatureDetailsFilter {
    ...
    fn check_program_id(program_id: &Pubkey) -> ProgramIdStatus {
        if program_id == &bovey_sdk_ids::secp256k1_program::ID {
            ProgramIdStatus::Secp256k1
        } else if program_id == &bovey_sdk_ids::ed25519_program::ID {
            ProgramIdStatus::Ed25519
        } else if program_id == &bovey_sdk_ids::secp256r1_program::ID {
            ProgramIdStatus::Secp256r1
        } else {
            ProgramIdStatus::NotSignature
        }
...
mod tests {
    ...
    fn test_get_signature_details_signatures_mixed() {
        let program_ids = [
            ...
            bovey_sdk_ids::ed25519_program::ID,
            bovey_sdk_ids::secp256r1_program::ID,
        ];
        let instructions = [
