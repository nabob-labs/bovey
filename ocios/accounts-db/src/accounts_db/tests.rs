...
fn create_loadable_account_with_fields(
    ...
    AccountSharedData::from(Account {
        lamports,
        owner: bovey_sdk_ids::native_loader::id(),
...
fn test_get_keys_to_unref_ancient() {
    ...
    let pubkey4 = bovey_pubkey::new_rand();
...
fn test_clean_old_with_both_normal_and_zero_lamport_accounts() {
    ...
    let mut accounts = AccountsDb {
        account_indexes: ocp_token_mint_index_enabled(),
    ...
    let mut account_data_with_mint = vec![0; bovey_inline_ocp::token::Account::get_packed_len()];
    ...
    normal_account.set_owner(bovey_inline_ocp::token::id());
    ...
    zero_account.set_owner(bovey_inline_ocp::token::id());
    ...
    let index_key = IndexKey::OcpTokenMint(mint_key);
    let bank_id = 0;
    accounts
        .accounts_index
        .index_scan_accounts(
...
fn test_verify_bank_capitalization() {
    for pass in 0..2 {
        ...
        let native_account_pubkey = bovey_pubkey::new_rand();
        db.store_for_tests(
            some_slot,
            &[(
...
fn test_storage_finder() {
    ...
    let lamports = 100;
    let data_len = 8190;
    let account = AccountSharedData::new(lamports, data_len, &bovey_pubkey::new_rand());
...
fn test_account_balance_for_capitalization_sysvar() {
    let normal_sysvar =
        bovey_account::create_account_for_test(&bovey_slot_history::SlotHistory::default());
...
fn test_flush_accounts_cache() {
    ...
    let root5 = 5;
    let root6 = 6;
    let unrooted_key = bovey_pubkey::new_rand();
    let key5 = bovey_pubkey::new_rand();
    let key6 = bovey_pubkey::new_rand();
...
define_accounts_db_test!(test_calculate_storage_count_and_alive_bytes, |accounts| {
    ...
    let shared_key = bovey_pubkey::new_rand();
    let account = AccountSharedData::new(1, 1, AccountSharedData::default().owner());
    let slot0 = 0;
...
define_accounts_db_test!(
    ...
    |accounts| {
        let keys = [
            bovey_pubkey::Pubkey::from([0; 32]),
            bovey_pubkey::Pubkey::from([255; 32]),
        ];
        accounts.accounts_index.set_startup(Startup::Startup);
...
define_accounts_db_test!(
    ...
    |accounts_db| {
        let pubkey = bovey_pubkey::new_rand();
        let owner = bovey_pubkey::new_rand();
        let space = 0;
...
fn test_hash_storage_info() {
    ...
    }
    {
        ...
        assert!(load);
        let mut hasher = DefaultHasher::new();
        append_sample_data_to_storage(&storage, &bovey_pubkey::new_rand(), false, None);
        let load = AccountsDb::hash_storage_info(&mut hasher, &storage, slot);
