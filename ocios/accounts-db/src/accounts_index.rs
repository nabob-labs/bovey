...
use {
    ...
    bovey_account::ReadableAccount,
    bovey_clock::{BankId, Slot},
    bovey_measure::measure::Measure,
    bovey_pubkey::Pubkey,
    std::{
...
pub struct AccountsIndex<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> {
    ...
    ocp_token_mint_index: SecondaryIndex<RwLockSecondaryIndexEntry>,
    ocp_token_owner_index: SecondaryIndex<RwLockSecondaryIndexEntry>,
...
impl<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> AccountsIndex<T, U> {
    ...
    pub fn new(config: Option<AccountsIndexConfig>, exit: Arc<AtomicBool>) -> Self {
        ...
        Self {
            ...
            ocp_token_mint_index: SecondaryIndex::<RwLockSecondaryIndexEntry>::new(
                "ocp_token_mint_index_stats",
            ),
            ocp_token_owner_index: SecondaryIndex::<RwLockSecondaryIndexEntry>::new(
                "ocp_token_owner_index_stats",
            ),
        ...
        R: RangeBounds<Pubkey> + std::fmt::Debug,
    {
        ...
        */
        match scan_type {
            ...
            ScanTypes::Indexed(IndexKey::OcpTokenMint(mint_key)) => {
                self.do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.ocp_token_mint_index,
                    &mint_key,
                    Some(max_root),
                    config,
                );
            }
            ScanTypes::Indexed(IndexKey::OcpTokenOwner(owner_key)) => {
                self.do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.ocp_token_owner_index,
                    &owner_key,
                    Some(max_root),
                    config,
                );
    ...
    fn update_ocp_token_secondary_indexes<G: bovey_inline_ocp::token::GenericTokenAccount>(
        &self,
        token_id: &Pubkey,
        pubkey: &Pubkey,
        ...
        if *account_owner == *token_id {
            if account_indexes.contains(&AccountIndex::OcpTokenOwner) {
                if let Some(owner_key) = G::unpack_account_owner(account_data) {
                    if account_indexes.include_key(owner_key) {
                        self.ocp_token_owner_index.insert(owner_key, pubkey);
                    }
                }
            }

            if account_indexes.contains(&AccountIndex::OcpTokenMint) {
                if let Some(mint_key) = G::unpack_account_mint(account_data) {
                    if account_indexes.include_key(mint_key) {
                        self.ocp_token_mint_index.insert(mint_key, pubkey);
                    }
                }
            }
        }
    ...
    pub fn get_index_key_size(&self, index: &AccountIndex, index_key: &Pubkey) -> Option<usize> {
        match index {
            ...
            AccountIndex::OcpTokenOwner => self
                .ocp_token_owner_index
                .index
                .get(index_key)
                .map(|x| x.len()),
            AccountIndex::OcpTokenMint => self
                .ocp_token_mint_index
                .index
                .get(index_key)
                .map(|x| x.len()),
        }
    ...
    pub(crate) fn log_secondary_indexes(&self) {
        ...
        if !self.ocp_token_mint_index.index.is_empty() {
            info!("secondary index: {:?}", AccountIndex::OcpTokenMint);
            self.ocp_token_mint_index.log_contents();
        }
        if !self.ocp_token_owner_index.index.is_empty() {
            info!("secondary index: {:?}", AccountIndex::OcpTokenOwner);
            self.ocp_token_owner_index.log_contents();
        }
    }
    ...
    pub(crate) fn update_secondary_indexes(
        ...
        // filtered out by post-scan filters, like in `get_filtered_ocp_token_accounts_by_owner()`.

        self.update_ocp_token_secondary_indexes::<bovey_inline_ocp::token::Account>(
        ...
        self.update_ocp_token_secondary_indexes::<bovey_inline_ocp::token_2022::Account>(
    ...
    fn purge_secondary_indexes_by_inner_key(
        ...
        if account_indexes.contains(&AccountIndex::OcpTokenOwner) {
            self.ocp_token_owner_index.remove_by_inner_key(inner_key);
        }

        if account_indexes.contains(&AccountIndex::OcpTokenMint) {
            self.ocp_token_mint_index.remove_by_inner_key(inner_key);
        }
    }
...
pub mod tests {
    use {
        super::*,
        bovey_account::{AccountSharedData, WritableAccount},
        bovey_inline_ocp::token::OCP_TOKEN_ACCOUNT_OWNER_OFFSET,
        bovey_pubkey::PUBKEY_BYTES,
        std::ops::RangeInclusive,
    };

    const OCP_TOKENS: &[Pubkey] = &[
        bovey_inline_ocp::token::id(),
        bovey_inline_ocp::token_2022::id(),
    ];
    ...
    pub fn ocp_token_mint_index_enabled() -> AccountSecondaryIndexes {
        let mut account_indexes = HashSet::new();
        account_indexes.insert(AccountIndex::OcpTokenMint);
    ...
    pub fn ocp_token_owner_index_enabled() -> AccountSecondaryIndexes {
        let mut account_indexes = HashSet::new();
        account_indexes.insert(AccountIndex::OcpTokenOwner);
        AccountSecondaryIndexes {
            indexes: account_indexes,
            keys: None,
        }
    ...
    fn create_ocp_token_mint_secondary_index_state() -> (usize, usize, AccountSecondaryIndexes) {
        {
            // Check that we're actually testing the correct variant
            let index = AccountsIndex::<bool, bool>::default_for_tests();
            let _type_check = SecondaryIndexTypes::RwLock(&index.ocp_token_mint_index);
        }

        (0, PUBKEY_BYTES, ocp_token_mint_index_enabled())
    }

    fn create_ocp_token_owner_secondary_index_state() -> (usize, usize, AccountSecondaryIndexes) {
        {
            // Check that we're actually testing the correct variant
            let index = AccountsIndex::<bool, bool>::default_for_tests();
            let _type_check = SecondaryIndexTypes::RwLock(&index.ocp_token_owner_index);
        }

        (
            OCP_TOKEN_ACCOUNT_OWNER_OFFSET,
            OCP_TOKEN_ACCOUNT_OWNER_OFFSET + PUBKEY_BYTES,
            ocp_token_owner_index_enabled(),
        )
    }
    ...
    fn setup_accounts_index_keys(num_pubkeys: usize) -> (AccountsIndex<bool, bool>, Vec<Pubkey>) {
        ...
        let mut pubkeys: Vec<Pubkey> = std::iter::repeat_with(|| {
            let new_pubkey = bovey_pubkey::new_rand();
            index.upsert(
                root_slot,
                root_slot,
                &new_pubkey,
    ...
    fn make_empty_token_account_data() -> Vec<u8> {
        vec![0; bovey_inline_ocp::token::Account::get_packed_len()]
    }
    ...
    #[test]
    fn test_purge_exact_ocp_token_mint_secondary_index() {
        let (key_start, key_end, secondary_indexes) = create_ocp_token_mint_secondary_index_state();
        let index = AccountsIndex::<bool, bool>::default_for_tests();
        run_test_purge_exact_secondary_index(
            &index,
            &index.ocp_token_mint_index,
            key_start,
            key_end,
            &secondary_indexes,
        );
    ...
    #[test]
    fn test_purge_exact_ocp_token_owner_secondary_index() {
        let (key_start, key_end, secondary_indexes) =
            create_ocp_token_owner_secondary_index_state();
        let index = AccountsIndex::<bool, bool>::default_for_tests();
        run_test_purge_exact_secondary_index(
            &index,
            &index.ocp_token_owner_index,
            key_start,
            key_end,
            &secondary_indexes,
        );
    ...
    fn run_test_ocp_token_secondary_indexes<
        SecondaryIndexEntryType: SecondaryIndexEntry + Default + Sync + Send,
    >(
        token_id: &Pubkey,
        index: &AccountsIndex<bool, bool>,
    ...
    #[test]
    fn test_ocp_token_mint_secondary_index() {
        let (key_start, key_end, secondary_indexes) = create_ocp_token_mint_secondary_index_state();
        let index = AccountsIndex::<bool, bool>::default_for_tests();
        for token_id in OCP_TOKENS {
            run_test_ocp_token_secondary_indexes(
                token_id,
                &index,
                &index.ocp_token_mint_index,
                key_start,
    ...
    #[test]
    fn test_ocp_token_owner_secondary_index() {
        let (key_start, key_end, secondary_indexes) =
            create_ocp_token_owner_secondary_index_state();
        let index = AccountsIndex::<bool, bool>::default_for_tests();
        for token_id in OCP_TOKENS {
    ...
    #[test]
    fn test_ocp_token_mint_secondary_index_same_slot_and_forks() {
        let (key_start, key_end, account_index) = create_ocp_token_mint_secondary_index_state();
        let index = AccountsIndex::<bool, bool>::default_for_tests();
        for token_id in OCP_TOKENS {
    ...
    fn test_rwlock_secondary_index_same_slot_and_forks() {
        let (key_start, key_end, account_index) = create_ocp_token_owner_secondary_index_state();
        let index = AccountsIndex::<bool, bool>::default_for_tests();
        for token_id in OCP_TOKENS {
            run_test_secondary_indexes_same_slot_and_forks(
                token_id,
    ...
    fn test_clean_rooted_entries_return() {
        ...
        let value = true;
        let key = bovey_pubkey::new_rand();
        let key_unknown = bovey_pubkey::new_rand();
        let index = AccountsIndex::<bool, bool>::default_for_tests();
        let slot1 = 1;
