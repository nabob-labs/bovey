use {
    ...
    bovey_pubkey::Pubkey,
    bovey_vote_interface::state::VoteState,
    std::{
        cmp::Ordering,
...
impl VoteAccount {
    ...
    pub fn new_random() -> VoteAccount {
        use {
            rand::Rng as _,
            bovey_clock::Clock,
            bovey_vote_interface::state::{VoteInit, VoteStateVersions},
        };
...
impl TryFrom<AccountSharedData> for VoteAccount {
    ...
    fn try_from(account: AccountSharedData) -> Result<Self, Self::Error> {
        if !bovey_sdk_ids::vote::check_id(account.owner()) {
...
mod tests {
    ...
    fn test_vote_account_try_from_invalid_account() {
        ...
        account.set_owner(bovey_sdk_ids::vote::id());
        VoteAccount::try_from(account).unwrap();
    }
    ...
    fn test_vote_accounts_deserialize_invalid_account() {
        ...
        let invalid_account_data =
            AccountSharedData::new_data(42, &vec![0xFF; 42], &bovey_sdk_ids::vote::id()).unwrap();
