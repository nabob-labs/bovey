use {
    crate::vote_transaction::VoteTransaction, bovey_bincode::limited_deserialize,
    bovey_hash::Hash, bovey_pubkey::Pubkey, bovey_signature::Signature,
    bovey_vm_transaction::vm_transaction::VMTransaction, bovey_transaction::Transaction,
    bovey_vote_interface::instruction::VoteInstruction,
};
...
pub fn parse_sanitized_vote_transaction(tx: &impl VMTransaction) -> Option<ParsedVote> {
    // Check first instruction for a vote
    let (program_id, first_instruction) = tx.program_instructions_iter().next()?;
    if !bovey_sdk_ids::vote::check_id(program_id) {
        return None;
    }
...
mod test {
    use {
        ...
        bovey_signer::Signer,
        bovey_vote_interface::{instruction as vote_instruction, state::Vote},
    };
