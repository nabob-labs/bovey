use {
    ...
    bovey_feature_set::{
        lift_cpi_caller_restriction, move_precompile_verification_to_vm,
        remove_accounts_executable_flag_checks, FeatureSet,
    },
    bovey_hash::Hash,
    bovey_instruction::{error::InstructionError, AccountMeta},
    bovey_log_collector::{ic_msg, LogCollector},
    bovey_measure::measure::Measure,
    bovey_precompiles::Precompile,
    bovey_pubkey::Pubkey,
    bovey_bpf::{
        ebpf::MM_HEAP_START,
    ...
    bovey_sdk_ids::{bpf_loader_deprecated, native_loader, sysvar},
    bovey_stable_layout::stable_instruction::StableInstruction,
    bovey_timings::{ExecuteDetailsTimings, ExecuteTimings},
    bovey_transaction_context::{
        IndexOfAccount, InstructionAccount, TransactionAccount, TransactionContext,
    },
    bovey_type_overrides::sync::{atomic::Ordering, Arc},
    std::{
        alloc::Layout,
        cell::RefCell,
        fmt::{self, Debug},
...
macro_rules! declare_process_instruction {
    ($process_instruction:ident, $cu_to_consume:expr, |$invoke_context:ident| $inner:tt) => {
        $crate::bovey_bpf::declare_builtin_function!(
            $process_instruction,
            fn rust(
                ...
                _arg3: u64,
                _arg4: u64,
                _memory_mapping: &mut $crate::bovey_bpf::memory_region::MemoryMapping,
...
impl<'a> InvokeContext<'a> {
    ...
    pub fn process_precompile<'ix_data>(
        ...
        let move_precompile_verification_to_vm =
            feature_set.is_active(&move_precompile_verification_to_vm::id());
...
mod tests {
    use {
        ...
        bovey_account::WritableAccount,
        bovey_compute_budget::compute_budget_limits,
    ...
    fn test_instruction_stack_height() {
        ...
        for index in 0..one_more_than_max_depth {
            invoke_stack.push(bovey_pubkey::new_rand());
            transaction_accounts.push((
        ...
        for (index, program_id) in invoke_stack.iter().enumerate() {
            transaction_accounts.push((
                *program_id,
                AccountSharedData::new(1, 1, &bovey_pubkey::Pubkey::default()),
            ));
            instruction_accounts.push(InstructionAccount {
                index_in_transaction: index as IndexOfAccount,
                index_in_caller: index as IndexOfAccount,
    ...
    fn test_process_instruction() {
        let callee_program_id = bovey_pubkey::new_rand();
        let owned_account = AccountSharedData::new(42, 1, &callee_program_id);
        let not_owned_account = AccountSharedData::new(84, 1, &bovey_pubkey::new_rand());
        let readonly_account = AccountSharedData::new(168, 1, &bovey_pubkey::new_rand());
        ...
        let transaction_accounts = vec![
            (bovey_pubkey::new_rand(), owned_account),
            (bovey_pubkey::new_rand(), not_owned_account),
            (bovey_pubkey::new_rand(), readonly_account),
            (callee_program_id, program_account),
            (bovey_pubkey::new_rand(), loader_account),
        ];
        let metas = vec![
    ...
    fn test_invoke_context_compute_budget() {
        let transaction_accounts = vec![(bovey_pubkey::new_rand(), AccountSharedData::default())];
