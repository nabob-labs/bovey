...
macro_rules! create_vm {
    ($vm_name:ident, $verified_executable:expr, $context_object:expr, $stack:ident, $heap:ident, $additional_regions:expr, $cow_cb:expr) => {
        let mut $stack = bovey_bpf::aligned_memory::AlignedMemory::zero_filled(
            $verified_executable.get_config().stack_size(),
        );
        let mut $heap = bovey_bpf::aligned_memory::AlignedMemory::with_capacity(0);
        ...
        .unwrap();
        let mut $vm_name = bovey_bpf::vm::EbpfVm::new(
...
macro_rules! test_interpreter_and_jit {
    ...
    ($executable:expr, $mem:tt, $context_object:expr, $expected_result:expr $(,)?) => {
        ...
        if !matches!(expected_result, ProgramResult::Err(bovey_bpf::error::EbpfError::ExceededMaxInstructions)) {
