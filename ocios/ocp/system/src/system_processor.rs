...
declare_process_instruction!(Entrypoint, DEFAULT_COMPUTE_UNITS, |invoke_context| {
    ...
    let instruction =
        limited_deserialize(instruction_data, bovey_packet::PACKET_DATA_SIZE as u64)?;
...
mod tests {
    ...
    fn test_assign_native_loader_and_transfer() {
        for size in [0, 10] {
            ...
            let accounts = process_instruction(
                &bincode::serialize(&SystemInstruction::Assign {
                    owner: bovey_sdk::native_loader::id(),
                })
                .unwrap(),
                vec![(pubkey, account.clone())],
                vec![AccountMeta {
            ...
            assert_eq!(accounts[0].owner(), &bovey_sdk::native_loader::id());
            assert_eq!(accounts[0].lamports(), 100);
            ...
            assert_eq!(accounts[1].owner(), &bovey_sdk::native_loader::id());
