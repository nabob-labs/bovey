...
use {
    ...
    ocp_token_confidential_transfer_ciphertext_arithmetic as ciphertext_arithmetic,
};
use {
    crate::{
    ...
    ocp_elgamal_registry::state::ElGamalRegistry,
    ocp_pod::bytemuck::pod_from_bytes,
    ocp_token_confidential_transfer_proof_extraction::{
...
fn process_deposit(
    ...
    // Wrapped BOV deposits are not supported because lamports cannot be vanished.
...
fn process_withdraw(
    ...
    // Wrapped BOV withdrawals are not supported because lamports cannot be
    // apparated.
    assert!(!token_account.base.is_native());
