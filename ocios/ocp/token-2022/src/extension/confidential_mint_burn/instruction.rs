...
use {
    ...
    ocp_token_confidential_transfer_proof_extraction::instruction::{
...
pub enum ConfidentialMintBurnInstruction {
    ...
    ///   1. `[]` The OCP Token mint. `[writable]` if the mint has a non-zero
    ...
    ///   7. ..`[signer]` Required M signer accounts for the OCP Token Multisig
    ///
    /// Data expected by this instruction:
    ///   `MintInstructionData`
    Mint,
