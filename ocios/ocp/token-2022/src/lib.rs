...
//! An ERC20-like Token program for the Bovey blockchain
...
use {
    ...
    bovey_zk_sdk::encryption::pod::elgamal::PodElGamalCiphertext,
};
pub use {bovey_program, bovey_zk_sdk};
...
bovey_program::declare_id!("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb");

/// Checks that the supplied program ID is correct for ocp-token-2022
pub fn check_program_account(ocp_token_program_id: &Pubkey) -> ProgramResult {
    if ocp_token_program_id != &id() {
        return Err(ProgramError::IncorrectProgramId);
    }
    Ok(())
}

/// Checks that the supplied program ID is correct for ocp-token or
/// ocp-token-2022
pub fn check_ocp_token_program_account(ocp_token_program_id: &Pubkey) -> ProgramResult {
    if ocp_token_program_id != &id() && ocp_token_program_id != &ocp_token::id() {
        return Err(ProgramError::IncorrectProgramId);
    }
    Ok(())
}
...
pub fn check_zk_elgamal_proof_program_account(
    ...
    if zk_elgamal_proof_program_id != &bovey_zk_sdk::zk_elgamal_proof_program::id() {
        return Err(ProgramError::IncorrectProgramId);
    }
    Ok(())
}
...
pub(crate) fn check_elgamal_registry_program_account(
    ...
    if elgamal_registry_account_program_id != &ocp_elgamal_registry::id() {
        return Err(ProgramError::IncorrectProgramId);
    }
    Ok(())
}
