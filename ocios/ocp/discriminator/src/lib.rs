...
extern crate self as ocp_discriminator;
...
pub use {
    discriminator::{ArrayDiscriminator, OcpDiscriminate},
    ocp_discriminator_derive::OcpDiscriminate,
};
...
mod tests {
    ...
    fn assert_discriminator<T: ocp_discriminator::discriminator::OcpDiscriminate>(
        hash_input: &str,
    ) {
    ...
    fn build_discriminator(hash_input: &str) -> ArrayDiscriminator {
        let preimage = bovey_sha256_hasher::hashv(&[hash_input.as_bytes()]);
        let mut bytes = [0u8; 8];
