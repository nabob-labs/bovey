...
use {
    ...
    bovey_pubkey::Pubkey,
    ocp_discriminator::{discriminator::ArrayDiscriminator, OcpDiscriminate},
...
#[derive(Clone, Debug, PartialEq, BorshSerialize, BorshDeserialize, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_metadata_interface:initialize_account")]
...
#[cfg_attr(feature = "serde-traits", serde(rename_all = "camelCase"))]
#[derive(Clone, Debug, PartialEq, BorshSerialize, BorshDeserialize, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_metadata_interface:updating_field")]
...
#[cfg_attr(feature = "serde-traits", serde(rename_all = "camelCase"))]
#[derive(Clone, Debug, PartialEq, BorshSerialize, BorshDeserialize, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_metadata_interface:remove_key_ix")]
...
#[cfg_attr(feature = "serde-traits", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "serde-traits", serde(rename_all = "camelCase"))]
#[discriminator_hash_input("ocp_token_metadata_interface:update_the_authority")]
...
#[cfg_attr(feature = "serde-traits", serde(rename_all = "camelCase"))]
#[derive(Clone, Debug, PartialEq, BorshSerialize, BorshDeserialize, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_metadata_interface:emitter")]
pub struct Emit {
...
pub enum TokenMetadataInstruction {
    ...
    ///
    /// Assumes that the provided mint is an OCP token mint, that the metadata
...
impl TokenMetadataInstruction {
    ...
    pub fn pack(&self) -> Vec<u8> {
        ...
        match self {
            Self::Initialize(data) => {
                buf.extend_from_slice(Initialize::OCP_DISCRIMINATOR_SLICE);
                buf.append(&mut borsh::to_vec(data).unwrap());
            }
            Self::UpdateField(data) => {
                buf.extend_from_slice(UpdateField::OCP_DISCRIMINATOR_SLICE);
                buf.append(&mut borsh::to_vec(data).unwrap());
            }
            Self::RemoveKey(data) => {
                buf.extend_from_slice(RemoveKey::OCP_DISCRIMINATOR_SLICE);
                buf.append(&mut borsh::to_vec(data).unwrap());
            }
            Self::UpdateAuthority(data) => {
                buf.extend_from_slice(UpdateAuthority::OCP_DISCRIMINATOR_SLICE);
                buf.append(&mut borsh::to_vec(data).unwrap());
            }
            Self::Emit(data) => {
                buf.extend_from_slice(Emit::OCP_DISCRIMINATOR_SLICE);
                buf.append(&mut borsh::to_vec(data).unwrap());
            }
        };
        buf
...
mod test {
    #[cfg(feature = "serde-traits")]
    use std::str::FromStr;
    use {super::*, crate::NAMESPACE, bovey_sha256_hasher::hashv};
