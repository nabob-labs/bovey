...
use {
    ...
    ocp_associated_token_account_client::address::get_associated_token_address_and_bump_seed_internal,
    ocp_token_2022::{
...
fn process_create_associated_token_account(
    ...
    let ocp_token_mint_info = next_account_info(account_info_iter)?;
    let system_program_info = next_account_info(account_info_iter)?;
    let ocp_token_program_info = next_account_info(account_info_iter)?;
    let ocp_token_program_id = ocp_token_program_info.key;

    let (associated_token_address, bump_seed) = get_associated_token_address_and_bump_seed_internal(
        wallet_account_info.key,
        ocp_token_mint_info.key,
        program_id,
        ocp_token_program_id,
    );
    ...
    if create_mode == CreateMode::Idempotent
        && associated_token_account_info.owner == ocp_token_program_id
    {
        ...
        if let Ok(associated_token_account) = StateWithExtensions::<Account>::unpack(&ata_data) {
            ...
            if associated_token_account.base.mint != *ocp_token_mint_info.key {
    ...
    let associated_token_account_signer_seeds: &[&[_]] = &[
        ...
        &ocp_token_program_id.to_bytes(),
        &ocp_token_mint_info.key.to_bytes(),
        &[bump_seed],
    ];

    let account_len = get_account_len(
        ocp_token_mint_info,
        ocp_token_program_info,
    ...
    invoke(
        &ocp_token_2022::instruction::initialize_immutable_owner(
            ocp_token_program_id,
            associated_token_account_info.key,
        )?,
        &[
            associated_token_account_info.clone(),
            ocp_token_program_info.clone(),
        ],
    )?;
    invoke(
        &ocp_token_2022::instruction::initialize_account3(
            ocp_token_program_id,
            associated_token_account_info.key,
            ocp_token_mint_info.key,
            wallet_account_info.key,
...
pub fn process_recover_nested(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    if owner_token_mint_info.owner != ocp_token_program_id {
    ...
    let (amount, decimals) = {
        ...
        if owner_associated_token_account_info.owner != ocp_token_program_id {
        ...
        if nested_associated_token_account_info.owner != ocp_token_program_id {
        ...
        if nested_token_mint_info.owner != ocp_token_program_id {
    ...
    invoke_signed(
        &ocp_token_2022::instruction::transfer_checked(
            ocp_token_program_id,
    ...
    invoke_signed(
        &ocp_token_2022::instruction::close_account(
            ocp_token_program_id,
            nested_associated_token_account_info.key,
            wallet_account_info.key,
            owner_associated_token_account_info.key,
