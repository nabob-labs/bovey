...
use {
    ...
    ocp_token_2022::extension::ExtensionType,
    std::convert::TryInto,
};
...
pub fn get_account_len<'a>(
    mint: &AccountInfo<'a>,
    ocp_token_program: &AccountInfo<'a>,
    extension_types: &[ExtensionType],
) -> Result<usize, ProgramError> {
    invoke(
        &ocp_token_2022::instruction::get_account_data_size(
            ocp_token_program.key,
            mint.key,
            extension_types,
        )?,
        &[mint.clone(), ocp_token_program.clone()],
    )?;
    get_return_data()
        .ok_or(ProgramError::InvalidInstructionData)
        .and_then(|(key, data)| {
            if key != *ocp_token_program.key {
