...
#[deprecated(
    since = "1.8.0",
    note = "Please use `bovey_stake_interface::config` instead"
)]
pub use bovey_stake_interface::config::*;
use {
    bincode::deserialize,
    bovey_account::{AccountSharedData, ReadableAccount, WritableAccount},
    ocp_config_program::{create_config_account, get_config_data},
...
pub fn add_genesis_account(genesis_config: &mut GenesisConfig) -> u64 {
    ...
    genesis_config.add_account(bovey_sdk_ids::config::id(), account);
