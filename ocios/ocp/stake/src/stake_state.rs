...
#[deprecated(
    since = "1.8.0",
    note = "Please use `bovey_sdk::stake::state` or `bovey_stake_interface::state` instead"
)]
pub use bovey_stake_interface::state::*;
use {
    bovey_account::{state_traits::StateMut, AccountSharedData, ReadableAccount},
    bovey_clock::{Clock, Epoch},
    ...
    bovey_sysvar::stake_history::{StakeHistory, StakeHistoryEntry},
...
pub fn delegate(
    ...
    if *vote_account.get_owner() != bovey_sdk_ids::vote::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
...
pub(crate) fn deactivate_delinquent(
    ...
    if *delinquent_vote_account.get_owner() != bovey_sdk_ids::vote::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
    ...
    if *reference_vote_account.get_owner() != bovey_sdk_ids::vote::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
...
mod tests {
    use {
        ...
        bovey_program_runtime::with_mock_invoke_context,
        bovey_pubkey::Pubkey,
        bovey_sdk_ids::sysvar::epoch_schedule,
        bovey_stake_interface::state::warmup_cooldown_rate,
    ...
    fn test_authorized_authorize() {
        let staker = bovey_pubkey::new_rand();
    ...
    fn test_authorized_authorize_with_custodian() {
        ...
        let invalid_custodian = bovey_pubkey::new_rand();
        let mut authorized = Authorized::auto(&staker);
        let mut signers = HashSet::new();
        signers.insert(staker);
    ...
    fn test_dbg_stake_minimum_balance() {
        ...
        panic!(
            "stake minimum_balance: {} lamports, {} BOV",
            minimum_balance,
            minimum_balance as f64 / bovey_native_token::LAMPORTS_PER_BOV as f64
        );
    }
