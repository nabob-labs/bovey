...
use {
    ...
    ocp_pod::{bytemuck::pod_slice_to_bytes, slice::PodSlice},
    ocp_tlv_account_resolution::account::ExtraAccountMeta,
    std::convert::TryInto,
};
...
#[derive(OcpDiscriminate)]
#[discriminator_hash_input("ocp-transfer-hook-interface:execute")]
pub struct ExecuteInstruction;

/// TLV instruction type used to initialize extra account metas
/// for the transfer hook
#[derive(OcpDiscriminate)]
#[discriminator_hash_input("ocp-transfer-hook-interface:initialize-extra-account-metas")]
pub struct InitializeExtraAccountMetaListInstruction;

/// TLV instruction type used to update extra account metas
/// for the transfer hook
#[derive(OcpDiscriminate)]
#[discriminator_hash_input("ocp-transfer-hook-interface:update-extra-account-metas")]
...
impl TransferHookInstruction {
    ...
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        ...
        Ok(match discriminator {
            ExecuteInstruction::OCP_DISCRIMINATOR_SLICE => {
                let amount = rest
                    .get(..8)
            ...
            InitializeExtraAccountMetaListInstruction::OCP_DISCRIMINATOR_SLICE => {
            ...
            UpdateExtraAccountMetaListInstruction::OCP_DISCRIMINATOR_SLICE => {
                let pod_slice = PodSlice::<ExtraAccountMeta>::unpack(rest)?;
                let extra_account_metas = pod_slice.data().to_vec();
    ...
    pub fn pack(&self) -> Vec<u8> {
        ...
        match self {
            Self::Execute { amount } => {
                buf.extend_from_slice(ExecuteInstruction::OCP_DISCRIMINATOR_SLICE);
...
#[cfg(test)]
mod test {
    use {super::*, crate::NAMESPACE, bovey_program::hash, ocp_pod::bytemuck::pod_from_bytes};

    #[test]
    fn system_program_id() {
        assert_eq!(bovey_program::system_program::id(), SYSTEM_PROGRAM_ID);
    }
    ...
    fn validate_packing() {
        ...
        // Please use ExecuteInstruction::OCP_DISCRIMINATOR in your program, the
