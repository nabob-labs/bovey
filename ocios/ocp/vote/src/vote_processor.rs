...
use {
    ...
    bovey_pubkey::Pubkey,
    bovey_transaction_context::{BorrowedAccount, InstructionContext, TransactionContext},
    bovey_vote_interface::{instruction::VoteInstruction, program::id, state::VoteAuthorize},
...
declare_process_instruction!(Entrypoint, DEFAULT_COMPUTE_UNITS, |invoke_context| {
    ...
    match limited_deserialize(data, bovey_packet::PACKET_DATA_SIZE as u64)? {
...
mod tests {
    use {
        ...
        bovey_vote_interface::instruction::{tower_sync, tower_sync_switch},
    ...
    fn create_test_account() -> (Pubkey, AccountSharedData) {
        ...
        (
            vote_pubkey,
            vote_state::create_account(&vote_pubkey, &bovey_pubkey::new_rand(), 0, balance),
        )
    }

    fn create_test_account_with_authorized() -> (Pubkey, Pubkey, Pubkey, AccountSharedData) {
        let vote_pubkey = bovey_pubkey::new_rand();
        let authorized_voter = bovey_pubkey::new_rand();
        let authorized_withdrawer = bovey_pubkey::new_rand();
    ...
    fn test_authorize_voter() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_voter_pubkey = bovey_pubkey::new_rand();
        let clock = Clock {
            epoch: 1,
    ...
    fn test_authorize_withdrawer() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_withdrawer_pubkey = bovey_pubkey::new_rand();
        let instruction_data = serialize(&VoteInstruction::Authorize(
