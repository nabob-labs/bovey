//! Token parsing and generating library for the `ocp-discriminator` library
...
use {
    crate::{error::OcpDiscriminateError, parser::parse_hash_input},
...
/// "Builder" struct to implement the `OcpDiscriminate` trait
/// on an enum or struct
pub struct OcpDiscriminateBuilder {
...
impl TryFrom<ItemEnum> for OcpDiscriminateBuilder {
    type Error = OcpDiscriminateError;
...
impl TryFrom<ItemStruct> for OcpDiscriminateBuilder {
    type Error = OcpDiscriminateError;
...
impl Parse for OcpDiscriminateBuilder {
...
impl ToTokens for OcpDiscriminateBuilder {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        tokens.extend::<TokenStream>(self.into());
    }
}

impl From<&OcpDiscriminateBuilder> for TokenStream {
    fn from(builder: &OcpDiscriminateBuilder) -> Self {
        ...
        quote! {
            impl #generics ocp_discriminator::discriminator::OcpDiscriminate for #ident #generics #where_clause {
                const OCP_DISCRIMINATOR: ocp_discriminator::discriminator::ArrayDiscriminator
                    = ocp_discriminator::discriminator::ArrayDiscriminator::new(*#bytes);
            }
        }
    }
}
