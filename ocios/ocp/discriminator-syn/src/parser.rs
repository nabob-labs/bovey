...
use {
    crate::error::OcpDiscriminateError,
    syn::{
...
/// attribute
pub fn parse_hash_input(attrs: &[Attribute]) -> Result<String, OcpDiscriminateError> {
    match attrs
        .iter()
        .find(|a| a.path().is_ident("discriminator_hash_input"))
    {
        Some(attr) => {
            let parsed_args = attr
                ...
                .map_err(|_| OcpDiscriminateError::HashInputAttributeParseError)?;
            Ok(parsed_args.value.value())
        }
        None => Err(OcpDiscriminateError::HashInputAttributeNotProvided),
    }
}
