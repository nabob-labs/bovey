...
use {
    crate::parser::OcpProgramErrorArgs,
    proc_macro2::Span,
    quote::quote,
    sha2::{Digest, Sha256},
    syn::{
...
const OCP_ERROR_HASH_NAMESPACE: &str = "ocp_program_error";
const OCP_ERROR_HASH_MIN_VALUE: u32 = 7_000;
...
impl MacroType {
    ...
    pub fn generate_tokens(&mut self) -> proc_macro2::TokenStream {
        match self {
            ...
            Self::OcpProgramError { args, item_enum } => ocp_program_error(args, item_enum),
        }
    }
}

/// Builds the implementation of
/// `Into<bovey_program::program_error::ProgramError>` More specifically,
/// implements `From<Self> for bovey_program::program_error::ProgramError`
pub fn into_program_error(ident: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl From<#ident> for ::bovey_program::program_error::ProgramError {
            fn from(e: #ident) -> Self {
                ::bovey_program::program_error::ProgramError::Custom(e as u32)
            }
        }
    }
}

/// Builds the implementation of `bovey_program::decode_error::DecodeError<T>`
pub fn decode_error(ident: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl<T> ::bovey_program::decode_error::DecodeError<T> for #ident {
...
/// Builds the implementation of
/// `bovey_program::program_error::PrintProgramError`
pub fn print_program_error(
    ...
    let ppe_match_arms = variants.iter().map(|variant| {
        ...
        quote! {
            #ident::#variant_ident => {
                ::bovey_program::msg!(#error_msg)
            }
        }
    });
    quote! {
        impl ::bovey_program::program_error::PrintProgramError for #ident {
            fn print<E>(&self)
            where
                E: 'static
                    + std::error::Error
                    + ::bovey_program::decode_error::DecodeError<E>
                    + ::bovey_program::program_error::PrintProgramError
...
/// error enum into a Bovey Program Error
pub fn ocp_program_error(
    args: &OcpProgramErrorArgs,
...
/// hash of the `OCP_ERROR_HASH_NAMESPACE` constant, the enum name and variant
/// name.
    ...
    if discriminant == error_code_start {
        ...
        panic!(
            "Error code start value from hash must be {0}. Update your macro attribute to \
             `#[ocp_program_error(hash_error_code_start = {0})]`.",
            discriminant
        );
    }
}

/// Hashes the `OCP_ERROR_HASH_NAMESPACE` constant, the enum name and variant
/// name and returns four middle bytes (13 through 16) as a u32.
fn u32_from_hash(enum_ident: &Ident) -> u32 {
    let hash_input = format!("{}:{}", OCP_ERROR_HASH_NAMESPACE, enum_ident);

    // We don't want our error code to start at any number below
    // `OCP_ERROR_HASH_MIN_VALUE`!
    let mut nonce: u32 = 0;
    loop {
