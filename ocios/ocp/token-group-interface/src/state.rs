...
use {
    ...
    ocp_pod::{error::PodSliceError, optional_keys::OptionalNonZeroPubkey, primitives::PodU64},
};

/// Data struct for a `TokenGroup`
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, PartialEq, Pod, Zeroable, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_group_interface:group")]
...
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, PartialEq, Pod, Zeroable, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_group_interface:member")]
...
mod tests {
    use {
        ...
        ocp_discriminator::ArrayDiscriminator,
    ...
    fn discriminators() {
        ...
        assert_eq!(TokenGroup::OCP_DISCRIMINATOR, discriminator);

        let preimage = hashv(&[format!("{NAMESPACE}:member").as_bytes()]);
        let discriminator =
            ArrayDiscriminator::try_from(&preimage.as_ref()[..ArrayDiscriminator::LENGTH]).unwrap();
        assert_eq!(TokenGroupMember::OCP_DISCRIMINATOR, discriminator);
    }
