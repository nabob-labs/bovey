...
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Pod, Zeroable, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_group_interface:initialize_token_group")]
...
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Pod, Zeroable, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_group_interface:update_group_max_size")]
...
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Pod, Zeroable, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_group_interface:update_authority")]
...
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Pod, Zeroable, OcpDiscriminate)]
#[discriminator_hash_input("ocp_token_group_interface:initialize_member")]
pub struct InitializeMember;

/// All instructions that must be implemented in the OCP Token Group Interface
...
impl TokenGroupInstruction {
    ...
    pub fn pack(&self) -> Vec<u8> {
        ...
        match self {
            Self::InitializeGroup(data) => {
                buf.extend_from_slice(InitializeGroup::OCP_DISCRIMINATOR_SLICE);
                buf.extend_from_slice(pod_bytes_of(data));
            }
            Self::UpdateGroupMaxSize(data) => {
                buf.extend_from_slice(UpdateGroupMaxSize::OCP_DISCRIMINATOR_SLICE);
                buf.extend_from_slice(pod_bytes_of(data));
            }
            Self::UpdateGroupAuthority(data) => {
                buf.extend_from_slice(UpdateGroupAuthority::OCP_DISCRIMINATOR_SLICE);
                buf.extend_from_slice(pod_bytes_of(data));
            }
            Self::InitializeMember(data) => {
                buf.extend_from_slice(InitializeMember::OCP_DISCRIMINATOR_SLICE);
                buf.extend_from_slice(pod_bytes_of(data));
            }
        };
        buf
...
mod test {
    ...
    where
        I: core::fmt::Debug + PartialEq + Pod + Zeroable + OcpDiscriminate,
    {
        let mut expect = vec![];
