...
use {
    ...
    bovey_big_mod_exp::{big_mod_exp, BigModExpParams},
    bovey_blake3_hasher as blake3,
    bovey_bn254::prelude::{
    ...
    bovey_compute_budget::compute_budget::ComputeBudget,
    bovey_cpi::MAX_RETURN_DATA,
    bovey_feature_set::{
    ...
    bovey_hash::Hash,
    bovey_instruction::{error::InstructionError, AccountMeta, ProcessedSiblingInstruction},
    bovey_keccak_hasher as keccak,
    bovey_log_collector::{ic_logger_msg, ic_msg},
    bovey_poseidon as poseidon,
    bovey_precompiles::is_precompile,
    bovey_program_entrypoint::{BPF_ALIGN_OF_U128, MAX_PERMITTED_DATA_INCREASE, SUCCESS},
    bovey_program_memory::is_nonoverlapping,
    bovey_program_runtime::{invoke_context::InvokeContext, stable_log},
    bovey_pubkey::{Pubkey, PubkeyError, MAX_SEEDS, MAX_SEED_LEN, PUBKEY_BYTES},
    bovey_bpf::{
    ...
    bovey_sdk_ids::{bpf_loader, bpf_loader_deprecated, native_loader},
    bovey_secp256k1_recover::{
        Secp256k1RecoverError, SECP256K1_PUBLIC_KEY_LENGTH, SECP256K1_SIGNATURE_LENGTH,
    },
    bovey_sha256_hasher::Hasher,
    bovey_sysvar::Sysvar,
    bovey_sysvar_id::SysvarId,
    bovey_timings::ExecuteTimings,
    bovey_transaction_context::{IndexOfAccount, InstructionAccount},
    bovey_type_overrides::sync::Arc,
    std::{
        alloc::Layout,
        marker::PhantomData,
...
pub(crate) fn morph_into_deployment_environment_v1(
    ...
    for (_key, (name, value)) in from.get_function_registry().iter() {
        // Deployment of programs with bov_alloc_free is disabled. So do not register the syscall.
        if name != *b"bov_alloc_free_" {
...
pub fn create_program_runtime_environment_v1<'a>(
    ...
    // Panic
    result.register_function("bov_panic_", SyscallPanic::vm)?;

    // Logging
    result.register_function("bov_log_", SyscallLog::vm)?;
    result.register_function("bov_log_64_", SyscallLogU64::vm)?;
    result.register_function("bov_log_pubkey", SyscallLogPubkey::vm)?;
    result.register_function("bov_log_compute_units_", SyscallLogBpfComputeUnits::vm)?;

    // Program defined addresses (PDA)
    result.register_function(
        "bov_create_program_address",
        SyscallCreateProgramAddress::vm,
    )?;
    result.register_function(
        "bov_try_find_program_address",
        SyscallTryFindProgramAddress::vm,
    )?;

    // Sha256
    result.register_function("bov_sha256", SyscallHash::vm::<Sha256Hasher>)?;

    // Keccak256
    result.register_function("bov_keccak256", SyscallHash::vm::<Keccak256Hasher>)?;

    // Secp256k1 Recover
    result.register_function("bov_secp256k1_recover", SyscallSecp256k1Recover::vm)?;
    ...
    register_feature_gated_function!(
        result,
        curve25519_syscall_enabled,
        "bov_curve_multiscalar_mul",
        SyscallCurveMultiscalarMultiplication::vm,
    )?;

    // Sysvars
    result.register_function("bov_get_clock_sysvar", SyscallGetClockSysvar::vm)?;
    result.register_function(
        "bov_get_epoch_schedule_sysvar",
    ...
    result.register_function("bov_get_rent_sysvar", SyscallGetRentSysvar::vm)?;

    register_feature_gated_function!(
        result,
        last_restart_slot_syscall_enabled,
        "bov_get_last_restart_slot",
        SyscallGetLastRestartSlotSysvar::vm,
    )?;

    result.register_function(
        "bov_get_epoch_rewards_sysvar",
        SyscallGetEpochRewardsSysvar::vm,
    )?;

    // Memory ops
    result.register_function("bov_memcpy_", SyscallMemcpy::vm)?;
    result.register_function("bov_memmove_", SyscallMemmove::vm)?;
    result.register_function("bov_memset_", SyscallMemset::vm)?;
    result.register_function("bov_memcmp_", SyscallMemcmp::vm)?;

    // Processed sibling instructions
    result.register_function(
        "bov_get_processed_sibling_instruction",
        SyscallGetProcessedSiblingInstruction::vm,
    )?;

    // Stack height
    result.register_function("bov_get_stack_height", SyscallGetStackHeight::vm)?;

    // Return data
    result.register_function("bov_set_return_data", SyscallSetReturnData::vm)?;
    result.register_function("bov_get_return_data", SyscallGetReturnData::vm)?;

    // Cross-program invocation
    result.register_function("bov_invoke_signed_c", SyscallInvokeSignedC::vm)?;
    result.register_function("bov_invoke_signed_rust", SyscallInvokeSignedRust::vm)?;
    ...
    // Log data
    result.register_function("bov_log_data", SyscallLogData::vm)?;
...
declare_builtin_function!(
    /// Panic syscall function, called when the SBF program calls 'bov_panic_()`
    /// Causes the SBF program to be halted immediately
    SyscallPanic,
    fn rust(
...
declare_builtin_function!(
    ...
    /// `bov_alloc_free_()`.  The allocator is expected to allocate/free
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use bovey_curve25519::{curve_syscall_traits::*, edwards, ristretto};
        match curve_id {
            CURVE25519_EDWARDS => {
                let cost = invoke_context
                    .get_compute_budget()
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use bovey_curve25519::{curve_syscall_traits::*, edwards, ristretto, scalar};
        match curve_id {
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use bovey_bn254::prelude::{ALT_BN128_ADD, ALT_BN128_MUL, ALT_BN128_PAIRING};
        let budget = invoke_context.get_compute_budget();
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use bovey_bpf::vm::ContextObject;
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use bovey_bn254::compression::prelude::{
...
mod tests {
    #[allow(deprecated)]
    use bovey_sysvar::fees::Fees;
    use {
        super::*,
        crate::mock_create_vm,
        assert_matches::assert_matches,
        core::slice,
        bovey_account::{create_account_shared_data_for_test, AccountSharedData},
        bovey_clock::Clock,
        bovey_epoch_rewards::EpochRewards,
        bovey_epoch_schedule::EpochSchedule,
        bovey_fee_calculator::FeeCalculator,
        bovey_hash::HASH_BYTES,
        bovey_instruction::Instruction,
        bovey_last_restart_slot::LastRestartSlot,
        bovey_program::program::check_type_assumptions,
        bovey_program_runtime::{invoke_context::InvokeContext, with_mock_invoke_context},
        bovey_bpf::{
            error::EbpfError, memory_region::MemoryRegion, program::SBPFVersion, vm::Config,
        },
        bovey_sdk_ids::{bpf_loader, bpf_loader_upgradeable, sysvar},
        bovey_sha256_hasher::hashv,
        bovey_slot_hashes::{self as slot_hashes, SlotHashes},
        bovey_stable_layout::stable_instruction::StableInstruction,
        bovey_sysvar::stake_history::{self, StakeHistory, StakeHistoryEntry},
        std::{mem, str::FromStr},
        test_case::test_case,
    };
    ...
    fn test_translate_type() {
        ...
        let instruction = Instruction::new_with_bincode(
            ...
            &"foobar",
            vec![AccountMeta::new(bovey_pubkey::new_rand(), false)],
        );
    ...
    fn test_translate_slice() {
        ...
        *data.first_mut().unwrap() = 10;
        ...
        // Pubkeys
        let mut data = vec![bovey_pubkey::new_rand(); 5];
        let memory_mapping = MemoryMapping::new(
            vec![MemoryRegion::new_readonly(
                unsafe {
        ...
        let translated_data =
            translate_slice::<Pubkey>(&memory_mapping, 0x100000000, data.len() as u64, true)
                .unwrap();
        assert_eq!(data, translated_data);
        *data.first_mut().unwrap() = bovey_pubkey::new_rand(); // Both should point to same place
        assert_eq!(data, translated_data);
    }
    ...
    #[test]
    fn test_syscall_bov_alloc_free() {
        // large alloc
        {
            prepare_mockup!(invoke_context, program_id, bpf_loader::id());
            ...
            let result = SyscallAllocFree::rust(
                invoke_context,
                bovey_program_entrypoint::HEAP_LENGTH as u64,
                0,
                0,
                0,
                0,
    ...
    fn test_syscall_edwards_curve_point_validation() {
        use bovey_curve25519::curve_syscall_traits::CURVE25519_EDWARDS;
    ...
    fn test_syscall_ristretto_curve_point_validation() {
        use bovey_curve25519::curve_syscall_traits::CURVE25519_RISTRETTO;
    ...
    fn test_syscall_edwards_curve_group_ops() {
        use bovey_curve25519::curve_syscall_traits::{ADD, CURVE25519_EDWARDS, MUL, SUB};
    ...
    fn test_syscall_ristretto_curve_group_ops() {
        use bovey_curve25519::curve_syscall_traits::{ADD, CURVE25519_RISTRETTO, MUL, SUB};
    ...
    fn test_syscall_multiscalar_multiplication() {
        use bovey_curve25519::curve_syscall_traits::{CURVE25519_EDWARDS, CURVE25519_RISTRETTO};
    ...
    fn test_syscall_get_sysvar() {
        ...
        // Test fees sysvar
        {
            ...
            // fees sysvar is not accessible via bov_get_sysvar so nothing further to test
        }
    ...
    #[test]
    fn test_syscall_bov_get_processed_sibling_instruction() {
    ...
    fn test_create_program_address() {
        // These tests duplicate the direct tests in bovey_program::pubkey
