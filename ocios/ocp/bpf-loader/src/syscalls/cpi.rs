use {
    ...
    scopeguard::defer,
    bovey_feature_set::{self as feature_set, enable_bpf_loader_set_authority_checked_ix},
    bovey_loader_v3_interface::instruction as bpf_loader_upgradeable,
    bovey_measure::measure::Measure,
    bovey_program_runtime::invoke_context::SerializedAccountMetadata,
    bovey_bpf::{
        ebpf,
        memory_region::{MemoryRegion, MemoryState},
    },
    bovey_stable_layout::stable_instruction::StableInstruction,
    bovey_transaction_context::BorrowedAccount,
    std::{mem, ptr},
};
// consts inlined to avoid bovey-program dep
const MAX_CPI_INSTRUCTION_DATA_LEN: u64 = 10 * 1024;
#[cfg(test)]
static_assertions::const_assert_eq!(
    MAX_CPI_INSTRUCTION_DATA_LEN,
    bovey_program::syscalls::MAX_CPI_INSTRUCTION_DATA_LEN
);
const MAX_CPI_INSTRUCTION_ACCOUNTS: u8 = u8::MAX;
#[cfg(test)]
static_assertions::const_assert_eq!(
    MAX_CPI_INSTRUCTION_ACCOUNTS,
    bovey_program::syscalls::MAX_CPI_INSTRUCTION_ACCOUNTS
);
const MAX_CPI_ACCOUNT_INFOS: usize = 128;
#[cfg(test)]
static_assertions::const_assert_eq!(
    MAX_CPI_ACCOUNT_INFOS,
    bovey_program::syscalls::MAX_CPI_ACCOUNT_INFOS
);
...
/// Host side representation of AccountInfo or BovAccountInfo passed to the CPI syscall.
///
...
impl<'a, 'b> CallerAccount<'a, 'b> {
    ...
    // Create a CallerAccount given a BovAccountInfo.
    fn from_bov_account_info(
        invoke_context: &InvokeContext,
        memory_mapping: &'b MemoryMapping<'a>,
        vm_addr: u64,
        account_info: &BovAccountInfo,
        account_metadata: &SerializedAccountMetadata,
    ) -> Result<CallerAccount<'a, 'b>, Error> {
        let direct_mapping = invoke_context
            .get_feature_set()
        ...
        // which puts BovAccountInfo in rodata.
...
/// Rust representation of C's BovInstruction
#[derive(Debug)]
#[repr(C)]
struct BovInstruction {
    program_id_addr: u64,
    accounts_addr: u64,
    accounts_len: u64,
    data_addr: u64,
...
/// Rust representation of C's BovAccountMeta
#[derive(Debug)]
#[repr(C)]
struct BovAccountMeta {
    pubkey_addr: u64,
    is_writable: bool,
    is_signer: bool,
}

/// Rust representation of C's BovAccountInfo
#[derive(Debug)]
#[repr(C)]
struct BovAccountInfo {
    key_addr: u64,
    lamports_addr: u64,
    data_len: u64,
    data_addr: u64,
...
/// Rust representation of C's BovSignerSeed
#[derive(Debug)]
#[repr(C)]
struct BovSignerSeedC {
    addr: u64,
    len: u64,
}

/// Rust representation of C's BovSignerSeeds
#[derive(Debug)]
#[repr(C)]
struct BovSignerSeedsC {
    addr: u64,
    len: u64,
}
...
impl SyscallInvokeSigned for SyscallInvokeSignedC {
    fn translate_instruction(
        ...
        let ix_c = translate_type::<BovInstruction>(
            memory_mapping,
            addr,
            invoke_context.get_check_aligned(),
        )?;
        ...
        let account_metas = translate_slice::<BovAccountMeta>(
            memory_mapping,
            ix_c.accounts_addr,
            ix_c.accounts_len,
            invoke_context.get_check_aligned(),
    ...
    fn translate_accounts<'a, 'b>(
        ...
        let (account_infos, account_info_keys) = translate_account_infos(
            account_infos_addr,
            account_infos_len,
            |account_info: &BovAccountInfo| account_info.key_addr,
            memory_mapping,
            invoke_context,
        )?;
        ...
        translate_and_update_accounts(
            ...
            invoke_context,
            memory_mapping,
            CallerAccount::from_bov_account_info,
        )
    }
    ...
    fn translate_signers(
        ...
        if signers_seeds_len > 0 {
            let signers_seeds = translate_slice::<BovSignerSeedsC>(
                memory_mapping,
                signers_seeds_addr,
                signers_seeds_len,
                invoke_context.get_check_aligned(),
            ...
            Ok(signers_seeds
                .iter()
                .map(|signer_seeds| {
                    let seeds = translate_slice::<BovSignerSeedC>(
                        memory_mapping,
                        signer_seeds.addr,
                        signer_seeds.len,
                        invoke_context.get_check_aligned(),
...
fn check_authorized_program(
    ...
    if native_loader::check_id(program_id)
        ...
        || (bovey_sdk_ids::bpf_loader_upgradeable::check_id(program_id)
...
fn account_data_region<'a>(
    ...
    // enforce that in CallerAccount::from_(bov_)account_info.
...
mod tests {
    use {
        ...
        bovey_account::{Account, AccountSharedData, ReadableAccount},
        bovey_clock::Epoch,
        bovey_feature_set::bpf_account_data_direct_mapping,
        ...
        bovey_transaction_context::TransactionAccount,
        std::{
            cell::{Cell, RefCell},
            mem, ptr,
            rc::Rc,
