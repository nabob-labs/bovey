...
use {
    ...
    bovey_clock::Slot,
    bovey_compute_budget::compute_budget::MAX_INSTRUCTION_STACK_DEPTH,
    bovey_feature_set::{
    ...
    bovey_loader_v3_interface::{
        instruction::UpgradeableLoaderInstruction, state::UpgradeableLoaderState,
    },
    bovey_log_collector::{ic_logger_msg, ic_msg, LogCollector},
    bovey_measure::measure::Measure,
    bovey_program_entrypoint::{MAX_PERMITTED_DATA_INCREASE, SUCCESS},
    ...
    bovey_system_interface::{instruction as system_instruction, MAX_PERMITTED_DATA_LENGTH},
    ...
    ($invoke_context:expr, $program_id:expr, $loader_key:expr, $account_size:expr, $programdata:expr, $deployment_slot:expr $(,)?) => {
        let environments = $invoke_context
            .get_environments_for_slot($deployment_slot.saturating_add(
                bovey_program_runtime::loaded_programs::DELAY_VISIBILITY_SLOT_OFFSET,
            ))
            .map_err(|_err| {
...
macro_rules! mock_create_vm {
    ($vm:ident, $additional_regions:expr, $accounts_metadata:expr, $invoke_context:expr $(,)?) => {
        let loader = bovey_type_overrides::sync::Arc::new(BuiltinProgram::new_mock());
        let function_registry = bovey_bpf::program::FunctionRegistry::default();
        let executable = bovey_bpf::elf::Executable::<InvokeContext>::from_text_bytes(
            &[0x9D, 0, 0, 0, 0, 0, 0, 0],
            loader,
            SBPFVersion::V3,
            function_registry,
        ...
        .unwrap();
        executable
            .verify::<bovey_bpf::verifier::RequisiteVerifier>()
            .unwrap();
        $crate::create_vm!(
            $vm,
            &executable,
...
mod migration_authority {
    bovey_pubkey::declare_id!("3Scf35jMNk2xXBD6areNjgMtXgp5ZspDhms8vdcbzC42");
}
...
fn process_loader_upgradeable_instruction(
    ...
    match limited_deserialize(instruction_data, bovey_packet::PACKET_DATA_SIZE as u64)? {
        ...
        UpgradeableLoaderInstruction::DeployWithMaxDataLen { max_data_len } => {
            ...
            let signers = [[new_program_id.as_ref(), &[bump_seed]]]
                ...
                .collect::<Result<Vec<Pubkey>, bovey_pubkey::PubkeyError>>()?;
        ...
        UpgradeableLoaderInstruction::Migrate => {
            ...
            if program_len > 0 {
                invoke_context.native_invoke(
                    bovey_loader_v4_interface::instruction::set_program_length(
                        &program_address,
                        &provided_authority_address,
                        program_len as u32,
                        &program_address,
                ...
                invoke_context.native_invoke(
                    bovey_loader_v4_interface::instruction::copy(
                        &program_address,
                        &provided_authority_address,
                        &programdata_address,
                        0,
                ...
                invoke_context.native_invoke(
                    bovey_loader_v4_interface::instruction::deploy(
                        &program_address,
                        &provided_authority_address,
                    )
                    .into(),
                ...
                if upgrade_authority_address.is_none() {
                    invoke_context.native_invoke(
                        bovey_loader_v4_interface::instruction::finalize(
                            &program_address,
                            &provided_authority_address,
                            &program_address,
                        )
                    ...
                    invoke_context.native_invoke(
                        bovey_loader_v4_interface::instruction::transfer_authority(
                            &program_address,
                            &provided_authority_address,
    ...
    let execution_result = {
        ...
        match result {
            ...
            ProgramResult::Err(mut error) => {
                if invoke_context
                    .get_feature_set()
                    .is_active(&bovey_feature_set::deplete_cu_meter_on_vm_failure::id())
...
mod test_utils {
    ...
    use {
        ...
        bovey_account::ReadableAccount, bovey_loader_v4_interface::state::LoaderV4State,
        bovey_program_runtime::loaded_programs::DELAY_VISIBILITY_SLOT_OFFSET,
        bovey_sdk_ids::loader_v4,
    };
