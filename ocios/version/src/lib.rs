...
impl Default for Version {
    fn default() -> Self {
        let feature_set =
            u32::from_le_bytes(bovey_feature_set::ID.as_ref()[..4].try_into().unwrap());
        Self {
            ...
            client: u16::try_from(ClientId::Bovey).unwrap(),
        }
    }
}
...
mod test {
    ...
    #[test]
    fn test_client_id() {
        assert_eq!(ClientId::from(0u16), ClientId::Bovey);
        for client in 4u16..=u16::MAX {
            assert_eq!(ClientId::from(client), ClientId::Unknown(client));
        }
        assert_eq!(u16::try_from(ClientId::Bovey), Ok(0u16));
