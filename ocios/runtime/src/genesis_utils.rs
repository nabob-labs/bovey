...
pub fn create_genesis_config(mint_lamports: u64) -> GenesisConfigInfo {
    ...
    create_genesis_config_with_leader(
        mint_lamports,
        &bovey_pubkey::new_rand(), // validator_pubkey
...
pub fn create_genesis_config_with_leader_ex_no_features(
    ...
    let native_mint_account = bovey_sdk::account::AccountSharedData::from(Account {
        owner: bovey_inline_ocp::token::id(),
        data: bovey_inline_ocp::token::native_mint::ACCOUNT_DATA.to_vec(),
        lamports: bov_to_lamports(1.),
        executable: false,
        rent_epoch: 1,
    });
    initial_accounts.push((
        bovey_inline_ocp::token::native_mint::id(),
        native_mint_account,
    ));
