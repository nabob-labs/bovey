...
use {
    ...
    bovey_vote::vote_account::VoteAccounts,
    std::sync::Arc,
};
...
impl Bank {
    ...
    pub(super) fn get_reward_distribution_num_blocks(
        ...
        if self.epoch_schedule.warmup && self.epoch < self.first_normal_epoch() {
            ...
            let num_chunks = bovey_accounts_db::accounts_hash::AccountsHasher::div_ceil(
...
mod tests {
    use {
        ...
        ocp_vote_program::vote_state::{self, TowerSync},
    };
    ...
    fn test_force_reward_interval_end() {
        let (genesis_config, _mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_BOV);
    ...
    fn test_get_reward_distribution_num_blocks_cap() {
        let (mut genesis_config, _mint_keypair) =
            create_genesis_config(1_000_000 * LAMPORTS_PER_BOV);
        ...
        for slot in starting_slot..=(2 * SLOTS_PER_EPOCH) + 2 {
            ...
            if slot % SLOTS_PER_EPOCH == 0 {
                ...
                let account = curr_bank
                    .get_account(&bovey_sdk::sysvar::epoch_rewards::id())
                    .unwrap();
                let epoch_rewards: bovey_sdk::sysvar::epoch_rewards::EpochRewards =
                    bovey_sdk::account::from_account(&account).unwrap();
            ...
            if slot >= SLOTS_PER_EPOCH {
                let epoch_rewards_lamports =
                    curr_bank.get_balance(&bovey_sdk::sysvar::epoch_rewards::id());
    ...
    fn test_rewards_computation_and_partitioned_distribution_two_blocks() {
        ...
        for slot in starting_slot..=SLOTS_PER_EPOCH + 3 {
            ...
            let pre_sysvar_account = previous_bank
                .get_account(&bovey_sdk::sysvar::epoch_rewards::id())
                .unwrap_or_default();
            let pre_epoch_rewards: bovey_sdk::sysvar::epoch_rewards::EpochRewards =
                bovey_sdk::account::from_account(&pre_sysvar_account).unwrap_or_default();
    ...
    fn test_program_execution_restricted_for_stake_account_in_reward_period() {
        use bovey_sdk::transaction::TransactionError::InstructionError;
        ...
        let new_stake_account = Account::from(ocp_stake_program::stake_state::create_account(
            &new_stake_address,
            &vote_key,
            &vote_account.into(),
            &genesis_config.rent,
        ...
        for slot in 1..=num_slots_in_epoch + 2 {
            ...
            let stake_ix = bovey_sdk::stake::instruction::withdraw(
                &new_stake_address,
                &new_stake_address,
                &mint_keypair.pubkey(),
                transfer_amount,
