...
use {
    ...
    bovey_transaction_view::static_account_keys_frame::MAX_STATIC_ACCOUNTS_PER_PACKET,
    ...
    bovey_compute_budget::{
        compute_budget::ComputeBudget,
        compute_budget_limits::{self, ComputeBudgetLimits, MAX_COMPUTE_UNIT_LIMIT},
    },
    bovey_cost_model::block_cost_limits::{MAX_BLOCK_UNITS, MAX_BLOCK_UNITS_SIMD_0207},
    bovey_feature_set::{self as feature_set, FeatureSet},
    bovey_inline_ocp::token,
    bovey_logger,
    bovey_program_runtime::{
    ...
    bovey_sdk::{
        ...
        native_loader,
        native_token::{bov_to_lamports, LAMPORTS_PER_BOV},
    ...
    ocp_stake_program::stake_state::{self, StakeStateV2},
    bovey_vm::{
...
impl VoteReward {
    pub fn new_random() -> Self {
        ...
        let validator_pubkey = bovey_pubkey::new_rand();
...
fn create_genesis_config_no_tx_fee_no_rent(lamports: u64) -> (GenesisConfig, Keypair) {
    ...
    let genesis_config_info = bovey_runtime::genesis_utils::create_genesis_config(lamports);
    (
...
fn create_genesis_config_no_tx_fee(lamports: u64) -> (GenesisConfig, Keypair) {
    ...
    let (mut genesis_config, mint_keypair) =
        bovey_sdk::genesis_config::create_genesis_config(lamports);
...
#[test]
fn test_bank_new() {
    let dummy_leader_pubkey = bovey_pubkey::new_rand();
...
fn test_rent_distribution() {
    ...
    let bootstrap_validator_pubkey = bovey_pubkey::new_rand();
    ...
    let validator_1_pubkey = bovey_pubkey::new_rand();
    ...
    let validator_2_pubkey = bovey_pubkey::new_rand();
    ...
    let validator_3_pubkey = bovey_pubkey::new_rand();
...
fn test_rent_exempt_executable_account() {
    ...
    let account_pubkey = bovey_pubkey::new_rand();
    let account_balance = 1;
    let mut account = AccountSharedData::new(account_balance, 0, &bovey_pubkey::new_rand());
...
fn test_rent_eager_across_epoch_without_gap_under_multi_epoch_cycle() {
    let leader_pubkey = bovey_pubkey::new_rand();
...
fn test_rent_eager_collect_rent_in_partition(should_collect_rent: bool) {
    ...
    for feature_id in FeatureSet::default().inactive {
        if feature_id != bovey_feature_set::skip_rent_rewrites::id()
            && (!should_collect_rent
                || feature_id != bovey_feature_set::disable_rent_fees_collection::id())
        {
            activate_feature(&mut genesis_config, feature_id);
        }
    }

    let zero_lamport_pubkey = bovey_pubkey::new_rand();
    let rent_due_pubkey = bovey_pubkey::new_rand();
    let rent_exempt_pubkey = bovey_pubkey::new_rand();
    let mut bank = Arc::new(Bank::new_for_tests(&genesis_config));
...
impl Bank {
    fn _load_vote_and_stake_accounts(
        ...
        let ocp_vote_program: Pubkey = ocp_vote_program::id();
        ...
        let get_vote_account = |vote_pubkey: &Pubkey| -> Option<VoteAccount> {
            ...
            if account.owner() == &ocp_vote_program
        ...
        let make_vote_delegations_entry = |vote_pubkey| {
            ...
            if vote_account.owner() != &ocp_vote_program {
        ...
        let push_stake_delegation = |(stake_pubkey, stake_account): (&Pubkey, &StakeAccount<_>)| {
            ...
            if let Some(reward_calc_tracer) = reward_calc_tracer.as_ref() {
                let delegation =
                    InflationPointCalculationEvent::Delegation(*delegation, ocp_vote_program);
...
fn do_test_bank_update_rewards_determinism() -> u64 {
    ...
    let vote_id = bovey_pubkey::new_rand();
    let mut vote_account = vote_state::create_account(&vote_id, &bovey_pubkey::new_rand(), 0, 100);
    let stake_id1 = bovey_pubkey::new_rand();
    let stake_account1 = crate::stakes::tests::create_stake_account(123, &vote_id, &stake_id1);
    let stake_id2 = bovey_pubkey::new_rand();
...
fn test_purge_empty_accounts() {
    ...
    for pass in 0..3 {
        bovey_logger::setup();
        let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(bov_to_lamports(1.));
...
fn test_two_payments_to_one_party() {
    let (genesis_config, mint_keypair) = create_genesis_config(bov_to_lamports(1.));
    let pubkey = bovey_pubkey::new_rand();
...
fn test_one_source_two_tx_one_batch() {
    let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(bov_to_lamports(1.));
    let key1 = bovey_pubkey::new_rand();
    let key2 = bovey_pubkey::new_rand();
    let (bank, _bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let amount = genesis_config.rent.minimum_balance(0);
    assert_eq!(bank.last_blockhash(), genesis_config.hash());
...
fn test_insufficient_funds() {
    let mint_amount = bov_to_lamports(1.);
...
fn test_transfer_to_sysvar() {
    ...
    let normal_pubkey = bovey_pubkey::new_rand();
...
fn test_bank_tx_fee() {
    ...
    let leader = bovey_pubkey::new_rand();
    let GenesisConfigInfo {
        mut genesis_config,
        mint_keypair,
        ..
...
fn test_debits_before_credits() {
    let (genesis_config, mint_keypair) =
        create_genesis_config_no_tx_fee_no_rent(bov_to_lamports(2.));
    let (bank, _bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let keypair = Keypair::new();
...
fn test_readonly_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &bovey_pubkey::new_rand(), 0);
    let (bank, _bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);

    let vote_pubkey0 = bovey_pubkey::new_rand();
    let vote_pubkey1 = bovey_pubkey::new_rand();
    let vote_pubkey2 = bovey_pubkey::new_rand();
...
fn test_load_and_execute_commit_transactions_fees_only(enable_fees_only_txs: bool) {
    let GenesisConfigInfo {
        mut genesis_config, ..
    } = genesis_utils::create_genesis_config(100 * LAMPORTS_PER_BOV);
    if !enable_fees_only_txs {
        genesis_config
            .accounts
            .remove(&bovey_sdk::feature_set::enable_transaction_loading_failure_fees::id());
    }
...
fn test_readonly_relaxed_locks() {
    ...
    let key3 = bovey_pubkey::new_rand();
...
fn test_bank_parent_account_spend() {
    let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(bov_to_lamports(1.0));
    let key1 = Keypair::new();
    let key2 = Keypair::new();
    let (parent, bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let amount = genesis_config.rent.minimum_balance(0);
...
fn test_bank_hash_internal_state(is_accounts_lt_hash_enabled: bool) {
    let (mut genesis_config, mint_keypair) =
        create_genesis_config_no_tx_fee_no_rent(bov_to_lamports(1.));
    ...
    let pubkey2 = bovey_pubkey::new_rand();
...
fn test_hash_internal_state_order() {
    ...
    assert_eq!(bank0.hash_internal_state(), bank1.hash_internal_state());
    let key0 = bovey_pubkey::new_rand();
    let key1 = bovey_pubkey::new_rand();
...
fn test_hash_internal_state_error() {
    ...
    assert!(bank
        .transfer(bov_to_lamports(1.), &mint_keypair, &key0)
        .is_err());
...
fn test_bank_squash() {
    bovey_logger::setup();
    let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(bov_to_lamports(2.));
    let key1 = Keypair::new();
    let key2 = Keypair::new();
    let (parent, bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let amount = genesis_config.rent.minimum_balance(0);
...
fn test_is_delta_true() {
    let (genesis_config, mint_keypair) = create_genesis_config(bov_to_lamports(1.0));
    let (bank, _bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let key1 = Keypair::new();
...
fn test_bank_vote_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &bovey_pubkey::new_rand(), 1);
    let (bank, _bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
...
fn test_bank_cloned_stake_delegations() {
    ...
    let (vote_balance, stake_balance) = {
        ...
        let minimum_delegation = ocp_stake_program::get_minimum_delegation(&bank.feature_set);
        (
...
fn test_is_delta_with_no_committables() {
    ...
    assert_eq!(
        bank.transfer(10_001, &mint_keypair, &bovey_pubkey::new_rand()),
        Err(TransactionError::InstructionError(
            0,
            SystemError::ResultWithNegativeLamports.into(),
        ))
...
fn test_bank_get_program_accounts() {
    ...
    assert!(
        genesis_accounts
            .iter()
            .any(|(_, account, _)| bovey_sdk::sysvar::check_id(account.owner())),
        "no sysvars found"
    );

    let bank0 = Arc::new(new_from_parent(parent));
    let pubkey0 = bovey_pubkey::new_rand();
    ...
    let pubkey1 = bovey_pubkey::new_rand();
...
fn test_add_duplicate_static_program() {
    ...
    bank.add_mockup_builtin(ocp_vote_program::id(), MockBuiltin::vm);
    let bank = bank_forks
        .write()
        .unwrap()
        .insert(bank)
    ...
    let vote_loader_account = bank.get_account(&ocp_vote_program::id()).unwrap();
    let new_vote_loader_account = bank.get_account(&ocp_vote_program::id()).unwrap();
...
fn test_nonce_fee_calculator_updates() {
    ...
    let nonce_tx = Transaction::new_signed_with_payer(
        &[
            system_instruction::advance_nonce_account(&nonce_pubkey, &nonce_pubkey),
            system_instruction::transfer(&custodian_pubkey, &bovey_pubkey::new_rand(), 100_000),
        ],
        Some(&custodian_pubkey),
        &[&custodian_keypair, &nonce_keypair],
        stored_nonce_hash,
...
fn test_transaction_with_duplicate_accounts_in_instruction() {
    ...
    let from_pubkey = bovey_pubkey::new_rand();
    let to_pubkey = bovey_pubkey::new_rand();
    let dup_pubkey = from_pubkey;
    let from_account = AccountSharedData::new(bov_to_lamports(100.), 1, &mock_program_id);
    ...
    let instruction =
        Instruction::new_with_bincode(mock_program_id, &bov_to_lamports(10.), account_metas);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
    ...
    let result = bank.process_transaction(&tx);
    assert_eq!(result, Ok(()));
    assert_eq!(bank.get_balance(&from_pubkey), bov_to_lamports(80.));
    assert_eq!(bank.get_balance(&to_pubkey), bov_to_lamports(20.));
}
...
fn test_account_ids_after_program_ids() {
    ...
    let instruction = Instruction::new_with_bincode(ocp_vote_program::id(), &10, account_metas);
    ...
    tx.message.account_keys.push(bovey_pubkey::new_rand());
    ...
    let result = bank.process_transaction(&tx);
    assert_eq!(result, Ok(()));
    let account = bank.get_account(&ocp_vote_program::id()).unwrap();
...
fn test_program_id_as_payer() {
    ...
    tx.message.account_keys.push(ocp_vote_program::id());
...
fn test_bank_hash_consistency() {
    ...
    activate_feature(
        &mut genesis_config,
        bovey_feature_set::set_exempt_rent_epoch_max::id(),
    );
...
fn test_add_builtin_no_overwrite() {
    let slot = 123;
    let program_id = bovey_pubkey::new_rand();
...
fn test_add_builtin_loader_no_overwrite() {
    let slot = 123;
    let loader_id = bovey_pubkey::new_rand();
...
fn test_reconfigure_token2_native_mint() {
    ...
    let genesis_config =
        create_genesis_config_with_leader(5, &bovey_pubkey::new_rand(), 0).genesis_config;
    let bank = Arc::new(Bank::new_for_tests(&genesis_config));
...
fn test_bank_load_program() {
    ...
    let program_key = bovey_pubkey::new_rand();
    let programdata_key = bovey_pubkey::new_rand();
...
fn test_bpf_loader_upgradeable_deploy_with_max_len() {
    ...
    let mut bank = Bank::new_for_tests(&genesis_config);
    bank.feature_set = Arc::new(FeatureSet::all_enabled());
    bank.deactivate_feature(&bovey_feature_set::disable_new_loader_v3_deployments::id());
    let (bank, bank_forks) = bank.wrap_with_bank_forks_for_tests();
    ...
    let payer_base_balance = LAMPORTS_PER_BOV;
    let deploy_fees = {
        ...
        + 'static,
{
    ...
    // Set up initial bank
    let mut genesis_config =
        create_genesis_config_with_leader(10, &bovey_pubkey::new_rand(), 374_999_998_287_840)
            .genesis_config;
    ...
    let all_pubkeys: Vec<Pubkey> = std::iter::repeat_with(bovey_pubkey::new_rand)
        .take(total_pubkeys)
        .collect();
...
fn test_get_inflation_start_slot_devnet_testnet() {
    let GenesisConfigInfo {
        mut genesis_config, ..
    } = create_genesis_config_with_leader(42, &bovey_pubkey::new_rand(), 42);
    genesis_config
        .accounts
...
fn test_stake_vote_account_validity() {
    ...
    // owner is changed from vote/stake program to something else.
...
fn test_load_and_execute_commit_transactions_rent_debits() {
    let (mut genesis_config, mint_keypair) = create_genesis_config(bov_to_lamports(1.));
    genesis_config.rent = Rent::default();
    let (bank, _bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let bank = Bank::new_from_parent(
        bank,
...
fn test_get_largest_accounts() {
    ...
    // Initialize accounts; all have larger BOV balances than current Bank built-ins
    ...
    assert_eq!(
        bank.get_largest_accounts(1, &pubkeys_hashset, AccountAddressFilter::Include, false)
            .unwrap(),
        vec![(pubkeys[4], bov_to_lamports(5.0))]
    );
    assert_eq!(
    ...
    assert_eq!(
        bank.get_largest_accounts(1, &exclude4, AccountAddressFilter::Exclude, false)
            .unwrap(),
        vec![(pubkeys[3], bov_to_lamports(4.0))]
    );
...
fn test_call_precomiled_program() {
    ...
    let instruction =
        bovey_sdk::secp256k1_instruction::new_secp256k1_instruction(&secp_privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    let message_arr = b"hello";
    let instruction =
        bovey_sdk::ed25519_instruction::new_ed25519_instruction(&privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
...
fn test_an_empty_instruction_without_program() {
    ...
    let destination = bovey_pubkey::new_rand();
...
fn test_accounts_data_size_with_good_transaction() {
    ...
    let (genesis_config, mint_keypair) = create_genesis_config(1_000 * LAMPORTS_PER_BOV);
    let bank = Bank::new_for_tests(&genesis_config);
    let (bank, _bank_forks) = bank.wrap_with_bank_forks_for_tests();
...
fn test_accounts_data_size_with_bad_transaction() {
    const ACCOUNT_SIZE: u64 = MAX_PERMITTED_DATA_LENGTH;
    let (genesis_config, _mint_keypair) = create_genesis_config(1_000 * LAMPORTS_PER_BOV);
    let bank = Bank::new_for_tests(&genesis_config);
    let (bank, _bank_forks) = bank.wrap_with_bank_forks_for_tests();
    let transaction = system_transaction::create_account(
        &Keypair::new(),
...
fn test_invalid_rent_state_changes_existing_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(bov_to_lamports(100.), &Pubkey::new_unique(), 42);
    genesis_config.rent = Rent::default();
...
fn test_rent_state_changes_sysvars() {
    ...
    let validator_pubkey = bovey_pubkey::new_rand();
    let validator_stake_lamports = bov_to_lamports(1.);
...
fn test_invalid_rent_state_changes_fee_payer() {
    ...
    genesis_config.fee_rate_governor = FeeRateGovernor::new(
        bovey_sdk::fee_calculator::DEFAULT_TARGET_LAMPORTS_PER_SIGNATURE,
        bovey_sdk::fee_calculator::DEFAULT_TARGET_SIGNATURES_PER_SLOT,
    );
...
fn test_rent_state_incinerator() {
    ...
    for amount in [rent_exempt_minimum - 1, rent_exempt_minimum] {
        bank.transfer(amount, &mint_keypair, &bovey_sdk::incinerator::id())
            .unwrap();
    }
}
...
fn test_resize_and_rent() {
    ...
    let rent_paying_pubkey = bovey_pubkey::new_rand();
...
fn test_accounts_data_size_and_resize_transactions() {
    ...
    bank.store_account(
        &funding_keypair.pubkey(),
        &AccountSharedData::new(10 * LAMPORTS_PER_BOV, 0, &mock_program_id),
    );
    ...
    // Test case: Grow account
    {
        ...
        let account_balance = LAMPORTS_PER_BOV;
        let account_size =
...
fn test_accounts_data_size_and_rent_collection(should_collect_rent: bool) {
    ...
    if should_collect_rent {
        genesis_config
            .accounts
            .remove(&bovey_feature_set::disable_rent_fees_collection::id());
    }
...
fn test_accounts_data_size_from_genesis() {
    let GenesisConfigInfo {
        ...
        1_000_000 * LAMPORTS_PER_BOV,
        &Pubkey::new_unique(),
        100 * LAMPORTS_PER_BOV,
    );
    genesis_config.rent = Rent::default();
...
fn test_cap_accounts_data_allocations_per_transaction() {
    ...
    let (genesis_config, mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_BOV);
    let (bank, _bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    ...
    assert_eq!(
        ...
        Err(TransactionError::InstructionError(
            ...
            bovey_sdk::instruction::InstructionError::MaxAccountsDataAllocationsExceeded,
        )),
    );
}
...
fn test_is_in_slot_hashes_history() {
    use bovey_sdk::slot_hashes::MAX_ENTRIES;
...
fn test_feature_activation_loaded_programs_epoch_transition() {
    ...
    // Bank Setup
    let (mut genesis_config, mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_BOV);
    genesis_config
        .accounts
    ...
    F: Fn(&Bank),
{
    ...
    let mint_lamports = bov_to_lamports(1.0);
    let len1 = 123;
    let len2 = 456;
...
fn test_system_instruction_unsigned_transaction() {
    let (genesis_config, alice_keypair) = create_genesis_config_no_tx_fee(bov_to_lamports(1.0));
    let alice_pubkey = alice_keypair.pubkey();
...
fn test_calc_vote_accounts_to_store_three() {
    ...
    let pubkey = bovey_pubkey::new_rand();
    let pubkey2 = bovey_pubkey::new_rand();
    let pubkey3 = bovey_pubkey::new_rand();
    let mut vote_account = AccountSharedData::default();
    vote_account.set_lamports(u64::MAX);
    vote_account_rewards.insert(
        pubkey,
...
fn test_failed_simulation_compute_units() {
    let (genesis_config, mint_keypair) = create_genesis_config(LAMPORTS_PER_BOV);
    let program_id = Pubkey::new_unique();
    let (bank, _bank_forks) =
        Bank::new_with_mockup_builtin_for_tests(&genesis_config, program_id, MockBuiltin::vm);
...
fn test_deploy_last_epoch_slot() {
    ...
    activate_feature(
        &mut genesis_config,
        bovey_feature_set::enable_loader_v4::id(),
    );
    genesis_config
        .accounts
...
fn test_loader_v3_to_v4_migration() {
    ...
    finalized_programdata_account
        ...
        .copy_from_slice(&elf);
    let message = Message::new(
        &[bovey_loader_v3_interface::instruction::migrate_program(
            &programdata_address,
            &program_keypair.pubkey(),
            &program_keypair.pubkey(),
        )],
    ...
    let payer_account = AccountSharedData::new(LAMPORTS_PER_BOV, 0, &system_program::id());
    bank.store_account(
        &programdata_address,
    ...
    for (mut programdata_account, transaction, expected_execution_result) in [
        ...
        let payer_balance = min_program_balance
            .saturating_add(min_programdata_balance)
            .saturating_add(LAMPORTS_PER_BOV)
...
fn test_rehash_accounts_unmodified(has_accounts_delta_hash: bool) {
    let ten_sol = 10 * LAMPORTS_PER_BOV;
