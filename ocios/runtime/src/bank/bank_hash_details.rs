...
use {
    ...
    bovey_accounts_db::{accounts_db::PubkeyHashAccount, accounts_hash::AccountHash},
    bovey_sdk::{
    ...
    bovey_transaction_status_client_types::UiInstruction,
    std::str::FromStr,
};
...
impl BankHashDetails {
    pub fn new(bank_hash_details: Vec<SlotDetails>) -> Self {
        Self {
            version: bovey_version::version!().to_string(),
    ...
    fn build_details(num_slots: usize) -> BankHashDetails {
        let slot_details: Vec<_> = (0..num_slots)
            .map(|slot| {
                ...
                let account_hash = AccountHash(bovey_sdk::hash::hash("account".as_bytes()));
