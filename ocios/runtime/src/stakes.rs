...
use {
    ...
    ocp_stake_program::stake_state::Stake,
    bovey_vote::vote_account::{VoteAccount, VoteAccounts},
    std::{
        collections::HashMap,
        ops::Add,
...
impl StakesCache {
    ...
    pub(crate) fn check_and_store(
        ...
        if account.lamports() == 0 {
            if ocp_vote_program::check_id(owner) {
                let _old_vote_account = {
                    let mut stakes = self.0.write().unwrap();
                    stakes.remove_vote_account(pubkey)
                };
            } else if ocp_stake_program::check_id(owner) {
                let mut stakes = self.0.write().unwrap();
...
pub(crate) mod tests {
    use {
        ...
        bovey_sdk::{account::WritableAccount, pubkey::Pubkey, rent::Rent, stake},
    ...
    pub(crate) fn create_staked_node_accounts(
        ...
        let vote_account =
            vote_state::create_account(&vote_pubkey, &bovey_pubkey::new_rand(), 0, 1);
        let stake_pubkey = bovey_pubkey::new_rand();
        (
    ...
    fn test_stakes_basic() {
        for i in 0..4 {
            ...
            let mut stake_account =
                create_stake_account(42, &vote_pubkey, &bovey_pubkey::new_rand());
            stakes_cache.check_and_store(&stake_pubkey, &stake_account, None);
            let stake = stake_state::stake_from(&stake_account).unwrap();
            {
                let stakes = stakes_cache.stakes();
    ...
    fn test_stakes_multiple_stakers() {
        ...
        let stake_pubkey2 = bovey_pubkey::new_rand();
