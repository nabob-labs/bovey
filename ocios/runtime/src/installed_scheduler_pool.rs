...
//! actual implementations provided by the dependent crate (`bovey-unified-scheduler-pool`, which
//! in turn depends on `bovey-ledger`, which in turn depends on `bovey-runtime`), avoiding a
//! cyclic dependency.
//!
...
use {
    ...
    bovey_unified_scheduler_logic::SchedulingMode,
    std::{
        fmt::{self, Debug},
        mem,
        ops::Deref,
...
///     Bank["Arc#lt;Bank#gt;"]
///
///     subgraph bovey-runtime[<span style="font-size: 70%">bovey-runtime</span>]
///         BankForks;
///         BankWithScheduler;
///         Bank;
...
///     end
///
///     subgraph bovey-unified-scheduler-pool[<span style="font-size: 70%">bovey-unified-scheduler-pool</span>]
///         SchedulerPool;
///         PooledScheduler;
///         ScheduleExecution(["schedule_execution()"]);
///     end
///
///     subgraph bovey-ledger[<span style="font-size: 60%">bovey-ledger</span>]
...
pub enum SchedulerStatus {
    ...
    /// `bovey_unified_scheduler_pool::DEFAULT_TIMEOUT_DURATION`) has passed if its bank hasn't
    ...
    fn do_create_timeout_listener(self: &Arc<Self>) -> TimeoutListener {
        ...
        TimeoutListener::new(move |pool| {
            ...
            scheduler.maybe_transition_from_active_to_stale(|scheduler| {
                ...
                // executions by `bovey_unified_scheduler_pool::ThreadManager::end_session()`
                // internally.
