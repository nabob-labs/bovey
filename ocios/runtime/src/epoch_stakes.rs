use {
    ...
    bovey_sdk::{clock::Epoch, pubkey::Pubkey},
    ...
    pub fn new_for_tests(
        ...
        Self::new(
            Arc::new(StakesEnum::Accounts(crate::stakes::Stakes::new_for_tests(
                0,
                bovey_vote::vote_account::VoteAccounts::from(Arc::new(vote_accounts_hash_map)),
                im::HashMap::default(),
            ))),
...
pub(crate) mod tests {
    use {
        ...
        bovey_sdk::{account::AccountSharedData, rent::Rent},
        ocp_stake_program::stake_state::{self, Delegation, Stake},
        bovey_vote::vote_account::VoteAccount,
        ocp_vote_program::vote_state::{self, create_account_with_authorized},
        std::iter,
    };
    ...
    fn new_vote_accounts(
        ...
        (0..num_nodes)
            .map(|_| {
                let node_id = bovey_pubkey::new_rand();
                (
                    node_id,
                    iter::repeat_with(|| {
                        let authorized_voter = bovey_pubkey::new_rand();
                        VoteAccountInfo {
                            vote_account: bovey_pubkey::new_rand(),
