use {
    crate::bank::Bank, core::ops::Deref, bovey_sdk::transaction::Result,
...
pub struct TransactionBatch<'a, 'b, Tx: VMMessage> {
...
impl<'a, 'b, Tx: VMMessage> TransactionBatch<'a, 'b, Tx> {
    pub fn new(
        lock_results: Vec<Result<()>>,
        bank: &'a Bank,
        sanitized_txs: OwnedOrBorrowed<'b, Tx>,
...
impl<Tx: VMMessage> Drop for TransactionBatch<'_, '_, Tx> {
    fn drop(&mut self) {
        if self.needs_unlock() {
