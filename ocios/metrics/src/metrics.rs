...
pub enum MetricsError {
    ...
    #[error("BOVEY_METRICS_CONFIG is invalid: '{0}'")]
    ConfigInvalid(String),
    #[error("BOVEY_METRICS_CONFIG is incomplete")]
    ConfigIncomplete,
    #[error("BOVEY_METRICS_CONFIG database mismatch: {0}")]
    DbMismatch(String),
}
...
impl Default for MetricsAgent {
    fn default() -> Self {
        let max_points_per_sec = env::var("BOVEY_METRICS_MAX_POINTS_PER_SECOND")
            .map(|x| {
                x.parse()
                    .expect("Failed to parse BOVEY_METRICS_MAX_POINTS_PER_SECOND")
            })
            .unwrap_or(4000);
...
fn get_metrics_config() -> Result<MetricsConfig, MetricsError> {
    ...
    let config_var = env::var("BOVEY_METRICS_CONFIG")?;
