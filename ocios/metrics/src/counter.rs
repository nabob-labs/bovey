...
impl Counter {
    fn default_metrics_rate() -> u64 {
        let v = env::var("BOVEY_DEFAULT_METRICS_RATE")
    ...
    fn default_log_rate() -> usize {
        let v = env::var("BOVEY_DEFAULT_LOG_RATE")
    ...
    pub fn inc(&self, level: log::Level, events: usize) {
        let now = bovey_time_utils::timestamp();
...
mod tests {
    ...
    fn test_metricsrate() {
        try_init_logger_at_level_info().ok();
        let _readlock = get_env_lock().read();
        env::remove_var("BOVEY_DEFAULT_METRICS_RATE");
    ...
    fn test_metricsrate_env() {
        ...
        env::set_var("BOVEY_DEFAULT_METRICS_RATE", "50");
        let mut counter = create_counter!("test", 1000, 0);
        counter.init();
    ...
    fn test_lograte() {
        ...
        assert_eq!(
            ...
            DEFAULT_LOG_RATE,
            "default_log_rate() is {}, expected {}, BOVEY_DEFAULT_LOG_RATE environment variable set?",
            Counter::default_log_rate(),
            DEFAULT_LOG_RATE,
        );
