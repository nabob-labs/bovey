...
fn test_get_durable_nonce() {
    ...
    // No instructions - no nonce
    {
        ...
        assert!(VMMessage::get_durable_nonce(&message).is_none());
    }
    ...
    // system program id - nonce instruction fee-payer
    {
        ...
        assert_eq!(VMMessage::get_durable_nonce(&message), Some(&payer_nonce));
    }
    ...
    // system program id - nonce instruction (non-fee-payer)
    {
        ...
        assert_eq!(VMMessage::get_durable_nonce(&message), Some(&nonce));
    }
...
fn test_get_ix_signers() {
    ...
    assert_eq!(
        VMMessage::get_ix_signers(&message, 0).collect::<HashSet<_>>(),
        HashSet::from_iter([&signer0])
    );
    assert_eq!(
        VMMessage::get_ix_signers(&message, 1).collect::<HashSet<_>>(),
        HashSet::from_iter([&signer0, &signer1])
    );
    assert_eq!(
        VMMessage::get_ix_signers(&message, 2).collect::<HashSet<_>>(),
        HashSet::from_iter([&signer0])
    );
    assert_eq!(
        VMMessage::get_ix_signers(&message, 3).collect::<HashSet<_>>(),
        HashSet::default()
    );
}
