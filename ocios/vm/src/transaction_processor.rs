...
use {
    ...
    bovey_account::{state_traits::StateMut, AccountSharedData, ReadableAccount, PROGRAM_OWNERS},
    ocp_bpf_loader_program::syscalls::{
        create_program_runtime_environment_v1, create_program_runtime_environment_v2,
    },
    bovey_clock::{Epoch, Slot},
    ...
    bovey_fee_structure::{FeeBudgetLimits, FeeDetails, FeeStructure},
    bovey_hash::Hash,
    bovey_instruction::TRANSACTION_LEVEL_STACK_HEIGHT,
    bovey_log_collector::LogCollector,
    bovey_measure::{measure::Measure, measure_us},
    bovey_message::compiled_instruction::CompiledInstruction,
    bovey_nonce::{
    ...
    bovey_program_runtime::{
        ...
        bovey_bpf::{program::BuiltinProgram, vm::Config as VmConfig},
    ...
    bovey_sdk_ids::{native_loader, system_program},
    bovey_vm_rent_collector::vm_rent_collector::VMRentCollector,
    bovey_vm_transaction::{vm_message::VMMessage, vm_transaction::VMTransaction},
    bovey_timings::{ExecuteTimingType, ExecuteTimings},
    bovey_transaction_context::{ExecutionRecord, TransactionContext},
    bovey_transaction_error::{TransactionError, TransactionResult},
    bovey_type_overrides::sync::{atomic::Ordering, Arc, RwLock, RwLockReadGuard},
    std::{
...
pub struct TransactionProcessingEnvironment<'a> {
    ...
    pub rent_collector: Option<&'a dyn VMRentCollector>,
}
...
pub struct TransactionBatchProcessor<FG: ForkGraph> {
    ...
    /// accessible from on chain programs. It is passed to VM from
...
impl<FG: ForkGraph> TransactionBatchProcessor<FG> {
    ...
    pub fn get_environments_for_epoch(
        &self,
        epoch: Epoch,
    ) -> Option<bovey_program_runtime::loaded_programs::ProgramRuntimeEnvironments> {
        self.program_cache
            .try_read()
            .ok()
    ...
    /// Main entrypoint to the VM.
    pub fn load_and_execute_sanitized_transactions<CB: TransactionProcessingCallback>(
        &self,
        callbacks: &CB,
        sanitized_txs: &[impl VMTransaction],
    ...
    fn validate_transaction_nonce_and_fee_payer<CB: TransactionProcessingCallback>(
        ...
        rent_collector: &dyn VMRentCollector,
        ...
        loop {
            ...
            if let Some((key, program)) = program_to_store {
                    ...
                    && limit_to_load_programs
                {
                    ...
                    // cache slot. It is not possible to mock this error for VM unit
                    // tests purposes.
    ...
    pub fn prepare_program_cache_for_upcoming_feature_set<CB: TransactionProcessingCallback>(
        ...
        let slots_in_recompilation_phase =
            (bovey_program_runtime::loaded_programs::MAX_LOADED_ENTRY_COUNT as u64)
                .min(slots_in_epoch)
                .checked_div(2)
                .unwrap();
        let mut program_cache = self.program_cache.write().unwrap();
...
mod tests {
    ...
    use {
        ...
        bovey_account::{create_account_shared_data_for_test, WritableAccount},
        bovey_clock::Clock,
        bovey_compute_budget::compute_budget_limits::ComputeBudgetLimits,
        bovey_compute_budget_instruction::instructions_processor::process_compute_budget_instructions,
        bovey_compute_budget_interface::ComputeBudgetInstruction,
        bovey_epoch_schedule::EpochSchedule,
        bovey_feature_set::FeatureSet,
        bovey_fee_calculator::FeeCalculator,
        bovey_fee_structure::{FeeDetails, FeeStructure},
        bovey_hash::Hash,
        bovey_keypair::Keypair,
        bovey_message::{LegacyMessage, Message, MessageHeader, SanitizedMessage},
        bovey_nonce as nonce,
        bovey_program_runtime::loaded_programs::{BlockRelation, ProgramCacheEntryType},
        bovey_rent::Rent,
        bovey_rent_debits::RentDebits,
        bovey_reserved_account_keys::ReservedAccountKeys,
        bovey_sdk::rent_collector::{RentCollector, RENT_EXEMPT_RENT_EPOCH},
        bovey_sdk_ids::{bpf_loader, system_program, sysvar},
        bovey_signature::Signature,
        bovey_transaction::{sanitized::SanitizedTransaction, Transaction},
    ...
    fn test_validate_transaction_fee_payer_exact_balance() {
        ...
        let compute_budget_limits = process_compute_budget_instructions(
            VMMessage::program_instructions_iter(&message),
            &FeatureSet::default(),
        )
        .unwrap();
        let fee_payer_address = message.fee_payer();
    ...
    fn test_validate_transaction_fee_payer_is_nonce() {
        let lamports_per_signature = 5000;
        let rent_collector = RentCollector::default();
        let compute_unit_limit = 2 * ocp_compute_budget_program::DEFAULT_COMPUTE_UNITS;
