use {
    ...
    bovey_vm_transaction::message_address_table_lookup::VMMessageAddressTableLookup,
};
...
impl<'a> Iterator for AddressTableLookupIterator<'a> {
    type Item = VMMessageAddressTableLookup<'a>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.index < self.num_address_table_lookups {
            ...
            Some(VMMessageAddressTableLookup {
                account_key,
                writable_indexes,
                readonly_indexes,
            })
...
#[cfg(test)]
mod tests {
    use {super::*, bovey_message::v0::MessageAddressTableLookup, bovey_short_vec::ShortVec};
