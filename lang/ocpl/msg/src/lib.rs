...
/// single expression it will be passed directly to [`bov_log`]. The expression
...
/// [`format!`] macro before being logged with `bov_log`.
///
...
/// for constrained environments like the Bovey VM.
///
/// # Examples
///
/// ```
...
macro_rules! msg {
    ...
    ($($arg:tt)*) => ($crate::bov_log(&format!($($arg)*)));
}
...
pub fn bov_log(message: &str) {
    #[cfg(target_os = "bovey")]
    unsafe {
        syscalls::bov_log_(message.as_ptr(), message.len() as u64);
    }
