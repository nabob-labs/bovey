...
fn derive_abi_sample_enum_type(input: ItemEnum) -> TokenStream {
    ...
    let result = quote! {
        ...
        #( #attrs )*
        impl #impl_generics ::bovey_frozen_abi::abi_example::AbiExample for #type_name #ty_generics #where_clause {
            fn example() -> Self {
                ::bovey_frozen_abi::__private::log::info!(
                    "AbiExample for enum: {}",
    ...
    let result = quote! {
        ...
        impl #impl_generics ::bovey_frozen_abi::abi_example::AbiExample for #type_name #ty_generics #where_clause {
            fn example() -> Self {
                ...
                use ::bovey_frozen_abi::abi_example::AbiExample;
...
fn do_derive_abi_enum_visitor(input: ItemEnum) -> TokenStream {
    ...
    (quote! {
        impl #impl_generics ::bovey_frozen_abi::abi_example::AbiEnumVisitor for #type_name #ty_generics #where_clause {
            fn visit_for_abi(&self, digester: &mut ::bovey_frozen_abi::abi_digester::AbiDigester) -> ::bovey_frozen_abi::abi_digester::DigestResult {
...
fn quote_for_test(
    ...
    quote! {
        ...
        mod #test_mod_ident {
            use super::*;
            use ::bovey_frozen_abi::abi_example::{AbiExample, AbiEnumVisitor};

            #[test]
            fn test_abi_digest() {
                ::bovey_logger::setup();
                let mut digester = ::bovey_frozen_abi::abi_digester::AbiDigester::create();
                let example = <#type_name>::example();
                let result = <_>::visit_for_abi(&&example, &mut digester);
                let mut hash = digester.finalize();
                // pretty-print error
                if result.is_err() {
                    ::bovey_frozen_abi::__private::log::error!("digest error: {:#?}", result);
                }
                result.unwrap();
                let actual_digest = ::std::format!("{}", hash);
                if ::std::env::var("BOVEY_ABI_BULK_UPDATE").is_ok() {
                    if #expected_digest != actual_digest {
                        #p!("sed -i -e 's/{}/{}/g' $(git grep --files-with-matches frozen_abi)", #expected_digest, hash);
                    }
                    ::bovey_frozen_abi::__private::log::warn!("Not testing the abi digest under BOVEY_ABI_BULK_UPDATE!");
                } else {
                    if let Ok(dir) = ::std::env::var("BOVEY_ABI_DUMP_DIR") {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Examine the diff in BOVEY_ABI_DUMP_DIR!: \n$ diff -u {}/*{}* {}/*{}*", dir, #expected_digest, dir, actual_digest);
                    } else {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Confirm the diff by rerunning before and after this test failed with BOVEY_ABI_DUMP_DIR!");
                    }
                }
            }
        }
