...
//! [`ecrecover`] precompile. For that Bovey provides the [`secp256k1_recover`]
//! syscall.
//!
...
//!
//! The `bovey_sdk` crate provides few APIs for building the instructions and
//! transactions necessary for properly using the secp256k1 native program.
//! Many steps must be done manually.
//!
//! The `bovey_program` crate provides no APIs to assist in interpreting
...
//! and callers should ensure they use the same version of `bincode` as the Bovey SDK.
//! This data structure is not provided to Bovey programs,
...
//!
//! **The bovey_ `secp256k1_recover` function does not prevent signature
//! malleability**. This is in contrast to the Bitcoin secp256k1 library, which
//! does prevent malleability by default. Bovey accepts signatures with `S`
...
//! to parse the secp256k1 instruction data from within a Bovey program.
//!
//! ```no_run
//! mod secp256k1_defs {
//!     use bovey_program_error::ProgramError;
...
//!
//! This example has two components: a Bovey program, and an RPC client that
//! sends a transaction to call it. The RPC client will sign a single message,
//! and the Bovey program will introspect the secp256k1 instruction to verify
//! that the signer matches a known authorized public key.
//!
//! The Bovey program. Note that it uses `libsecp256k1` version 0.7.0 to parse
...
//! #     }
//! # }
//! use bovey_account_info::{next_account_info, AccountInfo};
//! use bovey_msg::msg;
//! use bovey_program_error::{ProgramError, ProgramResult};
//! use bovey_sdk_ids::secp256k1_program;
//! use bovey_instructions_sysvar::load_instruction_at_checked;
//!
...
//!     assert!(bovey_sdk_ids::sysvar::instructions::check_id(
...
//!     let secp256k1_instr =
//!         bovey_instructions_sysvar::load_instruction_at_checked(0, instructions_sysvar_account)?;
//!
...
//!
//! ```no_run
//! # use bovey_sdk::example_mocks::bovey_rpc_client;
//! use anyhow::Result;
//! use bovey_instruction::{AccountMeta, Instruction};
//! use bovey_keypair::Keypair;
//! use bovey_rpc_client::rpc_client::RpcClient;
//! use bovey_signer::Signer;
//! use bovey_sdk::transaction::Transaction;
//!
//! fn demo_secp256k1_verify_basic(
...
//!     let secp256k1_instr = ocp_secp256k1_program::new_secp256k1_instruction(&secp256k1_secret_key, msg);
//!
//!     let program_instr = Instruction::new_with_bytes(
//!         program_keypair.pubkey(),
//!         &[],
//!         vec![
//!             AccountMeta::new_readonly(bovey_sdk_ids::sysvar::instructions::ID, false)
//!         ],
//!     );
//!
...
//! containing many signatures, and a Bovey program that parses them all. This
...
//! transaction. In the _previous_ example, the Bovey program asserted that all
//! signatures, messages, and addresses were stored in the instruction at 0. In
//! this next example the Bovey program supports signatures, messages, and
...
//! use bovey_msg::msg;
//! use bovey_sdk_ids::secp256k1_program;
//! use bovey_instructions_sysvar::{get_instruction_relative, load_instruction_at_checked};
//!
...
//!
//!     let secp256k1_instr =
//!         bovey_instructions_sysvar::get_instruction_relative(-1, instructions_sysvar_account)?;
//!
//!     assert!(secp256k1_program::check_id(&secp256k1_instr.program_id));
//!
...
//!         let message_hash = {
//!             let mut hasher = bovey_keccak_hasher::Hasher::default();
//!             hasher.hash(&message);
//!             hasher.result()
//!         };
...
//!         bovey_sdk_ids::secp256k1_program::ID,
...
#[cfg(feature = "bincode")]
use {bovey_instruction::Instruction, bovey_precompile_error::PrecompileError};
...
pub fn new_secp256k1_instruction(
    ...
    Instruction {
        program_id: bovey_sdk_ids::secp256k1_program::id(),
        accounts: vec![],
...
///
/// `feature_set` is the set of active Bovey features. It is used to enable or
...
pub fn verify(
    ...
    _feature_set: &bovey_feature_set::FeatureSet,
...
pub mod test {
    use {
        ...
        bovey_sdk::transaction::Transaction,
        bovey_signer::Signer,
    };
    ...
    fn test_secp256k1() {
        ...
        let feature_set = bovey_feature_set::FeatureSet::all_enabled();
