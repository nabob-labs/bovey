...
// `bovey_sdk_ids` once the version is updated to 2.2.0.
...
#[cfg(feature = "bincode")]
#[cfg(test)]
static_assertions::const_assert_eq!(bovey_nonce::state::State::size(), NONCE_STATE_SIZE);
...
#[cfg_attr(
    feature = "frozen-abi",
    bovey_frozen_abi_macro::frozen_abi(digest = "2LnVTnJg7LxB1FawNZLoQEY8yiYx3MT3paTdx4s5kAXU"),
    derive(
        bovey_frozen_abi_macro::AbiExample,
        bovey_frozen_abi_macro::AbiEnumVisitor
    )
)]
#[cfg_attr(
    feature = "serde",
...
///
/// ```
/// # use bovey_program::example_mocks::{bovey_sdk, bovey_rpc_client};
/// use bovey_rpc_client::rpc_client::RpcClient;
/// use bovey_sdk::{
///     signature::{Keypair, Signer},
///     transaction::Transaction,
/// };
/// use bovey_system_interface::{instruction, program};
/// use anyhow::Result;
///
/// fn create_account(
///     client: &RpcClient,
...
///
/// This example submits the instruction from an on-chain Bovey program. The
...
/// use bovey_cpi::invoke_signed;
/// use bovey_program::sysvar::{rent::Rent, Sysvar};
/// use bovey_program_entrypoint::entrypoint;
...
///     transaction::Transaction,
/// };
/// use bovey_system_interface::instruction;
/// use anyhow::Result;
///
/// fn create_account(
///     client: &RpcClient,
...
/// ## Example: on-chain program
///
/// This example submits the instructions from an on-chain Bovey program. The
...
/// ```
/// # use borsh::{BorshDeserialize, BorshSerialize};
/// use bovey_account_info::{next_account_info, next_account_infos, AccountInfo};
/// use bovey_cpi::invoke_signed;
/// use bovey_program_entrypoint::entrypoint;
/// use bovey_program_error::ProgramResult;
/// use bovey_pubkey::Pubkey;
...
///
/// Standard Bovey transactions include a [recent blockhash][rbh] (sometimes
/// referred to as a _[nonce]_). During execution the Bovey runtime verifies
...
/// in the first position, the Bovey runtime recognizes the transaction as one
...
/// [`bovey_rpc_client_nonce_utils::data_from_account`][dfa] function.
///
...
/// # use bovey_program::example_mocks::bovey_sdk;
/// # use bovey_program::example_mocks::bovey_rpc_client;
/// # use bovey_program::example_mocks::bovey_rpc_client_nonce_utils;
...
///     #   lamports: 1,
///     #   data: vec![0],
///     #   owner: bovey_sdk::system_program::ID,
///     #   executable: false,
///     #   rent_epoch: 1,
///     # });
///     let nonce_account = client.get_account(nonce_account_pubkey)?;
///     let nonce_data = bovey_rpc_client_nonce_utils::data_from_account(&nonce_account)?;
...
mod tests {
    use super::*;
    use bovey_program::sysvar::SysvarId;
    ...
    #[test]
    fn test_constants() {
        // Ensure that the constants are in sync with the bovey_ program.
        assert_eq!(
            RECENT_BLOCKHASHES_ID,
            bovey_program::sysvar::recent_blockhashes::RecentBlockhashes::id(),
        );

        // Ensure that the constants are in sync with the bovey_ rent.
        assert_eq!(RENT_ID, bovey_program::sysvar::rent::Rent::id());
    }
