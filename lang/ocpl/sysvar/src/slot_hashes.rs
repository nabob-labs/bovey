...
//! [`bovey_program_error::ProgramError::UnsupportedSysvar`] because this sysvar account is too large
...
//! # use bovey_sdk_ids::sysvar::slot_hashes;
//! # use bovey_slot_hashes::SlotHashes;
//! # use anyhow::Result;
//! #
...
#[cfg(feature = "bincode")]
use {crate::Sysvar, bovey_account_info::AccountInfo};
use {bovey_clock::Slot, bovey_hash::Hash};

#[cfg(all(feature = "bincode", feature = "bytemuck"))]
const U64_SIZE: usize = std::mem::size_of::<u64>();

pub use {
    bovey_sdk_ids::sysvar::slot_hashes::{check_id, id, ID},
    bovey_slot_hashes::SlotHashes,
...
impl Sysvar for SlotHashes {
    ...
    ) -> Result<Self, bovey_program_error::ProgramError> {
        // This sysvar is too large to bincode::deserialize in-program
        Err(bovey_program_error::ProgramError::UnsupportedSysvar)
    }
}
...
#[cfg(feature = "bytemuck")]
impl PodSlotHashes {
    /// Fetch all of the raw sysvar data using the `bov_get_sysvar` syscall.
    pub fn fetch() -> Result<Self, bovey_program_error::ProgramError> {
        ...
        if data.as_ptr().align_offset(8) != 0 {
            return Err(bovey_program_error::ProgramError::InvalidAccountData);
        }

        // Populate the buffer by fetching all sysvar data using the
        // `bov_get_sysvar` syscall.
        crate::get_sysvar(
            &mut data,
            &SlotHashes::id(),
            /* offset */ 0,
        ...
        let length = data
            ...
            .ok_or(bovey_program_error::ProgramError::InvalidAccountData)?;
    ...
    pub fn as_slice(&self) -> Result<&[PodSlotHash], bovey_program_error::ProgramError> {
        self.data
    ...
    pub fn get(&self, slot: &Slot) -> Result<Option<Hash>, bovey_program_error::ProgramError> {
    ...
    pub fn position(
        &self,
        slot: &Slot,
    ) -> Result<Option<usize>, bovey_program_error::ProgramError> {
        self.as_slice().map(|pod_hashes| {
            pod_hashes
                .binary_search_by(|PodSlotHash { slot: this, .. }| slot.cmp(this))
                .ok()
...
impl SlotHashesSysvar {
    ...
    pub fn get(slot: &Slot) -> Result<Option<Hash>, bovey_program_error::ProgramError> {
    ...
    /// Returns `None` if the key is not found.
    pub fn position(slot: &Slot) -> Result<Option<usize>, bovey_program_error::ProgramError> {
        get_pod_slot_hashes().map(|pod_hashes| {
            pod_hashes
                .binary_search_by(|PodSlotHash { slot: this, .. }| slot.cmp(this))
                .ok()
...
#[cfg(feature = "bytemuck")]
fn get_pod_slot_hashes() -> Result<Vec<PodSlotHash>, bovey_program_error::ProgramError> {
    let mut pod_hashes = vec![PodSlotHash::default(); bovey_slot_hashes::MAX_ENTRIES];
    {
        let data = bytemuck::try_cast_slice_mut::<PodSlotHash, u8>(&mut pod_hashes)
            .map_err(|_| bovey_program_error::ProgramError::InvalidAccountData)?;
...
mod tests {
    use {
        super::*, crate::tests::mock_get_sysvar_syscall, serial_test::serial, bovey_hash::Hash,
        bovey_sha256_hasher::hash, bovey_slot_hashes::MAX_ENTRIES, test_case::test_case,
    };
