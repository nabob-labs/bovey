...
//!     instruction_data: &[u8],
//! ) -> ProgramResult {
//!     let clock = bovey_clock::Clock::get()?;
//!     msg!("clock: {:#?}", clock);
//!     Ok(())
//! }
//! ```
//!
//! Since Bovey sysvars are accounts, if the `AccountInfo` is provided to the
...
//!     let clock = bovey_clock::Clock::from_account_info(&clock_account)?;
//!     msg!("clock: {:#?}", clock);
//!     Ok(())
//! }
//! ```
...
pub mod __private {
    ...
    pub use {bovey_program_entrypoint::SUCCESS, bovey_program_error::ProgramError};
}
use bovey_pubkey::Pubkey;
#[allow(deprecated)]
#[doc(inline)]
...
#[cfg(feature = "bincode")]
use {
    bovey_account_info::AccountInfo, bovey_program_error::ProgramError,
...
mod sysvar_ids {
    ...
    lazy_static! {
        ...
        pub static ref ALL_IDS: Vec<Pubkey> = vec![
            ...
            slot_history::id(),
            stake_history::id(),
            bovey_sdk_ids::sysvar::instructions::id(),
        ];
    }
}
...
#[deprecated(
    since = "2.0.0",
    note = "please check the account's owner or use bovey_sdk::reserved_account_keys::ReservedAccountKeys instead"
)]
#[allow(deprecated)]
...
/// Handler for retrieving a slice of sysvar data from the `bov_get_sysvar`
/// syscall.
fn get_sysvar(
    dst: &mut [u8],
    sysvar_id: &Pubkey,
    offset: u64,
    length: u64,
) -> Result<(), bovey_program_error::ProgramError> {
    // Check that the provided destination buffer is large enough to hold the
    // requested data.
    if dst.len() < length as usize {
        return Err(bovey_program_error::ProgramError::InvalidArgument);
    }
    ...
    #[cfg(target_os = "bovey")]
    let result = unsafe {
        bovey_define_syscall::definitions::bov_get_sysvar(sysvar_id, var_addr, offset, length)
    };

    #[cfg(not(target_os = "bovey"))]
    let result = crate::program_stubs::bov_get_sysvar(sysvar_id, var_addr, offset, length);

    match result {
        bovey_program_entrypoint::SUCCESS => Ok(()),
        e => Err(e.into()),
    }
}
...
mod tests {
    use {
        ...
        bovey_clock::Epoch,
        bovey_program_entrypoint::SUCCESS,
    ...
    bovey_pubkey::declare_id!("TestSysvar111111111111111111111111111111111");
    impl bovey_sysvar_id::SysvarId for TestSysvar {
        fn id() -> bovey_pubkey::Pubkey {
            id()
        }

        fn check_id(pubkey: &bovey_pubkey::Pubkey) -> bool {
            check_id(pubkey)
        }
    }
