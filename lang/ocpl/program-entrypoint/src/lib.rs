...
pub use {
    bovey_account_info::MAX_PERMITTED_DATA_INCREASE, bovey_msg::msg as __msg,
...
/// errors. To avoid this, it is common for Bovey programs to define an
...
///     use bovey_program_entrypoint::ProgramResult;
///     use bovey_msg::msg;
...
macro_rules! custom_heap_default {
    () => {
        #[cfg(all(not(feature = "custom-heap"), target_os = "bovey"))]
        #[global_allocator]
...
///
/// # Defining a panic handler for Bovey
///
/// _The mechanism for defining a Bovey panic handler is different [from most
...
///
/// ```ignore
/// #[cfg(all(feature = "custom-panic", target_os = "bovey"))]
/// #[no_mangle]
...
/// ```
///
/// The above is how Bovey defines the default panic handler.
#[macro_export]
macro_rules! custom_panic_default {
    () => {
        #[cfg(all(not(feature = "custom-panic"), target_os = "bovey"))]
        #[no_mangle]
        fn custom_panic(info: &core::panic::PanicInfo<'_>) {
