//! The original and current Bovey message format.
//!
//! This crate defines two versions of `Message` in their own modules:
//! [`legacy`] and [`v0`]. `legacy` is the current version as of Bovey 1.10.0.
...
use bovey_frozen_abi_macro::{frozen_abi, AbiExample};
...
// copied from deprecated code in bovey_program::sysvar to avoid a dependency.
...
#[deprecated(
    since = "2.0.0",
    note = "please use `bovey_sdk::reserved_account_keys::ReservedAccountKeys::is_reserved` instead"
)]
#[allow(deprecated)]
...
/// A Bovey transaction message (legacy).
///
...
pub struct Message {
    ...
    #[cfg_attr(feature = "serde", serde(with = "bovey_short_vec"))]
...
impl Message {
    ...
    /// # Examples
    ///
    /// This example uses the [`bovey_sdk`], [`bovey_rpc_client`] and [`anyhow`] crates.
    ///
    /// ```
    ...
    ///     transaction::Transaction,
    /// };
    /// use bovey_system_interface::instruction::create_nonce_account;
    ///
    /// // A custom program instruction. This would typically be defined in
    /// // another crate so it can be shared between the on-chain program and
    /// // the client.
    ...
    ///     let nonce_account_size = bovey_nonce::state::State::size();
    ...
    pub fn new_with_nonce(
        ...
        let nonce_ix = bovey_system_interface::instruction::advance_nonce_account(
    ...
    #[cfg(all(not(target_os = "bovey"), feature = "bincode", feature = "blake3"))]
    ...
    #[cfg(all(not(target_os = "bovey"), feature = "blake3"))]
    pub fn hash_raw_message(message_bytes: &[u8]) -> Hash {
        use {blake3::traits::digest::Digest, bovey_hash::HASH_BYTES};
        let mut hasher = blake3::Hasher::new();
        hasher.update(b"bovey-tx-message-v1");
...
mod tests {
    #![allow(deprecated)]
    use {
        super::*, crate::MESSAGE_HEADER_LENGTH, bovey_instruction::AccountMeta,
        bovey_sha256_hasher::hash, std::collections::HashSet,
    };
    ...
    fn test_message_hash() {
        ...
        // in this case, the domain prefix `bovey-tx-message-v1` should be updated.
    ...
    fn test_inline_all_ids() {
        assert_eq!(bovey_sysvar::ALL_IDS.to_vec(), ALL_IDS.to_vec());
    }
}
