#[deprecated(
    since = "2.1.0",
    note = "Use bovey_transaction_error::SanitizeMessageError instead"
)]
pub use bovey_transaction_error::SanitizeMessageError;
use {
    crate::{
    ...
    bovey_sdk_ids::{ed25519_program, secp256k1_program, secp256r1_program},
    std::{borrow::Cow, collections::HashSet, convert::TryFrom},
};

// inlined to avoid bovey_nonce dep
#[cfg(feature = "bincode")]
        ...
        self.instructions()
            ...
            .filter(
                |ix| match self.account_keys().get(ix.program_id_index as usize) {
                    Some(program_id) => bovey_sdk_ids::system_program::check_id(program_id),
                    _ => false,
                },
            )
            .filter(|ix| {
                matches!(
                    ...
                    Ok(bovey_system_interface::instruction::SystemInstruction::AdvanceNonceAccount)
                )
            })
            .and_then(|ix| {
