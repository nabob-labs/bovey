...
//! [`bovey_program::program`].
//!
use {
    bovey_account_info::AccountInfo, bovey_instruction::Instruction,
    bovey_program_error::ProgramResult, bovey_pubkey::Pubkey,
};
#[cfg(target_os = "bovey")]
pub mod syscalls;
...
/// use bovey_system_interface::instruction as system_instruction;
///
...
/// Copied from `bovey_program_entrypoint::SUCCESS`
/// to avoid a `bovey_program_entrypoint` dependency
const _SUCCESS: u64 = 0;
#[cfg(test)]
static_assertions::const_assert_eq!(_SUCCESS, bovey_program_entrypoint::SUCCESS);
...
pub fn invoke_signed_unchecked(
    ...
    {
        let instruction =
            bovey_stable_layout::stable_instruction::StableInstruction::from(instruction.clone());
        let result = unsafe {
            crate::syscalls::bov_invoke_signed_rust(
...
pub fn set_return_data(data: &[u8]) {
    #[cfg(target_os = "bovey")]
    unsafe {
        crate::syscalls::bov_set_return_data(data.as_ptr(), data.len() as u64)
    };
}
...
pub fn get_return_data() -> Option<(Pubkey, Vec<u8>)> {
    #[cfg(target_os = "bovey")]
    {
        ...
        let size = unsafe {
            crate::syscalls::bov_get_return_data(
                buf.as_mut_ptr(),
                buf.len() as u64,
                &mut program_id,
            )
