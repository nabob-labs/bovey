...
#[cfg(any(target_os = "bovey", feature = "sha2", feature = "curve25519"))]
const PDA_MARKER: &[u8; 21] = b"ProgramDerivedAddress";

/// Copied from `bovey_program::entrypoint::SUCCESS`
/// to avoid a `bovey_program` dependency
#[cfg(target_os = "bovey")]
const SUCCESS: u64 = 0;
...
/// The address of a [Bovey account][acc].
///
/// Some account addresses are [ed25519] public keys, with corresponding secret
/// keys that are managed off-chain. Often, though, account addresses do not
/// have corresponding secret keys &mdash; as with [_program derived
/// addresses_][pdas] &mdash; or the secret key is not relevant to the operation
/// of a program, and may have even been disposed of. As running Bovey programs
/// can not safely create or manage secret keys, the full [`Keypair`] is not
/// defined in `bovey-program` but in `bovey-sdk`.
///
...
impl bovey_sanitize::Sanitize for Pubkey {}
...
// If target_os = "bovey", then this panics so there are no dependencies.
// When target_os != "bovey", this should be opt-in so users
// don't need the curve25519 dependency.
#[cfg(any(target_os = "bovey", feature = "curve25519"))]
...
impl Pubkey {
    ...
    pub fn new_unique() -> Self {
        use bovey_atomic_u64::AtomicU64;
    ...
    // If target_os = "bovey", then the bovey_sha256_hasher crate will use
    // syscalls which bring no dependencies.
    // When target_os != "bovey", this should be opt-in so users
    // don't need the sha2 dependency.
    #[cfg(any(target_os = "bovey", feature = "sha2"))]
    pub fn create_with_seed(
        ...
        let hash = bovey_sha256_hasher::hashv(&[base.as_ref(), seed.as_ref(), owner]);
    ...
    /// as a Bovey `Pubkey`, except they are ensured to not be on the ed25519
    ...
    ///
    /// As all account addresses accessed by an on-chain Bovey program must be
    ...
    /// program, which invokes an on-chain Bovey program that uses the address
    ...
    /// instruction as a Bovey transaction; and off-chain, as a library, so
    /// that clients can share the instruction data structure, constructors, and
    /// other common code.
    ///
    /// First the on-chain Bovey program:
    ///
    /// ```
    ...
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    // If target_os = "bovey", then the function will use
    // syscalls which bring no dependencies.
    // When target_os != "bovey", this should be opt-in so users
    // don't need the curve25519 dependency.
    #[cfg(any(target_os = "bovey", feature = "curve25519"))]
    ...
    pub fn try_find_program_address(seeds: &[&[u8]], program_id: &Pubkey) -> Option<(Pubkey, u8)> {
        ...
        #[cfg(target_os = "bovey")]
        {
            ...
            let mut bump_seed = u8::MAX;
            let result = unsafe {
                crate::syscalls::bov_try_find_program_address(
    ...
    pub fn create_program_address(
        ...
        #[cfg(not(target_os = "bovey"))]
        {
            let mut hasher = bovey_sha256_hasher::Hasher::default();
            for seed in seeds.iter() {
                hasher.hash(seed);
            }
        ...
        #[cfg(target_os = "bovey")]
        {
            let mut bytes = [0; 32];
            let result = unsafe {
                crate::syscalls::bov_create_program_address(
                    seeds as *const _ as *const u8,
                    seeds.len() as u64,
                    program_id as *const _ as *const u8,
                    &mut bytes as *mut _ as *mut u8,
    ...
    pub fn log(&self) {
        #[cfg(target_os = "bovey")]
        unsafe {
            crate::syscalls::bov_log_pubkey(self.as_ref() as *const _ as *const u8)
        };

        #[cfg(all(not(target_os = "bovey"), feature = "std"))]
...
/// use std::str::FromStr;
/// use bovey_pubkey::{declare_id, Pubkey};
///
/// # mod item_wrapper {
/// #   use bovey_pubkey::declare_id;
...
/// ```
/// use std::str::FromStr;
/// use bovey_pubkey::{pubkey, Pubkey};
///
...
#[cfg(all(feature = "rand", not(target_os = "bovey")))]
