use {
    crate::versioned::sanitized::SanitizedVersionedTransaction, bovey_message::VersionedMessage,
    bovey_pubkey::Pubkey, bovey_signature::Signature,
};
...
pub fn is_simple_vote_transaction_impl<'a>(
    ...
    signatures.len() < 3
        ...
        && instruction_programs
            ...
            .map(|program_id| program_id == &bovey_sdk_ids::vote::ID)
            .unwrap_or(false)
}
